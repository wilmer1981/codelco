<? 	
	...
	$CodigoDePantalla = 1;
	$Fechac = date("Y-m-d");
	$Fecha2 = date("Y-m-d", mktime(0,0,0,intval(substr($fechac, 5, 2)) ,intval(substr($fechac, 8, 2)) - 10 ,intval(substr($fechac, 0, 4))));

	$AnoHF = substr($Fechac,0,4)."".substr($Fechac,5,2);
	$AnoHI = substr($Fecha2,0,4)."".substr($Fecha2,5,2);
	//echo $AnoHF."-".$AnoHI;
	...
	if($Proceso == "H")
	{
		//blister liquido
	  	/*if($BlisterPeso == '')
		{*/
			$Hornada_new = $AnoTer."".$MesTer."".$Hornada;
		...
		$Consulta2.= " where hornada = '".$Hornada_new."'";
...
function MostrarPopupProceso(Proceso)
{
	...
	switch (Proceso)
	{
		...
		case "H":
			...
			Frm.action = "raf_ingreso_carga_preparacion.php?Proceso=H" ;
			Frm.submit();	
			break;
...
//			break;
		...
		case "V":
				...
				valores = valores + "&Anito=" + Frm.Ano.value + "&MesJ=" + Frm.Mes.Value;
      ...
      <td valign="top">
		...
		<table width='760' border='0' cellpadding='0' cellspacing='0' class="TablaInterior">
          <tr> 
            ...
            <td width="180">&nbsp;
              <input type="button" name="BtnBuscar" value="Buscar" style="width:60" onClick="MostrarPopupProceso('B');"></td>
