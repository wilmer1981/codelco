<?
	...
	//PROCESO Nº 13 Mezcla Acumulada Ram Cu Peso (Beneficio)
	$Valor=proceso_mez_acum_ram_cu_peso_fino($FechaHoraIni,$FechaHoraFin,'PS');
	proceso_insertar_data(13,$FechaIniReporte,$FechaHoraActual,$Valor,"Mezcla Acumulada Ram Cu Peso (Beneficio) (Ton.)");

	//PROCESO Nº 14 Mezcla Acumulada Ram Cu Fino (Beneficio)
	$Valor=proceso_mez_acum_ram_cu_peso_fino($FechaHoraIni,$FechaHoraFin,'FI');
	proceso_insertar_data(14,$FechaIniReporte,$FechaHoraActual,$Valor,"Mezcla Acumulada Ram Cu Fino (Beneficio) (Ton.)");
	...
	function proceso_mez_acum_ram_cu_peso_fino($FechaHoraIni,$FechaHoraFin,$ValorRetorno)
	{
		$Valor=0;
		$FechaHoraIniMes=substr($FechaHoraIni,0,8)."01 08:00:00";
		$Consulta = " SELECT Sum(t1.peso_seco) AS peso_seco, Sum(t1.fino_cu) AS fino_cu FROM ram_web.movimiento_proveedor t1 INNER JOIN ram_web.conjunto_ram t2 ON (t1.num_conjunto = t2.num_conjunto)";
		$Consulta.= " AND (t1.cod_conjunto = t2.cod_conjunto) INNER JOIN ram_web.movimiento_conjunto t3 ON (t1.num_conjunto = t3.num_conjunto) ";
		$Consulta.= " AND (t1.conjunto_destino = t3.conjunto_destino) AND (t1.fecha_movimiento = t3.fecha_movimiento)";
		$Consulta.= " AND case when t1.cod_conjunto='03' then (t3.cod_lugar_destino <= 12 OR t3.cod_lugar_destino >= 26) AND (t3.cod_existencia = 6 OR t3.cod_existencia = 15) ";
		$Consulta.= " else (t3.cod_lugar_destino > 12 OR t3.cod_lugar_destino < 26) AND (t3.cod_existencia = 6 OR t3.cod_existencia = 15) end ";					
