...
<form name="FrmPrincipal" method="post" action="">
  <table width="300" align="center" border="1" cellspacing="0" cellpadding="0" class="TablaDetalle">
    <tr> 
      <td align="center">B E N E F I C I O S &nbsp;&nbsp;E N&nbsp;&nbsp; R A F</td>
	</tr>	
  </table>
  <br>
  <table width="150" align="center" border="1" cellspacing="0" cellpadding="0" class="TablaDetalle">
    <tr> 
      <td>Fecha : <? echo ucwords(strtolower($Meses[$Mes - 1])).' '.$Ano?></td>
	</tr>	
  </table>
  <br>
  <table width="200" border="0" cellspacing="0" cellpadding="0" class="Detalle01">
    <tr> 
      <td>Beneficio Refino # 1</td>
	</tr>	
  </table>
  <table width="980" border="1" cellspacing="0" cellpadding="0" class="TablaDetalle">
	...
	<?
		...
		{			
			...
			echo'<tr>';
			  ...
			  // esta es una prueba 03-10-2005 
			  $Consulta = "SELECT sum(peso) as peso FROM raf_web.det_carga";
			  $Consulta.= " WHERE hornada = $Fila["hornada"] AND cod_producto = 16 and cod_subproducto NOT IN (40,41,42,43,44,45)";
               //echo $Consulta;
			 ...
			 // echo'<td align="right">&nbsp;</td>';
			  ...
			  //blister potrerillos yo 2005-04-27 	
			  $Consulta = "SELECT sum(peso) as peso FROM raf_web.det_carga";
			  ...
			  //Gran/des1p.	yo 2005-04-27
			 /* $Consulta = "SELECT sum(peso) as peso FROM raf_web.det_carga";
			  $Consulta.= " WHERE hornada = $Fila["hornada"] AND cod_producto = 29 and cod_subproducto = '1'";	
			  $resp = mysql_query($Consulta);
			  $fila = mysql_fetch_array($resp);
			  echo'<td align="right">'.$fila["peso"].'&nbsp;</td>';
			  $TotalDesp = $TotalDesp + $fila["peso"];*/
			  ...
			  //$CargaDiaria = $TotalBlis + $TotalRech + $TotalRest + $TotalCatElec + $TotalCatDesc + $TotalMoldes + $TotalRebal + $TotalAnodCirc + $TotalReten + $TotalBasc;
