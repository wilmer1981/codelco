<? 	
	...
	$Fecha1 = date("Y-m-d");
	//$Fecha1 ="2006-11-08";
	$Fecha2 = date("Y-m-d", mktime(0,0,0,intval(substr($Fecha1, 5, 2)) ,intval(substr($Fecha1, 8, 2)) - 10 ,intval(substr($Fecha1, 0, 4))));
...
//echo $Fecha1."-".$Fecha2;
	...
	if($Proceso == "B" || $Proceso == "H")
	{
		...
		//$Consulta2.=" and left(hornada,4) = '".$Ano."'";
		$Consulta2.=" and left(hornada,6) between '".$AnoI."' and '".$AnoF."'";
	...
	if($Proceso == "E")
	{
		...
		//$Eliminar.= " and left(hornada,4) = '".$Ano."'";
		$Eliminar.= " and left(hornada,6) between '".$AnoI."' and '".$AnoF."'";
...
function calcula(f,j,i,tipo,carga)
{
	...
	switch (tipo)
	{
		case 1:
			...
			{
				f.elements[posi + 1].value = f.elements[posi - 9].value;
				f.elements[posi].value = f.elements[posi - 10].value;
			}
			break;
...
function MostrarPopupProceso(Proceso)
{
	...
	switch (Proceso)
	{
		...
		case "V":
			...
			valores = valores + "&Ano=" + Frm.Ano.value;
...
<form name="FrmPrincipal" method="post" action="">
  ...
  <table width="770" height="340" border="0" cellpadding="0" cellspacing="0" class="TablaPrincipal" left="5">
    <tr> 
      <td valign="top"> <table width='400' border='0' cellpadding='0' cellspacing='0' class="TablaInterior">
        ...
        <table width='760' border='0' cellpadding='0' cellspacing='0' class="TablaInterior">
          <tr> 
            ...
            <td width="52"><input type="button" name="BtnBuscar2" value="Ok" style="width:30" onClick="MostrarPopupProceso('H');"></td>
			...
			<?
			 ...
			 {	
			?>
            <input type="button" name="BtnCerrar2" value="Cerrar Hornada" style="width:100" onClick="MostrarPopupProceso('C');">			
            <?
			 }
			 if($estado == "C")
			 {
...
/*				if($Proceso == "H")
				{*/
					...
					if(strlen($Hornada) == 4)
						...
						$Consulta.= " WHERE right(hornada,5) = $Hornada";
					$Consulta.= " AND turno = '$Row[turno]' and left(hornada,6) between '".$AnoI."' and '".$AnoF."'";
				...
				while ($Fila=mysql_fetch_array($Resultado))
				{
						...
						//CONCATENA TURNO Y PRODUCTO
						echo'<input name="cod_producto['.$i.']" type="hidden" size="2" value="'.$Row[turno]."".$Fila["cod_producto"].'">';
