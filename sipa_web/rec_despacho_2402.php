			...
			if($PatenteOk==true)
			{
				...
				switch($Proceso)
				{
					case "BC"://BUSCAR CORRELATIVO
						...
						$Consulta.="t1.cod_despacho,t1.cod_mop,t1.peso_bruto,t1.peso_tara,t1.observacion from sipa_web.despachos t1 ";
						...
						while($Fila = mysqli_fetch_array($Resp2))
						{
							...
							switch($OrigenDatosGuia)
							{
								...
								case "A"://DATOS ACIDO
									$TitCmbCorr="Corr  -  Guia";
									$Consulta ="SELECT t1.num_guia,t1.nro_patente,t1.rut_chofer,t2.nombre,t1.rut_cliente from pac_web.guia_despacho t1 ";
									...
									$FilaGrupo=mysqli_fetch_array($RespGrupo);
									$CmbGrupoProd=$FilaGrupo["cod_grupo"];
									$CmbProveedor=$FilaPac["rut_cliente"];
									$CmbTipoDespacho='V';
		...
		{		
			...
			if(ValorPorc > 3 || ValorPorc < -3)
			{
				...
				else
				{
					...
					f.action = "rec_despacho_2402.php?LimpiaPag=S&TxtNumBascula="+f.TxtNumBascula.value;
					f.submit();		
				}	
			}
		}
	...
	switch (opt)
	{
		case "G"://ACTUALIZAR DESPACHOS
			if(ValidarCampos())
			{
				...
				f.action = "rec_despacho01_2402.php?Proceso="+f.TipoProceso.value;
				f.submit();	
			}
			break;
			...
			if(f.TxtLote.value==''&&f.TxtRecargo.value=='')
			{
				...
				{				
					if(f.CmbLotes.value=='-1')//ES LOTE NUEVO
						f.action = "rec_despacho_2402.php?Proceso=B1";
					else
						f.action = "rec_despacho_2402.php?Proceso=B2";//LOTE INGRESADO
					f.submit();	
				}	
			}	
			break;
		case "BC":
			...
			{
				f.action = "rec_despacho_2402.php?Proceso=BC&ObjFoco="+ObjFoco.name;//BUSCAR CORRELATIVO
				f.submit();	
			}	
			break;	
		case "I"://IMPRIMIR
			...
			{
				f.action = "rec_despacho01_2402.php?Proceso=I";
				f.submit();	
			}					
			break;	
		case "S"://SALIR
			...
			break;
		case "C"://CANCELAR
			f.action = "rec_despacho01_2402.php?Proceso=C";
			f.submit();	
			break;
		case "M"://MODIFICAR
			f.TipoProceso.value="S";
			...
			{
				f.action = "rec_despacho01_2402.php?Proceso=M";
				f.submit();	
			}	
			break;
		case "A"://ANULAR
			...
			{
				f.action = "rec_despacho01_2402.php?Proceso=A";
				f.submit();	
			}	
			break;
	}
...
function Recarga(ObjFoco,Tipo)
{
	...
	if(ProdSubProd[0]=='18' || ProdSubProd[0]=='19' || ProdSubProd[0]=='48' || (ProdSubProd[0]=='64' && ProdSubProd[1]=='6'))
		f.action = "rec_despacho_2402.php?ObjFoco=TxtPesoNetoSec";
	else
		f.action = "rec_despacho_2402.php?ObjFoco="+ObjFoco.name;
	f.submit();		
}
function ValidarCampos()
{
	...
	if(f.TxtRutChofer.value=='E' && f.TxtRutChofer.value=='')
	{
...
function BuscarProveedor()
{
	...
	{
		f.action = "rec_despacho_2402.php?BuscarPrv=S";
		f.submit();		
	}	
}
function BuscarChofer()
{
	...
	{
		f.action = "rec_despacho_2402.php?BuscarChofer=S";
		f.submit();		
	}	
}
function ValidarPesoNeto()
{
	...
	if(ProdSubProd[0]=='18' || ProdSubProd[0]=='19' || ProdSubProd[0]=='48' || (ProdSubProd[0]=='64' && ProdSubProd[1]=='6'))
	{
		Dif=Math.abs(parseInt(f.TxtPesoBruto.value)-parseInt(f.TxtPesoNetoSec.value));
		if(!(isNaN(Dif)))
		{
			if(Dif>parseInt(f.TxtLimitePeso.value))
			{
				Dif=Math.abs(Dif-parseInt(f.TxtLimitePeso.value))
		...
		{
			alert('Debe Ingresar Peso Neto Guia SEC');
...
<table width="700"  border="0" align="center" cellpadding="2" cellspacing="1" bgcolor="#000000" class="TablaInterior">
  <tr class="ColorTabla01">
    ...
    <input type="text" name="TxtNumBascula" class="InputCen" value="<?php echo $TxtNumBascula;?>" size="2" readonly="true" style="background:#FF0000">
	<?php
	}
	else
	{
	?>
	<input type="text" name="TxtNumBascula" class="InputCen" value="<?php echo $TxtNumBascula;?>" size="2" readonly="true" style="background:#009933"> 
	<?php
	}
	?>
...
&nbsp;OPERANDO EN ROMANA:
<input type="text" name="TxtNumRomana" class="InputCen" value="<?php echo $TxtNumRomana;?>" size="2" readonly="true">
<input name="TxtPesoHistorico" type="text" class="InputCen" value="<?php echo $TxtPesoHistorico; ?>" size="8" readonly="true">
<input name="TxtPorcRango" type="hidden" value="<?php echo $TxtPorcRango; ?>" size="2">
  ...
  <tr>
	...
	<input name="TxtPatente" type="text" class="InputCen" id="TxtPatente2" value="<?php echo strtoupper($TxtPatente); ?>" size="10" maxlength="10" onKeyDown="TeclaPulsada2('N',true,this.form,'TxtCorrelativo');" onBlur="Recarga(TxtCorrelativo,'S')" <?php echo $EstPatente;?>>    
	...
	<input name="TxtPesoBruto" type="text" class="InputCen" value="<?php echo $TxtPesoBruto; ?>" size="10" maxlength="10" onBlur="CalculaPNeto()" readonly="true"></td>
  </tr>
  <tr>
    ...
    <?php
		...
		if($OrigenGuia=='')
		{
			$Consulta3 ="SELECT correlativo from sipa_web.despachos ";
			...
			while($FilaCorr=mysqli_fetch_array($RespCorr))
			{
				...
				if($Datos[0]==$FilaCorr["correlativo"]&&$Datos[1]==$FilaCorr["num_guia"])
					echo "<option value='".$FilaCorr["correlativo"]."~~' SELECTed>".str_pad($FilaCorr["correlativo"],6,0,STR_PAD_LEFT)."</option>";
				else
					echo "<option value='".$FilaCorr["correlativo"]."~~'>".str_pad($FilaCorr["correlativo"],6,0,STR_PAD_LEFT)."</option>";			
			}
    ...
    <td class="ColorTabla02"><input <?php echo $EstadoInput; ?> name="TxtPesoTara" type="text" class="InputCen" id="TxtPesoTara" value="<?php echo $TxtPesoTara; ?>" size="10" maxlength="10" onBlur="CalculaPNeto()" readonly="true"></td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td colspan="5" class="ColorTabla02"><SELECT name="CmbGrupoProd" style="width:150" onChange="Recarga(CmbSubProducto)" onkeypress="buscar_op(this,CmbSubProducto,0,'S')" onBlur="borrar_buffer()" onclick="borrar_buffer()" <?php echo $HabilitarCmb;?>>
  ...
  <tr>
	...
	if($TipoProceso=='S'&&$CmbSubProducto!='S')
	{
		...
		{	
			...
			$Consulta="SELECT * from proyecto_modernizacion.sub_clase where cod_clase='3105' and cod_subclase='1'";
			$RespSec=mysqli_query($link, $Consulta);
			$FilaSec=mysqli_fetch_array($RespSec);
			echo "&nbsp;&nbsp;Limite Peso:<input name='TxtLimitePeso' type='text' class='InputDer' value='".$FilaSec["valor_subclase1"]."' style='background:yellow'  size='4' readonly>&nbsp;Kg.";
  ...
  <tr>
    ...
    <td class="ColorTabla02">
      <SELECT name="CmbTipoDespacho" style="width:160" onkeypress="buscar_op(this,CmbLotes,0,'N')" onBlur="borrar_buffer()" onclick="borrar_buffer()" <?php echo $HabilitarCmb;?>>
  ...
  <tr>
	...
	<input <?php echo $EstadoInput; ?> name="TxtLote" type="text" class="InputCen" id="TxtLote" value="<?php echo $TxtLote; ?>" size="10" maxlength="10" onKeyDown="TeclaPulsada2('S',true,this.form,'BtnOK');" readonly="true">      &nbsp;&nbsp;Rec.:
      <input <?php echo $EstadoInput; ?> name="TxtRecargo" type="text" class="InputCen" id="TxtRecargo" value="<?php echo $TxtRecargo; ?>" size="4" maxlength="2" onKeyDown="TeclaPulsada2('S',true,this.form,'BtnOK');" readonly="true">
      &nbsp;&nbsp;Ult.Rec.:
  ...
  <tr>
    ...
    <td colspan="5" class="ColorTabla02">
    <input name="TxtObs" type="text" class="InputIzq" id="TxtObs" onKeyDown="TeclaPulsada2('N',true,this.form,'TxtRutChofer');" value="<?php echo $TxtObs; ?>" size="100" <?php echo $EstadoInput; ?>></td>
    </tr>
  <tr>
    <td align="right" class="ColorTabla02">Rut Chofer:</td>
    <td colspan="4" class="ColorTabla02"><input name="TxtRutChofer" type="text" class="InputCen" onKeyDown="TeclaPulsada2('N',true,this.form,'TxtNomChofer');" value="<?php echo $TxtRutChofer; ?>" size="14" maxlength="10" <?php echo $EstadoInputChofer; ?> onBlur="BuscarChofer()">
      <input name="TxtNomChofer" type="text" class="InputIzq" id="TxtNomChofer" onKeyDown="TeclaPulsada2('N',true,this.form,'');" value="<?php echo $TxtNomChofer; ?>" size="45" <?php echo $EstadoInputChofer; ?>>
	</td>
	<td align="left" class="ColorTabla02">
<?php
	if($TipoProceso=='S'&&$CmbSubProducto!='S')
	{
		...
		// modif. 03 - 2009
		if($ProdSubProd[0]!='18' && $ProdSubProd[0]!='19' && $ProdSubProd[0]!='48' && ($ProdSubProd[0]!='64' && $ProdSubProd[1]!='6'))
			echo "&nbsp;Sello:<input name='TxtSello' type='text' class='InputIzq' value='$TxtSello' size='10' onKeyDown=TeclaPulsada2('N',true,this.form,'TxtDirec')>";
	}
	?>	
	</td>
    </tr>
  ...
  <tr>
	...
	if($TipoProceso=='S'&&$CmbSubProducto!='S')
	{
		...
		if($ProdSubProd[0]=='18')
			...
			echo "<input name='TxtTransp' type='text' class='InputIzq' value='$TxtTransp' size='70' >";
	}		
	?>
	</td>
	<?php
	...
	<input type="text" name="TxtPNetoTot" readonly="true" value="<?php echo $TxtPNetoTot;?>" size="8" class="InputDer">
	</td>
  </tr>
	</table>
	<br>
