...
function Proceso(opt)
{
	...
	switch (opt)
	{
		...
		case "E"://ELIMINAR
			...
			else
			{
				if (confirm("¿Desea Eliminar este Elemento?"))
				{
					...
					f.action = "inter_empaques01.php?Proceso=E&Valores="+Valores;
					f.submit();
				}
			}
			break; 
		case "R"://RECARGA
			...
			break;
		case "N":	
			var URL = "inter_empaques_proceso.php?Proceso=N";
		...
		case "M":
			...
			else
			{
				if (contador == 1)
				{
					...
					Valores=Valores.substring(0,Largo-2);
					f.ValoresAux.value=Valores;
					var URL = "inter_empaques_proceso.php?Proceso=M&Valores="+Valores;
					window.open(URL,"","top=120,left=30,width=500,height=250,menubar=no,resizable=yes,scrollbars=yes");
  ...
  <table width="770" border="0" cellpadding="5" cellspacing="0" class="TablaPrincipal">
    <tr> 
	  ...
	  <table width="500" border="1" cellspacing="0" cellpadding="3" class="TablaInterior">
        <tr align="center">
          <td class="ColorTabla02"><strong>EMPAQUES</strong></td>
        </tr>
        <tr align="center">
...
<?php	
	$Consulta = "select * from interfaces_codelco.empaque order by ceiling(cod_empaque)";
	$Resp = mysqli_query($link, $Consulta);
	while ($Fila=mysqli_fetch_array($Resp))
	{
		echo "<tr bgcolor=\"#FFFFFF\">\n";
		$ClaveChk=$Fila["cod_empaque"];
		echo "<td ><input type=\"checkbox\" name=\"ChkAsig\" value=\"".$ClaveChk."\"></td>\n";
		echo "<td >".$Fila["cod_empaque"]."</td>\n";
