...
// agregar funcion length para saber largo de num_electrolisis
			...
			if ($largo2 < $largo)
			{
				$AuxElectrolisis = substr($AuxElectrolisis,$ini,$largo2);
			}
	...
	if (($ProdAux=="" && $SubProdAux=="") || ($ProdAux=="33" && $SubProdAux=="2") || ($ProdAux=="31" && $SubProdAux=="1") ||
		($ProdAux=="47" && $SubProdAux=="1") || ($ProdAux=="28" && $SubProdAux=="1"))
	{
		if ($ProdAux!="")
			DefinirArregloLeyes($ProdAux, $SubProdAux, &$ArregloLeyes);	
		else
		...
		while ($FilaAux = mysqli_fetch_array($RespAux))
		{
			...
			$Consulta.= " ON case when t1.cod_producto='31' and t1.cod_subproducto='1' ";
			$Consulta.= " then t2.id_muestra='".$FilaAux["numero"]."' ";
			$Consulta.= " else t2.id_muestra='".$FilaAux["id_analisis"]."' end ";	
