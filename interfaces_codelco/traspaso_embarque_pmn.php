...
function Proceso(opt)
{
	...
	switch (opt)
	{
		...
		case "I"://IMPRIMIR
			...
			break;
		case "C"://CONSULTAR
			f.action = "traspaso_embarque_pmn.php?Mostrar=S&Orden=<?php echo $Orden; ?>";
			f.submit();
			break;
		case "R"://RECARGA
...
if ($Mostrar == "S")
{
	...
	foreach ($ArrResp as $k => $Fila)
	{
		...
		echo '<td align="right">'.number_format($Fila["peso"],1,",",".").'</td>';//CANTIDAD
...
function EscribeSubTotal($Desc, $Reg, $Peso)
{
	...
	echo '<td align="left" colspan="4"><strong>'.$Reg.' reg(s)</strong></strong></td>';
	echo '<td align="left" colspan="4"><strong>>>TOTAL '.$Desc.'</strong></td>';
	echo '<td align="right"><strong>'.number_format($Peso,1,',','.').'</strong></td>';
