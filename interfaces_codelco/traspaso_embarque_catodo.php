...
<html>
<head>
<title>Traspaso Emb. Catodos Grado "A"</title>
...
function Proceso(opt)
{
	...
	switch (opt)
	{
		case "G"://GENERA ARCHIVO		
			...
			else
			{
				if(confirm('¿Esta Seguro de Traspasar Los Datos?'))
				{
					...
					//alert(Valor);
					f.action = "traspaso_embarque_catodo01.php?Proceso=G";
					f.submit();
				}
			}
			break;
		...
		case "C"://CONSULTAR
			...
			f.action = "traspaso_embarque_catodo.php?Mostrar=S&Orden=<?php echo $Orden; ?>";
			f.submit();
			break; 
		case "R"://RECARGA
			f.action = "traspaso_embarque_catodo.php";
			f.submit();
			break;
	}	
}
...
{
	var f = document.frmPrincipal;
	f.action = "traspaso_embarque_catodo.php?Mostrar=S&Orden="+opcion;
	f.submit();
}

function DescargaArchivos()
{
	window.open("descarga.php?Proceso=E&Tipo=CAT&M=GA","","top=35,left=10,width=600,height=400,scrollbars=yes,resizable=YES,toolbar=YES,menubar=YES");
}
    ...
    <tr> 
      <td width="762" height="313" align="center" valign="top">        <table width="550" border="0" cellpadding="2" cellspacing="1" bgcolor="#000000" class="TablaInterior">
          <tr align="center" class="ColorTabla01">
            <td height="23" colspan="5" class="ColorTabla02"><strong>GENERACION DE ARCHIVO DE EMBARQUE PARA &quot;SAP&quot; -CATODOS GRADO &quot;A&quot; </strong></td>
          </tr>
          <tr bgcolor="#FFFFFF">
...
?>		  
	...
	$Consulta.= " where cod_producto='18' and cod_subproducto='40' order by lpad(cod_subproducto,2,'0')";
	$Resp = mysqli_query($link, $Consulta);
	while ($Fila=mysqli_fetch_array($Resp))
	{
...
?>
      ...
      <tr class="ColorTabla01">
		...
		<td><img src="../principal/imagenes/ingles.jpg" alt="Marca Catodos en Ingles"><input name="ChkMarcaCatodo" type="checkbox" id="ChkMarcaCatodo" value="S" onClick="MarcaCatodo(this)"></td>
...
function EscribeSubTotal($Desc, $Reg, $Peso)
{
	...
	echo '<td align="left" colspan="3">&nbsp;</strong></td>';
