...
function Proceso(opt)
{
	...
	switch (opt)
	{
		...
		case "E"://ELIMINAR
			...
			else
			{
				if (confirm("¿Desea Eliminar este Elemento?"))
				{
					...
					f.action = "inter_orden_produccion01.php?Proceso=E&Valores="+Valores;
					f.submit();
				}
			}
			break; 
		case "R"://RECARGA
			...
			break;
		case "N":	
			var URL = "inter_orden_produccion_proceso.php?Proceso=N";
		...
		case "M":
			...
			else
			{
				if (contador == 1)
				{
					...
					Valores=Valores.substring(0,Largo-2);
					f.ValoresAux.value=Valores;
					var URL = "inter_orden_produccion_proceso.php?Proceso=M&Valores="+Valores;
					window.open(URL,"","top=120,left=30,width=700,height=350,menubar=no,resizable=yes,scrollbars=yes");
  ...
  <table width="770" border="0" cellpadding="5" cellspacing="0" class="TablaPrincipal">
    <tr> 
      <td width="762" height="313" align="center" valign="top"><table width="620" border="1" cellspacing="0" cellpadding="3" class="TablaInterior">
        <tr align="center">
          <td class="ColorTabla02"><strong>ORDENES DE PRODUCCION </strong></td>
        </tr>
        <tr align="center">
...
<?php	
	...
	while ($Fila=mysqli_fetch_array($Resp))
	{
		...
		$ClaveChk=$Fila["asignacion"]."~".$Fila["cod_producto"]."~".$Fila["cod_subproducto"]."~".$Fila["codigo_op"];
