...
function Proceso(opt,opt2,opt3)
{
	...
	{
		case "P"://PETALOS
			f.action = "age_adm_cierre_lote.php?Petalo="+opt2;
			f.submit();	
			break;
		...
		case "E"://EXCEL	
			...
			break;		
		case "R"://RECARGA					
			f.action = "age_adm_cierre_lote.php";
			f.submit();
			break
		case "S"://SALIR
...
{
	window.open("age_con_leyes_historicas_mensual.php?PopUp=S&MesAnt=S&CmbSubProducto="+ CmbSubProducto+"&CmbProveedor="+CmbProveedor+"&CmbAnoIni="+CmbAnoIni+"&CmbAnoFin="+CmbAnoFin+"&LeyUnica="+LeyUnica+"&Mes="+Mes,"","top=50,left=10,width=1000,height=450,scrollbars=yes,resizable = yes");					
}
...
<table width="750"  border="1" align="center" cellpadding="2" cellspacing="0" class="TablaInterior">
  ...
  <tr class="Colum01">
    <td width="120" class="Colum01">Lote:</td>
  ...
  <tr align="left" class="Detalle01">
	...
	<?php
		...
		echo "</tr>";
		//SOLICITUD DEL LOTE
		$Consulta = "select distinct t2.nro_solicitud ,t2.nro_sa_lims ,t2.recargo , t2.estado_actual, t3.nombre_subclase";
		...
		//RETALLA
		$Consulta = "select distinct t2.nro_solicitud, t2.nro_sa_lims, t2.estado_actual, t3.nombre_subclase";
		$Consulta.= " from age_web.lotes t1 ";
		$Consulta.= " inner join cal_web.solicitud_analisis t2 on t1.lote=t2.id_muestra  ";
		$Consulta.= " inner join proyecto_modernizacion.sub_clase t3 on t3.cod_clase='1002' and t3.cod_subclase=t2.estado_actual  ";
		$Consulta.= " where t1.lote = '".$TxtLote."' ";			
		...
		if ($FilaAux=mysqli_fetch_array($RespAux))
		{
			if ($FilaAux["nro_sa_lims"]=='') {
				...
				$SA_Retalla = $FilaAux["nro_solicitud"];
				$SALims_Retalla = $FilaAux["nro_sa_lims"];
			}
		...
		if ($FilaAux=mysqli_fetch_array($RespAux))
		{
			if ($FilaAux["nro_sa_lims"]=='') {
				...
				$SA_Paralela = $FilaAux["nro_solicitud"];
				$SALims_Paralela = $FilaAux["nro_sa_lims"];
			}
		...
		if 	($SA_Retalla==""){
			...
			if ($SALims_Retalla=='') {
					...
					echo "<td><a href=\"JavaScript:Historial('".$SA_Retalla."','R')\" class=\"LinksAzul\">".$SALims_Retalla."</a></td>\n";
				}
		...
		if 	($SA_Paralela==""){
			...
			if ($SALims_Paralela=='') {
					...
					echo "<td><a href=\"JavaScript:Historial('".$SA_Retalla."','R')\" class=\"LinksAzul\">".$SALims_Paralela."</a></td>\n";
				}
