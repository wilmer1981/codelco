...
<form name="frmPrincipal" action="" method="post">
  ...
  <?php
		...
		while ($FilaAsig = mysqli_fetch_array($RespAsig))
		{
			...
			echo "</tr>\n";
			$Consulta = "select STRAIGHT_JOIN distinct case when (t1.valor_subclase1)='METALICO' then 'METALICO' else 'MINERO' end  as valor_subclase1,t1.valor_subclase2 ";
			...
			while($FilaClaseProd=mysqli_fetch_array($RespClaseProd))
			{
				...
				while($FilaProd=mysqli_fetch_array($RespProd))
				{
					...
					$Consulta = "select STRAIGHT_JOIN distinct t1.rut_proveedor, t1.cod_recepcion,t2.nombre_prv as nom_prv  ";
					...
					{						
						...
						//TOTAL PROVEEDOR
						$DecPHum=0;$DecPSeco=0;$DecLeyes=8;$DecFinos=0;
						if($EsPlamen==true)
						{
							$DecPHum=4;$DecPSeco=4;$DecLeyes=4;$DecFinos=4;
						}
						...
						echo "</tr>\n";
						if($FilaProd["cod_producto"]==1&&$FilaProd["cod_subproducto"]==3)
						{
							$RutPrvRam=STR_PAD($FilaPrv["rut_proveedor"],10,"0",STR_PAD_LEFT);
							$ActualizarRAM="UPDATE ram_web.leyes_agencia set ph='".$TotalPesoHumPrv."', pseco='".$TotalPesoSecPrv."',fin_cu='".number_format($LeyCuPrv,$DecLeyes,'.',',')."',fin_ag='".number_format($LeyAgPrv,$DecLeyes,'.',',')."',fin_au='".number_format($LeyAuPrv,$DecLeyes,'.',',')."'";
							$ActualizarRAM.=" WHERE ano = '".$CmbAno."' AND mes = '".$CmbMes."' AND trecep = '03' and vended='".$RutPrvRam."'";	
							//echo $ActualizarRAM."<br>";
							mysqli_query($link, $ActualizarRAM);	
						}
