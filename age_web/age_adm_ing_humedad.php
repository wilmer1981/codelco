<?php
	...
	if (isset($TxtLote))
	{
		...
		if ($Fila = mysqli_fetch_array($Resp))
		{
			...
			$TxtLote = $Fila["lote"];
			$ConDetalle="select observacion,guia_despacho from age_web.detalle_lotes where lote='".$TxtLote."'";
			//echo $ConDetalle;
			$RespDet=mysqli_query($link, $ConDetalle);
			$FilaDet=mysqli_fetch_array($RespDet);
			$Observacion=explode('-',$FilaDet["observacion"]);	
			//echo '-'.$Observacion[1].'     -'.$FilaDet["guia_despacho"]; 		
			//-------------------------------MUESTRA SOLO Y CUANDO LA OBSERVACION ES IGUAL A LA GUIA DE DESPACHO MAS UN GUIN ADELANTE-------------- O SEA CARGADO DESDE EXCEL.
			$MuestraLote='N';
			if('-'.$Observacion[1]=='-'.$FilaDet["guia_despacho"])
			{
				$MuestraLote='S';
			...
			}//SI LA OBSERVACION ES IGUAL A - MAS GUIA DESPACHO	
		}
	}
?>
<html>
...
function Proceso(opt,opt2)
{
	...
	{
		case "P"://PETALOS
			f.action = "age_adm_ing_humedad.php?Petalo="+opt2+"&EsPopup="+f.TxtEsPopup.value;
			f.submit();	
			break;
		case "I"://IMPRIMIR			
			if (f.TxtLote.value=="")
			...
			window.open("age_adm_cierre_lote_imprimir.php?TxtLote="+f.TxtLote.value+"&Petalo="+opt2+"&CmbPlantilla="+f.CmbPlantilla.value,"","top=30,left=2,width=770,heiht=500,scrollbars=yes,resizable=yes");
			break;
		case "E"://EXCEL	
			if (f.TxtLote.value=="")
			{
			...
			break;		
		case "R"://RECARGA					
			f.action = "age_adm_ing_humedad.php";
			f.submit();
			break
		case "S"://SALIR
...
function IngHum(SA,ID,Recargo)
{
	var f = document.frmPrincipal;
	window.open("age_adm_ing_humedad_popup.php?SA="+SA+"&IDMuestra="+ID+"&Recargo="+Recargo,"","top=100,left=90,width=550,height=120,scrollbars=yes,resizable=yes,status=yes");
}
</script>
...
<table width="750"  border="1" align="center" cellpadding="2" cellspacing="0" class="TablaInterior">
  <tr class="ColorTabla02">
    <td colspan="4"><strong>INGRESO HUMEDAD PARA LOTES EXTERNOS CARGADOS DESDE EXCEL. </strong></td>
  </tr>
  <tr class="Colum01">
  ...
  <tr class="Colum01">
    ...
    <td class="Colum01"><strong>
    <?php if(isset($MuestraParalela)) echo $MuestraParalela."&nbsp;&nbsp;"; else echo "&nbsp;";?></strong>	</td>
  </tr>
  <tr class="ColorTabla02">
