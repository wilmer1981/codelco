...
function Proceso(opcion)
{
	...
	switch (opcion)
	{
		case "S":
			...
			break;	
		case "E":
			f.action = "age_con_leyes_historicas_mensual_excel.php";			
			f.submit();
			break;
		case "I":
...
{
	var f = document.frmPrincipal;
	f.action = "age_con_leyes_historicas_lote.php?PopUp="+popup+"&CmbAno="+ano+"&CmbMes="+mes+"&CmbProductoAux="+prod+"&CmbSubProductoAux="+subprod+"&CmbProveedorAux="+prov;			
	f.submit();
}
//-->
</script>
...
{		
    ...
    <tr>
      <td>Rango:</td>
    <td><?php echo $CmbAnoIni." al ".$CmbAnoFin; ?></td>
    </tr>
<?php	
if ($Plantilla!="S")
{
...
<?php		
	...
	echo "</tr>\n";
	$Consulta = "select ano, mes, cod_producto, cod_subproducto, rut_proveedor, nombre_prv as nom_proveedor,sum(peso_humedo) as peso_humedo, sum(peso_seco) as peso_seco ";
	for ($i=1;$i<=60;$i++)
	{
