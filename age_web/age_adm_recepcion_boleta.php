<?php
	...
	foreach($Datos as $c => $v)
	{
		...
		$Lote = $Datos2[0];
		$Recargo = $Datos2[1];
		$Consulta = "select t2.bascula, t2.bascula1, t2.rut_romanero, t1.lote, t2.corr, t2.peso_bruto,  ";
		$Consulta.= " t2.hora_entrada, t2.fecha_recepcion, t2.recargo, t2.fin_lote, t1.fecha_vence_padron, ";
		$Consulta.= " t2.peso_tara, t2.hora_salida, t2.peso_neto, t3.nomprv_a as nom_proveedor, t1.rut_proveedor, ";
		$Consulta.= " t4.descripcion as nom_faena, t4.sierra, t4.comuna, t1.cod_faena, t1.cod_producto, t1.cod_subproducto, ";
		$Consulta.= " t5.descripcion as nom_subproducto, t6.desc_a as tipo_recepcion, t1.clase_producto, t1.num_conjunto, t1.cancha, ";
		$Consulta.= " t7.desc_a as nom_descarga, t2.pastas, t2.impurezas, t2.observacion, t2.patente, t2.guia_despacho, t1.contrato"; 
		$Consulta.= " from age_web.lotes t1 inner join age_web.detalle_lotes t2 ";
		$Consulta.= " on t1.lote=t2.lote left join rec_web.proved t3 on t1.rut_proveedor=t3.rutprv_a ";
		$Consulta.= " left join age_web.mina t4 on t1.cod_faena=t4.cod_faena ";
		...
		$Consulta.= " left join rec_web.tipos t6 on t1.cod_recepcion=t6.cod_c ";
		$Consulta.= " left join rec_web.descarga t7 on t2.cod_descarga=t7.cod_c ";
		$Consulta.= " where t1.lote = '".$Lote."' and t2.recargo='".$Recargo."'";
		...
		if ($Fila = mysqli_fetch_array($Resp))
		{
			...
			$pdf->rectangle(310,700, 70,30);
			$pdf->line(310,715,380,715);
			$pdf->addTextWrap(310,720,70,8,"ROMANA",$justification='center',0,0);	
			
			$pdf->rectangle(420,700, 80,30);
			$pdf->line(420,715,500,715);
			$pdf->addTextWrap(420,720,80,8,"BASCULA",$justification='center',0,0);	
			...
			$pdf->rectangle(310,665, 70,30);
			$pdf->line(310,680,380,680);
			$pdf->addTextWrap(310,685,70,8,"PESO BRUTO",$justification='center',0,0);			
			...
			$pdf->rectangle(310,630, 70,30);
			$pdf->line(310,645,380,645);
			$pdf->addTextWrap(310,650,70,8,"PESO TARA",$justification='center',0,0);			
			...
			$pdf->addTextWrap(310,705,70,9,str_pad($Fila["bascula1"],2,'0',STR_PAD_LEFT),$justification='center',0,0);
			$pdf->addTextWrap(420,705,80,9,$Fila["bascula"]." - ".$Fila["bascula1"],$justification='center',0,0);
			$pdf->addTextWrap(510,705,80,9,$Fila["rut_romanero"],$justification='center',0,0);
			$pdf->addTextWrap(0,670,70,9,$Fila["lote"],$justification='center',0,0);
			$pdf->addTextWrap(80,670,100,9,$Fila["corr"],$justification='center',0,0);
			...
			$pdf->addTextWrap(510,670,80,9,$Fila["fecha_recepcion"],$justification='center',0,0);
			$pdf->addTextWrap(15,635,50,9,$Fila["recargo"],$justification='center',0,0);
			$pdf->addTextWrap(110,635,35,9,$Fila["fin_lote"],$justification='center',0,0);
			$pdf->addTextWrap(190,635,90,9,substr($Fila["fecha_vence_padron"],8,2)."/".substr($Fila["fecha_vence_padron"],5,2)."/".substr($Fila["fecha_vence_padron"],0,4),$justification='center',0,0);
			...
			$pdf->addTextWrap(455,590,135,9,$Fila["rut_proveedor"],$justification='center',0,0);
			$pdf->addTextWrap(0,550,250,9,$Fila["nom_faena"],$justification='center',0,0);
			$pdf->addTextWrap(230,550,240,9,strtoupper($Fila["sierra"]." / ".$Fila["comuna"]),$justification='center',0,0);
			$pdf->addTextWrap(455,550,135,9,$Fila["cod_faena"],$justification='center',0,0);
			...
			$pdf->addTextWrap(230,495,80,9,strtoupper($Fila["tipo_recepcion"]),$justification='center',0,0);
			...
			$pdf->addTextWrap(390,495,55,9,$Fila["num_conjunto"],$justification='center',0,0);	
