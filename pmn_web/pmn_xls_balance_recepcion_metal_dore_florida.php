...
<br>
<strong>
Informe: Recepcion Metal Dore Florida<br>
...
<table width="1100" border="1" cellspacing="0" cellpadding="3" class="TablaInterior">
  <tr class="ColorTabla01"> 
    ...
    <td width="103" rowspan="2" align="center">Peso</td>
...
?>
  ...
  <?php
	...
	{   
		...
		$consulta.= " '' AS peso_humedo, '' AS humedad, (t1.peso_bruto - t1.peso_resta) AS peso_seco, 'N' AS bloqueado,";
		...
		$consulta.= " '' AS peso_total, '' AS peso_muestra, t3.cod_leyes, t3.valor, (t3.valor * (t1.peso_bruto - t1.peso_resta) / CASE WHEN t3.cod_leyes IN ('04','05','37','38') THEN 1000000 ELSE 100 END) AS fino";				
				
		$consulta.= " FROM pmn_web.detalle_productos_externos AS t1";
	...
	while($row = mysqli_fetch_array($rs))
	{
		if (($IdProductoAnt != $row[id_producto]) or ($ReferenciaAnt != $row[referencia]))
		{
			...
			if ($row[id_producto] == '')
			{
				echo '<td colspan="3">'.$row[referencia].'</td>';
			}
			else
			{
