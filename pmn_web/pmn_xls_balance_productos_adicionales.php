...
<?php
	...
	echo "Informe: ".$row["nombre_subclase"];
...
<table width="750" border="1" cellspacing="0" cellpadding="3" class="TablaInterior">
  ...
  <?php
	...
	{  
		...
		$consulta.= " t1.peso_humedo, t1.humedad, CASE WHEN t1.signo = '-' THEN (-1 * t1.peso_seco) ELSE t1.peso_seco END AS peso_seco,";		
		...
		$consulta.= " CASE WHEN t2.cod_leyes = '02' THEN (t1.fino_cu * unid_cu / CASE WHEN t1.signo = '-' THEN (-1 * t1.peso_seco) ELSE t1.peso_seco END) ELSE";
		$consulta.= " CASE WHEN t2.cod_leyes = '04' THEN (t1.fino_ag * unid_ag / CASE WHEN t1.signo = '-' THEN (-1 * t1.peso_seco) ELSE t1.peso_seco END) ELSE";
		$consulta.= " CASE WHEN t2.cod_leyes = '05' THEN (t1.fino_au * unid_au / CASE WHEN t1.signo = '-' THEN (-1 * t1.peso_seco) ELSE t1.peso_seco END) END END END AS valor";		
		...
		$consulta.= " ON t9.cod_producto = '".$cmbproducto."' AND t9.cod_subproducto = '".$cmbsubproducto."' AND t9.tipo_mov = t1.tipo_mov AND t1.signo = t9.signo";				
		...
		$consulta.= " ORDER BY CEILING(t1.tipo_mov),t2.cod_leyes";
		mysqli_query($link, $consulta);
	}	
	
	$consulta = "SELECT t1.fecha, t1.lote AS id, t1.peso_humedo, t1.humedad, t1.peso_seco, t1.cod_leyes, t1.valor, t1.conversion";
