...
<br>
<strong>Informe :
Beneficio Barro Anodico Externo - Codelco <br>
Periodo: <?php echo substr($FechaIni,8,2).'-'.substr($FechaIni,5,2).'-'.substr($FechaIni,0,4).' AL '.substr($FechaFin,8,2).'-'.substr($FechaFin,5,2).'-'.substr($FechaFin,0,4) ?></strong><br>
<br>
...
?>
	...
	{   
		...
		$consulta.= " ON t1.id_producto = t2.observacion";
		...
		$consulta.= " ON t1.id_producto = t4.observacion AND t1.cod_producto = t4.cod_producto";
	...
	$consulta = "SELECT t1.fecha, t1.lote AS id_producto, hornada, t1.tambor AS referencia, t1.peso_humedo, t1.id_muestra, t1.humedad, t1.peso_seco, t1.cod_leyes, t1.valor AS ley, t1.conversion AS unidad, t1.fino";
	...
	while($row = mysqli_fetch_array($rs))
	{
		if ($IdProductoAnt != $row[id_producto])
		{
			...
			$sw = true;			
			echo '<tr>';
			echo '<td rowspan="'.$TotalFilas.'" align="center">'.$row[id_producto].'</td>';						
		}
