<?php
	...
	*/
	...
	if(isset($_REQUEST["ModRec"])){
		$ModRec=$_REQUEST["ModRec"];
	}else{
		$ModRec="";
	}
	...
	if(isset($_REQUEST["SInt"])){
		$SInt=$_REQUEST["SInt"];
	}else{
		$SInt="";
	}
	if(isset($_REQUEST["SExt"])){
		$SExt=$_REQUEST["SExt"];
	}else{
		$SExt="";
	}
	...
	if(isset($_REQUEST["PesoTotal"])){
		$PesoTotal=$_REQUEST["PesoTotal"];
	}else{
		$PesoTotal=0;
	}
	...
	switch($Opcion)
	{
		...
		default;
			if($LR=='')
			{
				$AnoDefault=$Anor;
				$MEsDefault=$Mesr;
				$Lote=ObtengoMaximo('pmn_pesa_bad_cabecera','lote',"year(fecha_hora)='".$AnoDefault."' and month(fecha_hora)='".$MEsDefault."'",$link);
				if(strlen($Lote)==1)
				{
					$Ano=$AnoDefault;
			...
			$Recargo=ObtengoMaximo('pmn_pesa_bad_detalle','recargo',"lote='".$Lote."'",$link);
...
<input type="hidden" name="BACLote" id="BACLote" value="<?php echo $BAC;?>">
	...
	<table width="761" cellpadding="3" cellspacing="0" align="center" class="TablaInterior">
          ...
          <tr> 
			...
			if($BAC=='S')
			{
				...
				$Respuesta = mysqli_query($link, $Consulta);$ExisteAlmenos1BAC='N';
				$Disabled='';
				if($Row = mysqli_fetch_array($Respuesta))
					$ExisteAlmenos1BAC='S';
				if($ExisteAlmenos1BAC=='S')
					$Disabled='disabled=disabled';		
			}
			?>
			&nbsp;&nbsp;<?php if($Mostrar=='S') { ?>Es BAC:&nbsp;<input type="checkbox" name="BAC" <?php echo $Disabled;?> <?php echo $ChkBAC;?> onClick="Proceso('BAC')"><?php }?></td>
          </tr>
		  <?php
		  if($BAC!='S')
		  {
		  ...
		  ?>
          <tr> 
            <td class="titulo_azul">Num. lixiviaci&oacute;n</td>
            <td colspan="7">
          ...
          <tr> 
            <td valign="top"> 
              <table width="100%" border="1" cellpadding="0" cellspacing="0" class="TablaDetalle">
				...
				$Consulta = "select * from pmn_web.pmn_pesa_bad_detalle ";
				$Consulta.= " where lote='".$Lote."' and recargo in('1','2','3','4')";
				$Consulta.= " order by recargo";
				$Respuesta = mysqli_query($link, $Consulta);
				$i=1;
				$Total02=0;
				...
				while ($Row = mysqli_fetch_array($Respuesta))
				{
					...
					if($Row["recargo"]!=$RecAux && $RecAux!='')
					{
						...
						<td colspan="5" align='right'>Sub-Total Tambor <?php echo $RecAux;?></td>
						...
						<?php
						$TotPaletA1=$TotPaletA1+$SubTotRec1;
						$TotPaletA2=$TotPaletA2+$SubTotRec2;
						$TotPaletA3=$TotPaletA3+$SubTotRec3;
				...
				?>
				<tr class="TituloCabecera"><td colspan="5" align='right' class="TituloCabeceraOz">Sub-Total Palet A</td>
				<td align='right' class="TituloCabeceraOz"><?php echo $TotPaletA1;?></td>
				<td align='right' class="TituloCabeceraOz"><?php echo $TotPaletA2;?></td>
				<td align='right' class="TituloCabeceraOz"><?php echo $TotPaletA3;?></td>
				<td colspan="3" class="TituloCabeceraOz">&nbsp;</td>
				</tr>
				<?php
				$TotalesFinales1=$TotPaletA1;
				$TotalesFinales2=$TotPaletA2;
				$TotalesFinales3=$TotPaletA3;
				...
				$Respuesta = mysqli_query($link, $Consulta);
				if($Row = mysqli_fetch_array($Respuesta))
					$UltimoCant=$Row["recargo"];
				$Consulta = "select * from pmn_web.pmn_pesa_bad_detalle ";
				$Consulta.= " where lote='".$Lote."' and recargo in('5','6','7','8')";
				$Consulta.= " order by recargo";
				$Respuesta = mysqli_query($link, $Consulta);
				$i=5;
				$Total02=0;
				$Total03=0;
				$Total04=0;$RecAux='';$SubTotRec1=0;$SubTotRec2=0;$SubTotRec3=0;
				echo "<input type='hidden' name='Selected' />";
				while ($Row = mysqli_fetch_array($Respuesta))
				{
					//$Row["pneto"]=0;
				...
				?>
				<tr class="TituloCabecera"><td colspan="5" align='right' class="TituloCabeceraOz">Sub-Total Palet B</td>
					<td align='right' class="TituloCabeceraOz"><?php echo $TotPaletA1;?></td>
					<td align='right' class="TituloCabeceraOz"><?php echo $TotPaletA2;?></td>
					<td align='right' class="TituloCabeceraOz"><?php echo $TotPaletA3;?></td>
					<td colspan="3" class="TituloCabeceraOz">&nbsp;</td>
				...
				<?php
				$TotalesFinales1=$TotalesFinales1+$TotPaletA1;
				$TotalesFinales2=$TotalesFinales2+$TotPaletA2;
				$TotalesFinales3=$TotalesFinales3+$TotPaletA3;
				?>
				<tr align="center"> 
				  <td height="15" colspan="3" align="right" class="TituloCabeceraCobre"><strong>TOTAL</strong></td>
				  <td align="right" class="TituloCabeceraCobre">&nbsp;</td>
				  <td align="right" class="TituloCabeceraCobre">&nbsp;</td>
				  <td align="right" class="TituloCabeceraCobre"><?php echo number_format($TotalesFinales1,2,',','.'); ?></td>
				  <td align="right" class="TituloCabeceraCobre"><?php echo number_format($TotalesFinales2,2,',','.'); ?></td>
				  <td align="right" class="TituloCabeceraCobre"><?php echo number_format($TotalesFinales3,2,',','.'); ?></td>
				  <td align="right" class="TituloCabeceraCobre">&nbsp;</td>
				  <td align="right" class="TituloCabeceraCobre">&nbsp;</td>
				  <td align="right">&nbsp;</td>
				</tr>
