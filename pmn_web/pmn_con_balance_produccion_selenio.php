...
function Guardar()
{
	...
	{	
		f.action = "pmn_con_balance_produccion_selenio.php?proceso=G";
		f.submit();
	}
}
/******************/
...
<table width="600" border="1" align="center" cellpadding="3" cellspacing="0" class="TablaInterior">
  <tr> 
    ...
    <td width="468" align="left">Produccion Selenio Crudo</td>
  </tr>
  <tr> 
    <td>Periodo</td>
...
<br>
  <table width="950" border="1" cellpadding="3" cellspacing="0" class="TablaInterior">
    <tr class="ColorTabla01"> 
      ...
      <td width="72" rowspan="2" align="center">Peso Humedo</td>
      <td width="60" rowspan="2" align="center">H<sub>2</sub>O<br>
        (%)</td>
      <td width="68" rowspan="2" align="center">Peso Seco</td>
	...
	{
?>	
      <td width="55" align="center">Se (%)</td>
      <td width="63" align="center">Cu (ppm)</td>
      <td width="63" align="center">Ag (g/T)</td>
      <td width="64" align="center">Au (g/T)</td>
      <td width="66" align="center">Sb (ppm)</td>
      <td width="66" align="center">Fe (ppm)</td>
      <td width="60" align="center">Pb (ppm)</td>
      <td width="57" align="center">Te (ppm)</td>
    <?php
	}
	else
	{
?>
      <td width="55" align="center">Se (Kg)</td>
      <td width="63" align="center">Cu (Kg)</td>
      <td width="63" align="center">Ag (Kg)</td>
      <td width="64" align="center">Au (Kg)</td>
      <td width="66" align="center">Sb (Kg)</td>
      <td width="66" align="center">Fe (Kg)</td>
      <td width="60" align="center">Pb (Kg)</td>
      <td width="57" align="center">Te (Kg)</td>
    <?php
	}
?>	  
    </tr>
	...
	{   
		...
		$consulta.= " CASE WHEN t3.cod_leyes = '40' THEN 100 ELSE 1000000 END AS conversion,";
		$consulta.= " '' AS num_cajon, '' AS horanada, '' AS num_barra, '' AS num_electrolisis, '' AS num_anodos,";				
		$consulta.= " '' AS peso_total, '' AS peso_muestra, t3.cod_leyes, t3.valor, (t3.valor * (t1.peso - (t1.peso * IFNULL(t5.valor,0) / 100)) / CASE WHEN t3.cod_leyes = '40' THEN 100 ELSE 1000000 END) AS fino";								
		...
		$consulta.= " AND t2.cod_periodo = '1' AND t2.agrupacion = '6' AND t2.recargo = 0 AND t2.estado_actual IN ('5','6')";
		...
		$consulta.= " AND (t3.cod_leyes IN ('40', '02', '31', '09', '39', '44', '04', '05') OR ISNULL(t3.cod_leyes))";
	...
	$consulta = "SELECT t1.fecha AS fecha_produccion, t1.lote AS numero, t1.cod_leyes, t1.valor, CASE WHEN t1.cod_leyes = '40' THEN 1 ELSE 2 END AS orden, t1.humedad, t1.peso_humedo, t1.peso_seco, t1.conversion, t1.fino";
	...
	$consulta.= " ORDER BY t1.fecha_produccion, t1.numero, orden, t1.cod_leyes";
	//echo $consulta."<br>";		
	   
	//---.
	$Finos = array('40'=>0, '02'=>0, '04'=>0, '05'=>0, '09'=>0, '31'=>0, '39'=>0, '44'=>0); //La Posicion Cero(0), es para almacenar el peso. 
	$Unidad = array('40'=>100, '02'=>1000000, '04'=>1000, '05'=>1000, '09'=>1000000, '31'=>1000000, '39'=>1000000, '44'=>1000000);
	$TotalPesoHumedo = 0;
	$TotalPesoSeco = 0;
	$TotalHumedad = 0;
	$CantidadReg = 0; 
