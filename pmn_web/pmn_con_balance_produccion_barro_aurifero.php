...
<table width="600" border="1" align="center" cellpadding="3" cellspacing="0" class="TablaInterior">
  <tr> 
    ...
    <td width="468" align="left">Produccion Barro Aurifero Lixiviado</td>
  </tr>
  <tr> 
    <td>Periodo</td>
...
<?php
	...
	{   
		...
		$consulta.= " '' AS peso_humedo, '' AS humedad, (ROUND(t1.peso_seco + 0.0001,3) + ROUND(IFNULL(t1.muestra01,0) + 0.0001,3)) AS peso_seco, 'N' AS bloqueado, 100 AS conversion,";								
		$consulta.= " '' AS num_caja, '' AS horanada, '' AS num_barra, t1.num_electrolisis AS num_electrolisis, '' AS num_anodos,";				
		$consulta.= " ROUND(t1.peso_seco + 0.0001,3) AS peso_total, ROUND(t1.muestra01 + 0.0001,3) AS peso_muestra, t3.cod_leyes, t3.valor, ROUND((t3.valor * (ROUND(t1.peso_seco + 0.0001,3) + ROUND(IFNULL(t1.muestra01,0) + 0.0001,3)) / 100) + 0.0001,3) AS fino";
	...
	//Los peso secos y total estan invertidos, para el anexo.
	$consulta = "SELECT t1.fecha, t1.num_electrolisis, t1.peso_total AS peso, t1.peso_seco AS peso_total, t1.peso_muestra, t1.cod_leyes, t1.valor, t1.conversion AS unidad, t1.fino";
