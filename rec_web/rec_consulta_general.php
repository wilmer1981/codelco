...
BORDER-RIGHT:solid 2px #000000; VISIBILITY: hidden; POSITION: absolute" onclick=event.cancelBubble=true>
        <IFRAME name=popFrame src="../principal/popcjs.htm" frameBorder=0 width=160 scrolling=no height=185></IFRAME>
    </DIV>
...
//					echo "<option value='O'>Otros Pesajes</option>\n";
			...
			?>
                        ...
                        <tr bgcolor="#FFFFFF">
                            <td>Fecha Inicio:</td>
                            <td><input name="TxtFechaIni" type="text" class="InputCen"
                                    value="<?php echo $TxtFechaIni; ?>" size="15" maxlength="10" readOnly>
                                <img src="../principal/imagenes/ico_cal.gif" alt="Pulse Aqui ParaSeleccionar Fecha"
                                    width="16" height="16" border="0" align="absmiddle"
                                    onClick="popFrame.fPopCalendar(TxtFechaIni,TxtFechaIni,popCal);return false">
                            </td>
                            <td>Fecha Termino:</td>
                            <td><input name="TxtFechaFin" type="text" class="InputCen" id="TxtFechaFin"
                                    value="<?php echo $TxtFechaFin; ?>" size="15" maxlength="10" readOnly>
                                <img src="../principal/imagenes/ico_cal.gif" alt="Pulse Aqui ParaSeleccionar Fecha"
                                    width="16" height="16" border="0" align="absmiddle"
		  ...
		  if (($TipoConsulta == "R") || ($TipoConsulta==""))
		  {
				...
				{
					if ($RutProveedor == $Row["rutprv_a"])
						echo "<option selected value='".$Row["rutprv_a"]."'>".str_pad($Row["rutprv_a"],10,'0',STR_PAD_LEFT)." - ".strtoupper($Row["nomprv_a"])."</option>\n";
					else
						echo "<option value='".$Row["rutprv_a"]."'>".str_pad($Row["rutprv_a"],10,'0',STR_PAD_LEFT)." - ".strtoupper($Row["nomprv_a"])."</option>\n";
				}
				/* 2010-01-20
				while ($Row = mysqli_fetch_array($Respuesta))
				{
			  ...
			  else
			 {
				if ($TipoConsulta == "D")
				{
					...
					echo "<td><select name='Producto' style='width:300px' onChange=\"Proceso('R');\">\n";
					echo "<option value='S'>Todos los Productos</option>\n";
				//	$Consulta = "select distinct lpad(cod_producto,2,'0') as c_prod_a, cod_subproducto as cod_subproducto,cod_producto from sipa_web.despachos t1 ";
					$Consulta = "select distinct cod_producto c_prod_a, cod_subproducto as cod_subproducto from sipa_web.despachos t1 ";
					$Consulta.= " where (cod_producto * 1)<>0 and t1.fecha>= date_sub(now(), interval 12 month)  order by lpad(cod_producto,2,'0')";
					$Resp = mysqli_query($link, $Consulta);
					while ($Fila=mysqli_fetch_array($Resp))
					{
						...
						if ($Fila2=mysqli_fetch_array($Resp2))
						{
							...
							echo "<option selected value='".$Fila["c_prod_a"]."-".$Fila["cod_subproducto"]."'>".str_pad($Fila["c_prod_a"],2,'0',STR_PAD_LEFT)." ".str_pad($Fila["cod_subproducto"],2,'0',STR_PAD_LEFT)." ".strtoupper($Fila2["descripcion"])."</option>\n";
							else
							echo "<option value='".$Fila["c_prod_a"]."-".$Fila["cod_subproducto"]."'>".str_pad($Fila["c_prod_a"],2,'0',STR_PAD_LEFT)." ".str_pad($Fila["cod_subproducto"],2,'0',STR_PAD_LEFT)." ".strtoupper($Fila2["descripcion"])."</option>\n";
						}
	...
	//				$prod = split('[-]', $Producto);
					...
					$Consulta.=" and fecha>= date_sub(now(), interval 12 month) group by rut_prv";
                                        $Respuesta = mysqli_query($link, $Consulta);
					while ($Row = mysqli_fetch_array($Respuesta))
					{					
							...
							{
									if ($RutProveedor == $Row2["rut_prv"])
									{	
										echo "<option selected value='".$Row2["rut_prv"]."'>".str_pad($Row2["rut_prv"],10,'0',STR_PAD_LEFT)." - ".strtoupper($Row2["nombre_prv"])."</option>\n";
									}
									else
									{		
										echo "<option value='".$Row2["rut_prv"]."'>".str_pad($Row2["rut_prv"],10,'0',STR_PAD_LEFT)." - ".strtoupper($Row2["nombre_prv"])."</option>\n";
									}
							}
/*							else
							{
		  ...
		  ?>
                        <tr valign="middle" class="Detalle02">
                            <td height="18" colspan="4" align="center">
                                <?php
				if (($OptAcumulado == "N") || ($OptAcumulado==""))
				{
                    ...
                    <table width="750" border="0" align="center" cellpadding="2" cellspacing="1" bgcolor="#000000"
                        ...
                        <tr>
                            <td align="center" bgcolor="#FFFFFF">
                                <span class="Estilo1">
                                    <input type="button" name="btnWeb" value="Consulta Web" onClick="Proceso('W');"
                                        style="width:90px">
                                    <input type="button" name="btnExcel" value="Consulta Excel" onClick="Proceso('E');"
                                        style="width:90px">
