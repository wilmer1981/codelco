...
function RecuperarValoresCheckeadoGUIA()
{
	var Frm=document.FrmGuia;
	var Valores="";
	for (i=1;i<Frm.CheckGuia.length;i++)
	{
		if (Frm.CheckGuia[i].checked==true)
		{
			...
			Valores=Valores + Frm.TxtNumGuiANULA[i].value+"//";
		}
	}
	return(Valores);
}	
...
function Eliminar()
{
	...
	for (i=1;i<Frm.CheckGuia.length;i++)
	{
		if (Frm.CheckGuia[i].checked==true)
		{
			...
			{
			    Valores1=Valores1 + Frm.TxtNumGuiaO[i].value+",";
...
function HistorialGDE(NG)
{
	var Frm=document.FrmGuia;
	var Valores="";
	window.open("pac_guia_despacho03.php?Valores="+NG,"","top=110,left=30,width=685,height=475,scrollbars=no,resizable = yes");
}
...
function GenerarPDFAnt(NG)
{
	window.open("pac_generacion_guia_despacho.php?Valores="+NG+"//&Proceso=I","","top=0px,left=5px,width=770px,height=550px,scrollbars=yes,resizable = yes");		
}
function Generar(Opcion)
{
	...
	if (SeleccionoCheck()) 
	{
		switch (Opcion)
		{
			...
			case "I":
				Valores=RecuperarValoresCheckeado();
				window.open("pac_impresion_guia_despacho.php?Valores="+Valores +"&Proceso=I","","top=0px,left=5px,width=770px,height=550px,scrollbars=yes,resizable = yes");									
...
<form name="FrmGuia" method="post" action="" >
 ...
 <?php
	...
	/*echo "<select name='CmbDia' size='1' style='width:40px;' onchange='Recarga()'>";
	for ($i=1;$i<=31;$i++)
	{
		if ($Proceso=='M')
		{
	...
	echo "<td width='60' align='left'>Corr.PAC</td>";
	echo "<td width='60' align='left'>N&deg; Gu&iacute;a</td>";
	echo "<td width='58' align='left'>Patente</td>";
	echo "<td width='170' align='left'>Transporte</td>";
	echo "<td width='200' align='left'>Cliente</td>";
	echo "<td width='100' align='left'>Producto</td>";
	echo "<td width='70' align='left'>Estanque</td>";
	echo "<td width='67' align='center'>Cant</td>";
	echo "<td width='67' align='left'>Tipo</td>";
	echo "<td width='65' align='left'>Estado</td>";
	echo "<td width='15' align='center'>GDE</td>";
	echo "</tr>";
	echo "</table>";
	?>
	...
	$Consulta.="left join pac_web.clientes t3 on t1.rut_cliente = t3.rut_cliente and t1.corr_interno_cliente=t3.corr_interno_cliente ";
	$Consulta.="left join pac_web.pac_logs t6 on t1.num_guia = t6.nro_guia ";
	$Consulta.="left join pac_web.pac_productos t9 on t1.cod_producto = t9.cod_producto ";
	...
	/*$Consulta.="ORDER BY CEILING(t1.num_guia) DESC";*/
	/*exit();*/
	//echo $Consulta;
	if ($CmbTipoGuia == 'V')
	{
		$Consulta.= " where (t1.fecha_hora between '".$FechaInicio."' and '".$FechaTermino."') and estado = 'N' group by t1.correlativo order by t1.fecha_hora desc";
	}
	else
	{
		if ($CmbTipoGuia == 'G')
		{
			$Consulta.= " where (t1.fecha_hora between '".$FechaInicio."' and '".$FechaTermino."') and (estado = 'I' or estado ='S') group by t1.correlativo order by t1.fecha_hora desc";
		}
		else
		{
			$Consulta.= " where (t1.fecha_hora between '".$FechaInicio."' and '".$FechaTermino."') and (estado = 'S' or estado ='I') group by t1.correlativo order by t1.fecha_hora desc";
		}
	}
	$Resultado2=mysqli_query($link, $Consulta);
	//echo $Consulta."<br>";
	echo "<input type='hidden' name='CheckGuia'><input type='hidden' name='TxtNumGuiaO'><input type='hidden' name='TxtNumGuiANULA'>";
	while ($Fila=mysqli_fetch_array($Resultado2))
	{
?>
		<tr> 
       <?php if ($CmbTipoGuia !='V') 	
		{
			echo "<td width='15' align='left'><input type='checkbox' name='CheckGuia' value='checkbox'></td>";
		}
		?>
         <td width='60'><?php echo $Fila["correlativo"]; ?></td>
		...
		<td width='58'>
			...
			{		
				...
				<a href="JavaScript:HistorialGDE('<?php echo $Fila["num_guia"];?>')">
					<?php echo $Fila["num_guia"];?>
				</a>
				<?php
			}
			else{
				?>
				<a href="JavaScript:Historial('<?php echo $Fila["num_guia"];?>')"><?php echo $Fila["num_guia"];?></a>
				<?php
		}
		?>
		</td>
		<?php
		echo "<td width='60' align='left'>".$Fila["nro_patente"]."&nbsp;<input type='hidden' name ='TxtNumGuiaO' value ='".$Fila["correlativo"]."'><input type='hidden' name ='TxtNumGuiANULA' value ='".$Fila["num_guia"]."'></td>";
		echo "<td width='143' align='left'>".$Fila["transportista"]."</td>";
		echo "<td width='174' align='left'>".$Fila["cliente"]."</td>";
		...
		if ($Fila["url_gde_local"]) {
			echo "<td align='center'><a href='".$Fila["url_gde_local"]."' style='text-decoration:none;' target='_blank'>
			<img   border='0' src='img/pdf.png' alt='Visualizar GDE'></a></td>";
		} else{
			?><td align='center'><a href="JavaScript:GenerarPDFAnt('<?php echo $Fila["num_guia"];?>')" style='text-decoration:none;'><img src='img/pdfBw.png'></a></td><?php 
		}
      ...
      <tr> 
		...
		if (($CmbTipoGuia!='V'))
	    {
			...
			echo " &nbsp; <input type='button' name='BtnModificar' value='Modificar' style='width:75' onClick=\"MostrarPopupProceso('M');\"> ";
			 ...
			 echo "&nbsp; <input type='button' name='BtnEliminar' value='Eliminar' style='width:75' onClick=\"MostrarPopupProceso('E');\"> ";
