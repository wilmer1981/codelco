...
<?php
    ...
    if($opcion == Eliminar)
    {
       $sql="delete from moneda where codigo_moneda = '$Codigo'";
       $result=mysql_query($sql);
       echo '<center>'.'<h1>'.'¡ELIMINACION EXITOSA!'.'</h1>'.'</center>';
    }
    else
    {
       echo '<form method="POST" action="mod2_moneda.php4">';

       $sql="select * from moneda where codigo_moneda like '$Codigo' order by codigo_moneda";
       ...
       if($row = mysql_fetch_array($result1))
       {
          ...
          {
             echo '<option>'.$row1['codigo_moneda'].'</option>\n';
          }
          echo '</select>';
          echo '<center>';
          ...
          echo '<TD align = center>'.$row['descripcion_moneda'];
