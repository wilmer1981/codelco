...
if($opcion == eliminar)
{
    ...
    $sql="delete from tipo_contrato where tipo_contrato = '$parametro'";
    $result=mysql_query($sql);
}else
if($opcion == modificar)
{
    echo '<form method="POST" action="tipo_contrato_modificado.php4">';
    echo '<center>'.'<H1>'.'MODIFICAR TIPO CONTRATO'.'</H1>'.'</center>';
    $sql="SELECT * FROM tipo_contrato where tipo_contrato = '$parametro'";
    $result=mysql_query($sql);
    $row=mysql_fetch_array($result);
    echo "Registro a Modificar:";
    echo '<select name ="parametro">';
    echo '<option>'.$row['tipo_contrato'].'</option>'.'\n';
    echo '<center>';
    ...
    echo '<tr>';
    echo '<TD align = center>'.'Descripcion Tipo Contrato';
