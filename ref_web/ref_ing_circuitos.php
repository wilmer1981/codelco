...
function Buscarant()
{
	...
	f.action='ref_ing_circuitos.php?mostrar=S&anterior=S';
	f.submit();
	//alert(f.dia1.value);
	//alert(f.mes1.value);
	//alert(f.ano1.value);	
...
function Buscarsig()
{
	...
	f.action='ref_ing_circuitos.php?mostrar=S&siguiente=S';
	f.submit();
	//alert(f.dia1.value);
	//alert(f.mes1.value);
	//alert(f.ano1.value);	
...
function Tabla4()
{
	...
	document.location = "../ref_web/tabla4.php?fecha="+fecha+"&ano="+ano1+"&mes="+mes1+"&dia="+dia1;
}
function Tabla5()
{
	var  f=document.form1;
	...
	var mes1=f.mes1.value;
	var dia1=f.dia1.value;
	document.location = "../ref_web/tabla5.php?fecha="+fecha+"&ano="+ano1+"&mes="+mes1+"&dia="+dia1;
}
...
<?php include("../principal/encabezado.php");?>
 ...
 <td width="760" align="center" valign="middle">
          ...
          <tr>
            <td width="168"><input name="buscar2" type="button" value="&lt;&lt; Anterior" onClick="Buscarant('<? echo $fecha;?>')" > 
            </td>
            ...
            <td width="128"> 
              <input name="buscar22" type="button" value="Siguiente &gt;&gt;" onClick="Buscarsig('<? echo $fecha;?>')" ></td>
          </tr>
        </table>
				<?php 
		//$fecha=$ano1."-".$mes1."-".$dia1;
       ...
       <TD>
          <TBODY>
            <TR> 
              ...
              <TD width="113"  class=tabsonline align=middle><B class=tabstext>Informe Completo</TD>
              <TD width="22"   class=tabsoff     align=middle><IMG height=40 alt="" src="archivos/tabMidOn.gif" width=22 border=0></TD>
			  <?php echo '<TD width="113"  class=tabsoffline align=middle><A class=tabstext href="ref_graficos_inf_diario.php?ano1='.$ano1.'&mes1='.$mes1.'&dia1='.$dia1.'&fecha='.$fecha.'"><B>Graficos</B></A></TD>'; ?>	
               ...
               <?php echo '<TD width="600%" class=tabsoffline align=center><B><A style="COLOR: #ffffff" href="ref_ing_circuitos.php?fecha='.$fecha.'&ano1='.$ano1.'&mes1='.$mes1.'&dia1='.$dia1.'&mostrar='.$mostrar.'" target=_top><SPAN style="COLOR: #ffffcc"><font size="4">
                  </font></SPAN></A></B></TD>'; ?>
            </TR>
          </TBODY>
  </TBODY>
<!-- COMIENZO PRIMERA TABLA -->
		 <table width="750" height="175" border="1"  align="center" cellpadding="2" cellspacing="0">
          ...
          <tr> 
            ...
            <td>&nbsp;</td>
			<td>&nbsp;</td>
			<td width="47" align="center"><strong>N&deg;</strong></td>
            <td width="46" align="center"><strong>%</strong></td>
            <td width="45" align="center"><strong>N&deg;</strong></td>
	...
	while ($Fila = mysqli_fetch_array($Respuesta))
	  {
			...
			$Consulta_turno="select turno as turno1 from cal_web.rechazo_catodos as t1 where t1.fecha = '".$fecha."' and t1.grupo = '".$cod_grupo."'";
			...
			echo "<td align='center' ><font color='blue'><a href=\"JavaScript:detalle('".$fecha."','".$cod_grupo."','".$turno1."')\">\n";
...
<!-- FINAL tercera TABLA -->  
  ...
  <?php 
		...
		for ($i = 0;$i<8;$i++)
		{
			...
			$calculo   = isset($Fila2["calculo"])?$Fila2["calculo"]:0;
			$total_total_ley=0;
			...
			//while ($Fila2 = mysqli_fetch_array($Respuesta2))
				...
				{						
					if ($calculo >= $limites[$l])
					{
						echo "<td align='center'><font color='red'><strong> ".number_format($calculo,2,",","")."&nbsp</strong></fornt></td>\n";
					}	 
					else
					{				
						echo "<td align='center'>".number_format($calculo,2,",","")."&nbsp</td>\n";						
					}
				$l=$l+1;	
				}	 	
			}
...
/****************************************************************************************************************************************/   						 
						...
						foreach($HM as $a=>$b) 
						 	{ 
							   //$Consulta_fecha="select left(fecha_hora,10) as fecha2 from cal_web.solicitud_analisis ";
                               //$Consulta_fecha=$Consulta_fecha." where left(fecha_muestra,10)='".$fecha."' and id_muestra='$b' and cod_producto='41' "; 		
							   //$Respuesta_fecha = mysqli_query($Consulta_fecha);
							   //$Fila_fecha = mysqli_fetch_array($Respuesta_fecha);
