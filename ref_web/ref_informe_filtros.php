...
function Buscar()
{
	...
	var cmbfiltro=f.cmbfiltro.value;
	//var checkbox=0;
	document.location = "../ref_web/ref_informe_filtros.php?opcion=H&fecha="+fecha+"&ano1="+ano1+"&mes1="+mes1+"&dia1="+dia1+"&cmbfiltro="+cmbfiltro+"&checkbox=0";
}
...
function Historial(f)
{
  if (f.checkbox.checked)
     {
      f.action="ref_informe_filtros.php?opcion=H&checkbox=1&ver=S";
	  f.submit();
	 }
  else {
        f.action="ref_informe_filtros.php?opcion=H&checkbox=0";
	    f.submit();
	   }	  
}
  ...
  <TR> 
        ...
        <TBODY>
          <TR> 
            ...
            <TD width="175"  class=tabsonline align=middle><B class=tabstext>Informe EStado de Filtros</B></TD>
    ...
    <tr> 
          <td width="175" height="32" align="center" valign="middle"><strong>Filtro:</strong> 
            <select name="cmbfiltro" id="cmbfiltro">
              ...
              <?php
			 $Consulta = "SELECT * FROM ref_web.filtros ORDER BY cod_filtro";
			 //echo $Consulta;
			 $Respuesta = mysqli_query($link, $Consulta);
			 while ($Row = mysqli_fetch_array($Respuesta))
				{
				  $cod_filtro=$Row[cod_filtro];
                  if($cmbfiltro==$cod_filtro)
                     {
					   echo "<option value=".$Row[cod_filtro]." selected>".$Row[filtro]."</option>\n";
           		     }
				  else
					  {
			  ...
			  if ($opcion=='H')
			     {
				  if ($checkbox=='0')
				     {
				      if ($cmbfiltro<>'-1')
				        {
			             $consulta="select * from ref_web.historia_filtros where fecha='".$ano1.'-'.$mes1.'-'.$dia1."' and cod_filtro='".$cmbfiltro."' order by fecha asc";
					    }
				       else { $consulta="select * from ref_web.historia_filtros where fecha='".$ano1.'-'.$mes1.'-'.$dia1."' order by fecha asc";}	  
					  }
				   else {
				         if ($cmbfiltro<>'-1')
						    { $consulta="select * from ref_web.historia_filtros where cod_filtro='".$cmbfiltro."' order by fecha asc";}
						 else {$consulta="select * from ref_web.historia_filtros order by fecha asc";}
						} 	
				   //echo $consulta;   
			      ...
			      while($row1 = mysqli_fetch_array($resultado))
				    {    
								...
								echo'<TD width="118" ><div align="center"><B>'.$row_equipo[filtro].'</B></div></TD>';
