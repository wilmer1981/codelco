...
function Buscar()
{
	...
	document.location = "../ref_web/Renovacion_grupos_resp.php?opcion=H&fecha="+fecha+"&ano1="+ano1+"&mes1="+mes1;
}
	...
	switch (opc)
	{
		case "M":
			...
			{				
				...
				window.open("ref_ing_ren_prog_prod_res.php?Proceso=M&Dia=" + valores + "&Mes=" + f.mes1.value + "&Ano=" + f.ano1.value + "","","top=70,left=100,width=400,height=400,scrollbars=yes,resizable = yes");
				break;
			}
			break;
		case "A":
			f.action = "Renovacion_grupos_resp.php?opcion=H";
			f.submit();
			break;
	    case "E":
			f.action = "Renovacion_grupos_xls_resp.php?opcion=H";
			f.submit();
			break;		
	}
}
	...
	if ($opcion=="H")
	  {
            ...
            if ($datos == "F")
            {
				...
				echo "document.frmPrincipal.action = 'Renovacion_grupos_resp.php?opcion=H';";
