...
$CmbProveedor = $_REQUEST["CmbProveedor"];
	...
	// -- 15-01-2015 inicio mf intellego --
	if ($MesAnt==0)
	{
		$MesAnt = -1;
	}
	// -- 15-01-2015 fin mf intellego --
			...
			//if (!is_null($v["usada"]) && $usada=="S")
			{
				...
				else
				{
					...
					if (isset($ArrTotal[$v["cod_leyes"]]["valor"])){
						$ArrTotal1 = $ArrTotal[$v["cod_leyes"]]["valor"];
					}else{
						$ArrTotal1 = 0;
					}
					//$ArrTotal[$v["cod_leyes"]]["valor"] = $ArrTotal[$v["cod_leyes"]]["valor"] + (($v["valor"]*$Fila["peso_humedo"])/$v["conversion"]);
					$ArrTotal[$v["cod_leyes"]]["valor"] = $ArrTotal1 + (($v["valor"]*$Fila["peso_humedo"])/$v["conversion"]);
