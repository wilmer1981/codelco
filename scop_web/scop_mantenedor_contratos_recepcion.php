...
function RecargaREC(Opc,check)
{
	var f= document.FrmPopupProceso;
	var R='';
	f.action = "scop_mantenedor_contratos_proceso.php?Opc="+Opc+"&TabRec=true&CheckREC="+check+"&R=S";
	f.submit();
}
</script>
<table width="100%"  border="0" align="center" cellpadding="0"  cellspacing="0">	
  <tr class="formulario2">
	 <td width="70%">
	   Tipo de Flujos
	     <input name="radiorecepcion" id="cbbCustomer" onclick="RecargaREC('<? echo $Opc;?>','1')" type="radio" class="SinBorde" value="1" <? if($CheckREC=='1') echo " checked ";?>>
		Enabal
		<input name="radiorecepcion" id="cbbCustomer" onclick="RecargaREC('<? echo $Opc;?>','2')" type="radio" class="SinBorde" value="2" <? if($CheckREC=='2') echo " checked ";?>>
		Pmn
		<input name="radiorecepcion" id="cbbCustomer" onclick="RecargaREC('<? echo $Opc;?>','3')" type="radio" class="SinBorde" value="3" <? if($CheckREC=='3') echo " checked ";?>>
	   Otros<br>	   
		<?
		  if($CheckREC=='1')
		  	$TipoFlujo='ENA';
		 ...
		 {
		?>
		 <select name="CmbFlujoRecep" style="width:300">
		...
		<?
			...
			while($Fila=mysql_fetch_assoc($result))
			{
				if ($CmbFlujoRecep==$Fila["cod_flujo"])
					echo "<option selected value='".$Fila["cod_flujo"]."'>".str_pad($Fila["cod_flujo"],2,'0',STR_PAD_LEFT)." - ".ucfirst(strtolower($Fila["nom_flujo"]))."</option>\n";
				else
		...
		<?
		 ...
		 {
			 echo "<select name='CmbFlujoRecep' style='width:300'>";
			   echo "<option value='T' class='Selected'>Seleccionar</option>";
		 	echo "</select>";
		 }	
		?>	
		 &nbsp;&nbsp;&nbsp;
		 <select name="CmbSignoRec" style="width:60">
		...
		&nbsp;<a href="JavaScript:ProcesoNuevo('NRECEP','<? echo $CheckREC;?>','<? echo $R;?>')"><img src="archivos/agregar.png" alt="Agregar" width="25" height="25"  border="0" align="absmiddle" /></a>
    </td>
			   ...
			   <tr class="TitulotablaVerde">
				...
				<?
				  ...
				  while ($Fila=mysql_fetch_array($Resp))
				  {
						...
						?>
						<tr>
							<td align='center' width='1%'><a href="JavaScript:ProcesoEliminar('ERECEP','<? echo $Cod;?>')"><img src='archivos/eliminar2.png'  alt='Eliminar' align='absmiddle' border='0' width='15' height='15'></a></td>
