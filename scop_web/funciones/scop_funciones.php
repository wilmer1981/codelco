...
{	
	...
	{	//ESTADO 2 pantalla valida inventario
		...
		$Mensaje.='<br><br>';
		$Mensaje.='<a href="http://'.HTTP_SERVER.'/proyecto/scop_web/scop_proceso_cobertura_carry_cost_proceso.php?Opc=N"><font size="2">Revisi�n de Inventario Validado</font></a>';
	}
	...
	{	//ESTADO 5.
		$Asunto='Validaci�n Resultado de Cobertura por la VCO '.$MesCorreo.'&nbsp;'.$Ano.'';
		$Titulo='Validaci�n Resultado de Cobertura por la VCO '.$MesCorreo.'&nbsp;'.$Ano.'';	
		...
		$Mensaje.='<br><br>';
		$Mensaje.='<a href="http://'.HTTP_SERVER.'/proyecto/scop_web/scop_proceso_cobertura_imputacion_cc.php?&Buscar=S&Ano='.$Ano.'&TipoEst='.$TipEst.'&CmbAcuerdo='.$Acuerdo.'"><font size="2">Link imputacion de valores</font></a>';
	}
	...
	{		
		...
		for($i=0;$i<=$x;$i++)
		{
			...
			if('Cu'==$arreglo[$i][0])
			{
				...
				if($CarryQP2[0]!='-1'&&$Acuerdo!='P')
					$MesQP=$MesQP.' Cu&nbsp;Mes&nbsp;+'.$CarryQP2[0].",";	
				if($Acuerdo=='P')
					$MesQP=$MesQP.' Cu&nbsp;'.$CarryQP2[0].",";	
			}
			if('Ag'==$arreglo[$i][0])
			{
				if($CarryQP2[1]=='-1')
					$MesQP=$MesQP.' Ag&nbsp;Mes&nbsp;'.$CarryQP2[1].",";
				if($CarryQP2[1]!='-1'&&$Acuerdo!='P')
					$MesQP=$MesQP.' Ag&nbsp;Mes&nbsp;+'.$CarryQP2[1].",";	
				if($Acuerdo=='P')
		...
		$Mensaje.='<br>';
		$Mensaje.='Con acuerdo contractual&nbsp;<b>'.$MesAcu.'</b>'.$Men;
		$Mensaje.='<br>';
