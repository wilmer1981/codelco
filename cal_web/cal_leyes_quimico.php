...
function Validar(valores,Personalizar,Producto,SubProducto,NombrePlantilla,CodPlantilla)
{
	...
	frm.action="cal_leyes_quimico01.php?ValoresLeyes=" + ValoresLeyes + "&ValoresImpurezas=" + ValoresImpurezas+ "&Personalizar=" +Personalizar+"&Producto="+Producto+"&SubProducto="+SubProducto+"&NombrePlantilla="+NombrePlantilla+"&CodPlantilla="+CodPlantilla;
	frm.submit();				
}	
	...
	<table width="600" border="0" cellpadding="0" class="ColorTabla01">
  	<tr>
   	...
   	<?php
	  			...
	  			echo"<tr>";
				$cont=1;	 
				$Consulta  = "select t1.cod_leyes,t1.tipo_leyes,t1.abreviatura as abrev,t1.cod_unidad,t2.abreviatura as abrev2 from leyes t1 inner join unidades t2 on t1.cod_unidad = t2.cod_unidad where t1.tipo_leyes = 1 or t1.tipo_leyes = 3 order by  t1.abreviatura";
				//echo $Consulta."<br>";
				$Resultado = mysqli_query($link, $Consulta);
				while ($Fila =mysqli_fetch_array($Resultado))
				{
