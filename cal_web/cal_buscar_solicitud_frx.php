...
function Recarga()
{
 var Frm=document.FrmBuscarSolicitud;
     Frm.action= "cal_buscar_solicitud_frx.php";
	 Frm.submit();
}
...
function ValidaIngreso(Sol_Aut)
{
	...
	var Frm=document.FrmBuscarSolicitud;
     Frm.action= "cal_buscar_solicitud_frx.php?Buscar=S&Sol_Aut="+Sol_Aut;
	 Frm.submit();
...
function RecuperarSeleccion(ValorProducto,ValorSubProducto,Sol_Aut)
{
	...
	{
		window.opener.document.FrmSolicitud.action="cal_solicitud_frx.php?FechaBusqueda=" + Fecha + "&Productos=" + ValorProducto + "&SubProducto=" + ValorSubProducto +"&Modificar=S";
...
?>
		...
		if (isset($Buscar))
		{
			...
			$Consulta = $Consulta." where (t1.rut_funcionario = '".$CookieRut."') and ((estado_actual is null and frx <> 'N') or (estado_actual = '31')) and ";
			if ($Sol_Aut=='S')
			{
				$Tipo ="A";
			}
