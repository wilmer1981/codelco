...
function Grabar(Solicitudes)
{
	...
	Frm.action="cal_ingreso_valor_humedad_frx01.php?ValoresSA="+ValoresSA+"&Valores="+Valores+"&Tipo=H&Opcion=G&CheckT=N";
	Frm.submit();
}
...
function Seguridad(CB,ValoresSA,Tipo)
{
	...
	{
		window.open("cal_desbloquear_leyes_frx.php?ValoresSA="+ValoresSA+"&Valores="+ValoresAux+"&Tipo=H","","top=200,left=175,width=409,height=210,scrollbars=no,resizable = no");	
	}
	else
	{
		...
		Frm.action="cal_ingreso_valor_humedad_frx01.php?Opcion=G&ValoresSA="+ValoresSA+"&Valores="+Valores+"&Tipo=H"+"&ValoresAux="+ValoresAux+"&PonerCandado=S";
		Frm.submit();	
	}
}
...
function Todos(Solicitudes)
{
	...
	Frm.action="cal_ingreso_valor_humedad_frx01.php?ValoresSA="+ValoresSA+"&Valores="+Valores+"&Tipo=H&Opcion=G&PonerCandado=S&CheckT=S";
	Frm.submit();
}
...
{
	var Frm=document.FrmIngresoValorHum;
	Frm.action="cal_ingreso_valor_Humedad_frx01.php?Opcion=S&Valores="+Valores;
	Frm.submit();
}
      ...
      <td width="720"><div align="center"></div>
        ...
        <table width="710" border="0" cellpadding="5" class="TablaInterior">
          <tr> 
            ...
            <td width="138"><input name="BtnGrabar2" type="button" id="BtnGrabar23" value="Grabar" style="width:60" onClick="Grabar('<?php echo $Solicitudes;?>');"> 
              <input name="BtnSalir2" type="button" id="BtnSalir23" value="Salir" style="width:60" onClick="JavaScript:window.close();"></td>
          </tr>
        </table>
        ...
        <tr class="ColorTabla01" align="center">
		  ...
		  <td width="60" height="20">Recargo</td>
		...
		<?php
        	$Consulta ="select t1.cod_leyes,t1.nro_solicitud,t1.rut_funcionario,t1.recargo,t2.abreviatura,t1.valor,t1.cod_unidad,t1.candado,t1.peso_humedo,t1.peso_seco from cal_web.leyes_por_solicitud t1 inner join proyecto_modernizacion.leyes t2 on t1.cod_leyes = t2.cod_leyes  and t2.cod_leyes = '01' where ".$Criterio." order by t1.nro_solicitud,t1.cod_leyes";
			$Respuesta=mysqli_query($link, $Consulta);
			$Cont=1;
			...
			while ($Fila=mysqli_fetch_array($Respuesta))
			{
				...
				{
					echo "<td height='28'><input type='text' name='TxtRecargo' style='width:100' disabled value = '".$Fila["recargo"]."'></td>";
				}
				else
				{
					echo "<td height='28'><input type='text' name='TxtRecargo' style='width:100' disabled value = 'Sin Recargo'></td>";
				}
