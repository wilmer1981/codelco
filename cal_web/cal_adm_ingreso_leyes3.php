...
function Mostrar(Pantalla)
{
	...
	if (CheckeoSolicitud())
	{
		switch (Pantalla)
		{
			case "V"://INGRESO DE LEYES VIRTUALES
				if(!EncontroSAconRecargo())			
				{
					ValoresSA=RecuperarSolicitudCheckeadas();
					window.open("cal_ingreso_valor_leyes_virtuales.php?ValoresSA="+ValoresSA,"","top=130,left=120,width=533,height=480,scrollbars=yes,resizable = no");
				}
				else
				{
...
function Detalle2(sa,rec)
{
	var frm =document.FrmIngLeyes;	
	var URL = "cal_con_registro_leyes_solo.php?SA="+ sa+"&Recargo="+rec;
	window.open(URL,'',"top=50,left=10,width=790,height=450,scrollbars=yes,resizable = yes");					
  ...
  <table width="770"  border="0" cellpadding="5" cellspacing="0" class="TablaPrincipal" left="5" >
    <tr>
      <td width="759"> <table width="756"border="0" cellpadding="3" cellspacing="0" class="TablaInterior">
          ...
          <tr> 
            ...
            <?php
				if ($Mostrar=="O")
				{
						...
						echo "<td width='115'><input name='BtnLeyesVirt' type='button' value='Leyes Virtuales' style='width:110' onClick=\"Mostrar('V');\" style='visibility:hidden'></td>";
        ...
        <br>
        <table width="758" border="1" cellpadding="3" cellspacing="0" bordercolor="#0033CC">
		...
		{
				$Consulta = "select t1.fecha_hora,t1.estado_actual,t2.descripcion as nomproducto,t3.abreviatura as nomsubproducto,t1.cod_producto,t1.cod_subproducto,t1.fecha_muestra, ";
				...
				if($Mostrar=='S')
				{
					$Estado="where (t6.fecha_hora between  '".$FechaI."' and '".$FechaT."') ";
					if($CmbEstado=='-1')
						$Estado.=" and (t1.estado_actual in ('4','5','6')) ";
					else
						$Estado.= "and (t1.estado_actual = '".$CmbEstado."')) ";
					if($CmbProductos!='-1')
					{	
						$Estado.="and  t1.cod_producto = '".$CmbProductos."' ";
						if($CmbSubProducto!='-1')
							$Estado.=" and t1.cod_subproducto = '".$CmbSubProducto."' ";
					}
				}
				else
				{
					...
					$Estado="where (t1.nro_solicitud between '".$SolIni."' and '".$SolFin."') and (t1.estado_actual in ('4','5','6'))";
				}
				$Estado.=$TipoAnalisis;
				...
				while ($Fila=mysqli_fetch_array($Respuesta))
				{
					...
					//ANALITOS
					$Analitos="";
					$Consulta ="select t0.agrupacion, t0.tipo, t2.abreviatura,t1.virtual as virt ";
					...
					echo "<tr>";
					//BLOQUEA LA LEY EN CASO DE QUE EL MES ESTE CERRADO Y EL PRODUCTO SEA MINERO
					$MensajeBloqueo='';
					$BloqueoLey=false;
					if($Fila["cod_producto"]=='1')
					{
						$Consulta="select cod_sistema from proyecto_modernizacion.productos_sistema where cod_producto='".$Fila["cod_producto"]."' and cod_subproducto='".$Fila["cod_subproducto"]."'";
						$RespSistema=mysqli_query($link, $Consulta);
						if ($FilaSistema=mysqli_fetch_array($RespSistema))
						{
							$Ano=substr($Fila[fecha_muestra],0,4);
							$Mes=substr($Fila[fecha_muestra],5,2);
							$Consulta = "select estado, fecha_cierre from proyecto_modernizacion.cierre_mes";
							$Consulta.= " where cod_sistema='".$FilaSistema[cod_sistema]."'";
							...
							$Resp = mysqli_query($link, $Consulta);
							//echo $Consulta."<br>";
							if ($FilaResp=mysqli_fetch_array($Resp))
							{
								if ($FilaResp[estado]=='C'and $LeyVirtual=='N')
								{
									$BloqueoLey=true;
									$MensajeBloqueo='Mes Cerrado Analisis No Modificable';
								}
								else
									$MensajeBloqueo='SA Con Leyes Virtuales';
							}	
						}
					}	
					...
					{ 
						if ($BloqueoLey==false)
							...
							//echo "<td bgcolor='red' width='25' align='rigth'><input type='button' name ='checkAtender' value='Det' onClick=\"JavaScript:Detalle2('".$Fila["nro_solicitud"]."','".$Fila["recargo"]."')\"></td>";
							echo "<td bgcolor='red' width='25' align='rigth'><input type='checkbox' name ='checkAtender' value='".$Cont."' disabled></td>"; 
					}
					else
					{
						if ($BloqueoLey==false)
							...
							//echo "<td bgcolor='red' width='25' align='rigth'><input type='button' name ='checkAtender' value='Det' onClick=\"JavaScript:Detalle2('".$Fila["nro_solicitud"]."','".$Fila["recargo"]."')\"></td>";
							echo "<td bgcolor='red' width='25' align='rigth'><input type='checkbox' name ='checkAtender' value='".$Cont."' checked disabled></td>"; 
					}	
					$Cont=$Cont+1;
					...
					{	
						...
						echo "<font face='courier' color='#000000' size=1><b>Analitos&nbsp;&nbsp;: </b>".$Analitos."</font><br>";
						echo "<font face='courier' color='red' size=1><b>".$MensajeBloqueo."</b></font>";
						echo "</div>\n";
						echo "<div align='center'><input name='TxtSA' type='hidden' style='background:#F4F5BA' readonly style='width:100' maxlength='15' value ='".$TxtSA = $Fila["nro_solicitud"]."'><a href=\"JavaScript:Detalle2('".$Fila["nro_solicitud"]."','".$Fila["recargo"]."')\">".substr($Fila["nro_solicitud"],4)."</a><input type = 'hidden' name = 'TxtSAO' value ='".$Fila["nro_solicitud"]."'><input type = 'hidden' name ='TxtRutO' value ='".$Fila["rut_funcionario"]."'><input type = 'hidden' name = 'TxtRecargoO' value ='N'></div></td>";
					}
					...
					{	
						...
						else
						{
							...
							echo "<font face='courier' color='red' size=1><b>".$MensajeBloqueo."</b></font>";
							echo "</div>\n";
							echo "<div align='center'><input name='TxtSA' type='hidden' style='background:#F4F5BA' readonly style='width:100' maxlength='15' value ='".$TxtSA = $Fila["nro_solicitud"].'-'.$Fila["recargo"]."'><a href=\"JavaScript:Detalle2('".$Fila["nro_solicitud"]."','".$Fila["recargo"]."')\">".substr($Fila["nro_solicitud"],4).'-'.$Fila["recargo"]."</a><input type = 'hidden' name = 'TxtSAO' value ='".$Fila["nro_solicitud"]."'><input type = 'hidden' name ='TxtRutO' value ='".$Fila["rut_funcionario"]."'><input type = 'hidden' name = 'TxtRecargoO' value ='".$Fila["recargo"]."'></div></td>";														
						}
					}
...
if($HayVirtuales=='S')
{
	...
	echo "var frm=document.FrmIngLeyes;";
	echo "try";
	echo "{";
	echo "frm.BtnLeyesHum.disabled=true;";
	echo "frm.BtnLeyes.disabled=true;";
	echo "frm.BtnLeyesVirt.style.visibility='';";
	echo "frm.BtnRetalla.disabled=true;";
	echo "frm.BtnModLeyes.disabled=true;";
	echo "}";
	echo "catch(e)";
	echo "{";
	echo "}";
...
else
{
	...
	echo "try";
	echo "{";
	echo "frm.BtnLeyesHum.disabled=false;";
	echo "frm.BtnLeyes.disabled=false;";
	echo "frm.BtnRetalla.disabled=false;";
	echo "frm.BtnModLeyes.disabled=false;";
	echo "}";
	echo "catch(e)";
	echo "{";
	echo "}";
