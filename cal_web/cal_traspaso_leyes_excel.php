		...
		while($Row = mysqli_fetch_array($Respuesta))
		{
			...
			echo "<tr bgcolor=\"".$BgCgolor."\">";	?>	
			 <td> <?php
			  ...
			  $rec='';
			  $ConsultaRec="Select * from cal_web.solicitud_analisis where nro_solicitud='".$Row["nro_solicitud"]."' and  recargo  in('','0')";
			$RespREC = mysqli_query($link, $ConsultaRec);
			if($RowREC = mysqli_fetch_array($RespREC))
			{
				$rec=$RowREC["recargo"];
			}
			if($Bloquear!=true)
			...
			?><input name="CheckSol" type="checkbox"  value="<?php echo $Row["nro_solicitud"];?>">
            ...
            <td ><?php echo $Row["ley_ag"];?></td>
			<td><?php
					if($Row["ley_ag"]!='')
						echo ObtenerDescripcionUnidad($Row["unidad_ag"],$link);
			
			?></td>
			<td><?php echo $Row["ley_au"];?></td>
			<td><?php 
					if($Row["ley_au"]!='')
						echo ObtenerDescripcionUnidad($Row["unidad_au"],$link);
						
			?></td>
			<td><?php echo $VerDetRet;?></td>
            <td><?php echo $Row["r_ley_ag"];?></td>
			<td><?php 
					if($Row["r_ley_ag"]!='')
						echo ObtenerDescripcionUnidad($Row["r_unidad_ag"],$link);
			
			?></td>
			<td><?php 	echo $Row["r_ley_au"];?></td>
			<td><?php
				if($Row["r_ley_au"]!='')
				 echo ObtenerDescripcionUnidad($Row["r_unidad_au"],$link);
				 
			?></td>
			<td><?php echo $Row["r_peso"];?></td>
			<td><?php echo $Row["r_porce_cu"];?></td>
			</tr>	
					<?php	
