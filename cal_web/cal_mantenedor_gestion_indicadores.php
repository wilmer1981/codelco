...
<html>
<head>
<title>Administración Gestión de Indicadores</title>
...
function Proceso(Opcion)
{
	...
	{
		case "B": 
			frm.action ="cal_mantenedor_gestion_indicadores.php?Buscar=S";  
			frm.submit();
		break;	
		case "R"://recarga pagina
			frm.action ="cal_mantenedor_gestion_indicadores.php";  
			frm.submit();
		break;
		case "N"://nuevo
			URL="cal_mantenedor_gestion_indicadores_proceso.php?Opc=N";
		...
		case "M"://modificar
			if(SoloUnElemento(frm.name,'CheckTipo','M'))
			{
				...
				URL="cal_mantenedor_gestion_indicadores_proceso.php?Opc=M&Valores="+Datos;
				opciones='left=50,top=30,toolbar=0,resizable=1,menubar=0,status=1,width=600,height=300,scrollbars=1';
				popup=window.open(URL,"",opciones);
				popup.focus();
			}
		...
		case "E":
			if(SoloUnElemento(frm.name,'CheckTipo','E'))
			{
				...
				{
					Datos=Recuperar(frm.name,'CheckTipo');
					frm.action='cal_mantenedor_gestion_indicadores_proceso01.php?Proceso=E&Valor='+Datos;
					frm.submit();
				}
			}	
		break; 
		case "EXC":
				...
				URL='cal_mantenedor_gestion_indicadores_excel.php?CmbProductos='+frm.CmbProductos.value+'&CmbSubProducto='+frm.CmbSubProducto.value+'&CmbLeyes='+frm.CmbLeyes.value;
  ...
  <table width="770" height="330" border="0" cellpadding="5" cellspacing="0" class="TablaPrincipal" left="5" >
    <tr>
      <td width="756" align="center" valign="top"><table width="761" border="0" align="center" cellpadding="2" cellspacing="1" bgcolor="#333333" class="TablaInterior">
        ...
        <br>        
		<table  border="0" cellpadding="2" cellspacing="1" bgcolor="#333333" class="TablaDetalle" >
          <tr align="center" class="ColorTabla01">
            <td >Todos<br><input name="ChkTodos" type="checkbox" onClick="CheckearTodo(this.form,'CheckTipo','ChkTodos');"></td>
            <td>Producto</td>
            <td>Subproducto</td>
            <td>Ley</td>
            <td>Unidad</td>
			<td>Uspec</td>
              ...
              <?php		
				if($Buscar=='S')
				{
					$Consulta="select t6.abreviatura as AbreUNI,t1.valor,t1.cod_producto,t1.cod_subproducto,t1.cod_leyes,t2.descripcion as nom_producto,t3.descripcion as nom_subproducto,t5.nombre_leyes,t5.abreviatura,t1.cod_unidad from cal_web.gestion_indicadores t1 inner join proyecto_modernizacion.productos t2 on t1.cod_producto=t2.cod_producto";
					...
					$Consulta.=" inner join proyecto_modernizacion.leyes t5 on t1.cod_leyes=t5.cod_leyes ";
					$Consulta.=" inner join proyecto_modernizacion.unidades t6 on t1.cod_unidad=t6.cod_unidad ";
					...
					$Consulta.=" group by t1.cod_producto,t1.cod_subproducto,t1.cod_leyes,t1.cod_unidad ";	
					echo "<input name='CheckTipo' type='hidden'  value=''>";
					$Respuesta = mysqli_query($link, $Consulta);
					while ($Fila=mysqli_fetch_array($Respuesta))
					{
						$Clave=$Fila["cod_producto"]."~".$Fila["cod_subproducto"]."~".$Fila["cod_leyes"]."~".$Fila["cod_unidad"];
						...
						?><td align='left'><?php echo $Fila["AbreUNI"];?></td><?php
							?><td align='left'><?php echo $Fila["valor"];?></td><?php
							?></tr><?php
					}					
				}	
			?>
