	...
	foreach ($DatosRecorrer as $valueDataEnviada) {
		...
		if (trim($valueDataEnviada["nro_solicitud"]) == '' || trim($valueDataEnviada["nro_solicitud"]) == '?')
		{
		    ...
		    if($row = mysqli_fetch_assoc($result)) 
		    {
				...
				,nro_sa_lims = '".$valueDataEnviada["nro_sa_lims"]."'
...
function Eliminacion_RegistrarLeyesSA($NumeroSA,$DatosLeyesSA)
{
	...
	$SQL = "SELECT nro_solicitud,corr_impresion from cal_web.solicitud_analisis where nro_solicitud = '".$NumeroSA."'";
	$result = $DataBase->consulta($SQL);
    if($row = mysqli_fetch_assoc($result)) 
    {
		...
		//if($DataBase->QueryAction($Query)=='')
		//{
			if($row["corr_impresion"] == '')
				$Cont=1;
			else
				$Cont=$row["corr_impresion"]+1;
			foreach ($DatosRecorrer as $valueDataEnviada) {
				...
				if(trim($valueDataEnviada["nro_solicitud"]) != '' || trim($valueDataEnviada["nro_solicitud"]) == '?')
				{
					...
					{
						Registroleyes($valueDataEnviada,$Cont);
					}
				}
				else
				{
...
function Registroleyes($valueDataEnviada,$Cont)
{
	global $DataBase;
	...
	$Query = "UPDATE cal_web.solicitud_analisis set corr_impresion	= '".$Cont."' where nro_solicitud	= '".$valueDataEnviada["nro_solicitud"]."' and recargo = '".$valueDataEnviada["recargo"]."'";
	$DataBase->QueryAction($Query);	
}
?>