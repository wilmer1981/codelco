...
function Proceso(Opcion)
{
	...
	{
		case "R":
			frm.action="cal_adm_ingreso_leyes_frx.php";
			frm.submit();
			break;
		case "B": 
		...
		case "S":
			...
			break;	
		case "O":
			frm.action="cal_adm_ingreso_leyes_frx.php?Mostrar=O";
			frm.submit();
			break;
	}	
}
...
function RecuperarSA()
{
	...
	{
		window.open("cal_ingreso_leyes_frx.php?ValoresSA="+ ValoresSA + "&Producto="+frm.CmbProductos.value + "&SubProducto="+frm.CmbSubProducto.value,"","top=200,left=35,width=640,height=300,scrollbars=no,resizable = yes");					
	}
}
function Salir()
{
...
function PorAtender()
{
	...
	frm.action="cal_adm_ingreso_leyes_frx01.php?Opcion=P&ValoresSA="+ValoresSA;
	frm.submit(); 
}
...
{
	var frm=document.FrmIngLeyes;
	frm.action ="cal_adm_ingreso_leyes_frx.php?Mostrar=S";  
	frm.submit();
	...
	if (CheckeoSolicitud())
	{
		switch (Pantalla)
		{
			case "L":
				...
				{
					ValoresSA=RecuperarSolicitudCheckeadas();
					window.open("cal_ingreso_valor_leyes_frx.php?ValoresSA="+ValoresSA,"","top=130,left=120,width=533,height=380,scrollbars=yes,resizable = no");
				}
				else
				{
			...
			case "I":		
				...
				{
					ValoresSA=RecuperarSolicitudCheckeadas();		
					window.open("cal_ingreso_valor_impurezas_frx.php?ValoresSA="+ValoresSA,"","top=130,left=120,width=533,height=380,scrollbars=yes,resizable = no");
				}	
				else
				{
					alert ("Valores de Leyes solo al Recargo Nro.0 ");
			...
			case "H":
				ValoresSA=RecuperarSolicitudCheckeadas();		
				window.open("cal_ingreso_valor_humedad_frx.php?ValoresSA="+ValoresSA,"","top=130,left=15,width=750,height=380,scrollbars=yes,resizable = no");
				break;						
			case "R":
			...
			case "A":
				...
				{
					ValoresSA=RecuperarSolicitudCheckeadas();
					window.open("cal_ingreso_valor_leyes_frx.php?ValoresSA="+ValoresSA,"","top=130,left=120,width=533,height=320,scrollbars=yes,resizable = no");
				}
				else
				{
					alert ("Valores de Leyes solo al Recargo Nro.0 ");
...
function Detalle()
{
	...
	for (i=1;i<frm.checkAtender.length;i++)
	{
		if (frm.checkAtender[i].checked==true)
		{
			...
			var URL = "cal_con_registro_leyes.php?SA="+ Checkeada;
      ...
      <td width="759"> <table width="756"border="0" cellpadding="3" cellspacing="0" class="TablaInterior">
          ...
          <tr> 
            ...
            <td height="29"><select name="CmbProductos" style="width:220" onChange="Proceso('R');">
                ...
                <?php
				$Consulta="select cod_producto,descripcion from productos where mostrar = 1  order by descripcion"; 
				$Respuesta = mysqli_query($link, $Consulta);
		...
		?>
          ...
          <tr> 
			...
			<option value="-1" selected>Todos</option>
			<?php
			$Consulta ="select * from proyecto_modernizacion.sub_clase where cod_clase = '1002' and (cod_subclase between '31' and '32')";
			$Respuesta=mysqli_query($link, $Consulta);
			while($Fila=mysqli_fetch_array($Respuesta))  
			{
        ...
        <table width="758" border="1" cellpadding="3" cellspacing="0" bordercolor="#b26c4a">
          <tr class="ColorTabla01"> 
           <td colspan="2"><div align="center">S.A</div></td>
			...
			<td width="108"><div align="center">SubProducto</div></td>
            <td width="90"><div align="center">Estado </div></td>
			<td width="78"><div align="center">F.Creacion</div></td>
            <td width="91"><div align="center">F.Quimico</div></td>
            <td width="58"><div align="center">Leyes</div></td>
          	<td width="78"><div align="center">Candado</div></td>
          </tr>
        <?php
		...
		if ($Mostrar=='S')
		{
			...
			if (($CmbProductos!=-1) && ($CmbSubProducto!= -1))
			{
				...
				{
					case "-1":
						$Estado ="where (t6.fecha_hora between  '".$FechaI."' and '".$FechaT."') and (t1.estado_actual = '31' or estado_actual = '32') and (t1.cod_producto = '".$CmbProductos."' and t1.cod_subproducto = '".$CmbSubProducto."')and (t1.cod_analisis='1')";
						break;
					case "31"://Atendidas por Lab Frx  
						$Estado = "where (t6.fecha_hora between  '".$FechaI."' and '".$FechaT."') and (t1.estado_actual = '31') and (t1.cod_producto = '".$CmbProductos."' and t1.cod_subproducto = '".$CmbSubProducto."')and (t1.cod_analisis='1')";
						break;
					case "32": //finalizada por Lab Frx  
						$Estado = "where (t6.fecha_hora between  '".$FechaI."' and '".$FechaT."') and (t1.estado_actual = '32') and (t1.cod_producto = '".$CmbProductos."' and t1.cod_subproducto = '".$CmbSubProducto."')and (t1.cod_analisis='1')";
						break;
					default:
						$Estado ="where (t6.fecha_hora between  '".$FechaI."' and '".$FechaT."') and (t1.estado_actual = '31' or t1.estado_actual = '32') and (t1.cod_producto = '".$CmbProductos."' and t1.cod_subproducto = '".$CmbSubProducto."')and (t1.cod_analisis='1')";
						break;
				}
				$Consulta = "select t1.fecha_hora,t1.estado_actual,t2.descripcion as nomproducto,t3.abreviatura as nomsubproducto,t1.id_muestra, ";
				$Consulta = $Consulta."t1.rut_funcionario,t1.recargo,t1.cod_analisis, ";
				...
				while ($Fila=mysqli_fetch_array($Respuesta))
				{
					$Consulta ="select fecha_hora from estados_por_solicitud  where (rut_funcionario = '".$Fila["rut_funcionario"]."') and (nro_solicitud = ".$Fila["nro_solicitud"].") and (cod_estado = '31')";
					...
					echo "<td width ='108'><div align ='left'><input name ='TxtProducto' type='text' readonly style='width:108' maxlength='250' value ='".$TxtProducto=ucwords(strtolower($Fila["nomsubproducto"]))."'><input  type = 'hidden' value ='".substr($Fila["fecha_hora"],0,10)."'><input type = 'hidden' value ='".substr($Fila["fecha_hora"],11,8)."'></div></td>";
					if ($Fila["cod_analisis"]==1)
					{
						echo "<td width ='90'><div align ='left'><input name ='TxtEstados' type='text' readonly style='width:90' maxlength='85'value ='".$TxtEstado= $Fila["nombre_subclase"]." Quimico'></div></td>";
					}
					else
					{
						echo "<td width ='90'><div align ='left'><input name ='TxtEstados' type='text' readonly style='width:90' maxlength='85'value ='".$TxtEstado= $Fila["nombre_subclase"]." Fisico'></div></td>";					
					}	
					...
					if (($Fila["cod_estado"]== '31') || ($Fila["cod_estado"]== '32') )
					{
					...
					{
						if ((!is_null($Fila4["valor"])) || ($Fila4["valor"] != ""))
						{
							$EncontroValor=true;
						}
						else
	  ...
	  if (($CmbProductos== -1) && ($CmbSubProducto == -1))
	   {
			switch ($CmbEstado) 
				{
					case "-1":
						$Estado ="where (t6.fecha_hora between  '".$FechaI."' and '".$FechaT."') and (t1.estado_actual = '31' or t1.estado_actual = '32')and (t1.cod_analisis='1')";
						break;
					case "31": 
						$Estado = "where (t6.fecha_hora between  '".$FechaI."' and '".$FechaT."') and (t1.estado_actual = '31')and (t1.cod_analisis='1')";
						break;
					case "32": 
						$Estado = "where (t6.fecha_hora between  '".$FechaI."' and '".$FechaT."') and (t1.estado_actual = '32')and (t1.cod_analisis='1')";
						break;
					default:
						$Estado ="where (t6.fecha_hora between  '".$FechaI."' and '".$FechaT."') and (t1.estado_actual = '31' or t1.estado_actual = '32')and (t1.cod_analisis='1')";
					break;
				}
				$Consulta = "select distinct t1.fecha_hora,t1.estado_actual,t2.descripcion as nomproducto,t3.abreviatura as nomsubproducto,t1.id_muestra, ";
				$Consulta = $Consulta."t1.rut_funcionario,t1.tipo_solicitud,t1.recargo,t1.cod_analisis, ";
				...
				while ($Fila=mysqli_fetch_array($Respuesta))
				{
					...
					echo "<td width='92'><div align='left'><input name='TxtIdMuestra' type='text' readonly style='width:92' maxlength='10' value ='".$TxtIdMuestra = $Fila["id_muestra"]."'></td>";
					echo "<td width ='108'><div align ='left'><input name ='TxtProducto' type='text' readonly style='width:108' maxlength='250' value ='".$TxtProducto=ucwords(strtolower($Fila["nomsubproducto"]))."'><input  type = 'hidden' value ='".substr($Fila["fecha_hora"],0,10)."'><input type = 'hidden' value ='".substr($Fila["fecha_hora"],11,8)."'></div></td>";
					if ($Fila["cod_analisis"]==1)
					{
						echo "<td width ='90'><div align ='left'><input name ='TxtEstados' type='text' readonly style='width:90' maxlength='85'value ='".$TxtEstado= $Fila["nombre_subclase"]."'></div></td>";
					}
					else
					{
						echo "<td width ='90'><div align ='left'><input name ='TxtEstados' type='text' readonly style='width:90' maxlength='85'value ='".$TxtEstado= $Fila["nombre_subclase"]."'></div></td>";					
	 ...
	 if (($CmbProductos!=-1) && ($CmbSubProducto== -1))
			{
				...
				{
					case "-1":
						$Estado ="where (t6.fecha_hora between  '".$FechaI."' and '".$FechaT."') and (t1.estado_actual = '31' or t1.estado_actual = '32' ) and (t1.cod_producto = '".$CmbProductos."')";
						break;
					case "31": 
						$Estado = "where (t6.fecha_hora between  '".$FechaI."' and '".$FechaT."') and (t1.estado_actual = '31') and (t1.cod_producto = '".$CmbProductos."')and (t1.cod_analisis='1')";
						break;
					case "32": 
						$Estado = "where (t6.fecha_hora between  '".$FechaI."' and '".$FechaT."') and (t1.estado_actual = '32') and (t1.cod_producto = '".$CmbProductos."' )and (t1.cod_analisis='1')";
						break;
					default:
						$Estado ="where (t6.fecha_hora between  '".$FechaI."' and '".$FechaT."') and (t1.estado_actual = '31' or t1.estado_actual = '32') and (t1.cod_producto = '".$CmbProductos."')";
					break;
				...
				$Consulta = "select distinct t1.fecha_hora,t1.estado_actual,t2.descripcion as nomproducto,t3.abreviatura as nomsubproducto,t1.id_muestra, ";
				$Consulta = $Consulta."t1.rut_funcionario,t1.recargo,t1.tipo_solicitud,t1.cod_analisis, ";
				$Consulta =	$Consulta."t1.nro_solicitud,t6.fecha_hora as FechaRecepcion,t7.nombre_subclase,t6.cod_estado,if(length(t1.recargo)=1,concat('0',t1.recargo),t1.recargo) as recargo_ordenado ";
				$Consulta = $Consulta."from cal_web.solicitud_analisis t1 " ;
				$Consulta = $Consulta."inner join proyecto_modernizacion.productos t2 on t2.cod_producto=t1.cod_producto ";
				$Consulta = $Consulta."inner join proyecto_modernizacion.subproducto t3 on t3.cod_producto=t1.cod_producto and t3.cod_subproducto=t1.cod_subproducto ";
	   ...
	   if ($Mostrar== 'O')  //BUSQUEDA DIRECTA
	   {
				...
				$Consulta = "select distinct t1.fecha_hora,t1.tipo_solicitud,t1.estado_actual,t2.descripcion as nomproducto,t3.abreviatura as nomsubproducto,t1.id_muestra, ";
				$Consulta = $Consulta." t1.rut_funcionario,t1.recargo, ";
				$Consulta =	$Consulta." t1.nro_solicitud,t6.fecha_hora as FechaRecepcion,t7.nombre_subclase,t6.cod_estado,if(length(t1.recargo)=1,concat('0',t1.recargo),t1.recargo) as recargo_ordenado ";
				$Consulta = $Consulta." from cal_web.solicitud_analisis t1 " ;
				$Consulta = $Consulta." inner join proyecto_modernizacion.productos t2 on t2.cod_producto=t1.cod_producto ";
				$Consulta = $Consulta." inner join proyecto_modernizacion.subproducto t3 on t3.cod_producto=t1.cod_producto and t3.cod_subproducto=t1.cod_subproducto ";
				...
				$Consulta = $Consulta." left join cal_web.estados_por_solicitud t6 on (t1.rut_funcionario=t6.rut_funcionario) and (t1.nro_solicitud = t6.nro_solicitud) and (t1.recargo = t6.recargo) and (t1.estado_actual = t6.cod_estado)";
				$Consulta = $Consulta." inner join proyecto_modernizacion.sub_clase t7 on t1.estado_actual = t7.cod_subclase  and t7.cod_clase = '1002'";
				$Consulta = $Consulta." where (t1.nro_solicitud between '".$SolIni."' and '".$SolFin."') and (t1.estado_actual = '31' or t1.estado_actual = '32')and (t1.cod_analisis='1')";
			...
			<td height="25" align="center" valign="middle">Paginas &gt;&gt;
				<?php
					...
					{	
						...
						$Consulta = $Consulta." left join cal_web.estados_por_solicitud t6 on (t1.rut_funcionario=t6.rut_funcionario) and (t1.nro_solicitud = t6.nro_solicitud) and (t1.recargo = t6.recargo) and (t1.estado_actual = t6.cod_estado)";
						$Consulta = $Consulta." inner join proyecto_modernizacion.sub_clase t7 on t1.estado_actual = t7.cod_subclase  and t7.cod_clase = '1002'";
						$Consulta = $Consulta." where (t1.nro_solicitud between '".$SolIni."' and '".$SolFin."') and (t1.estado_actual = '31' or t1.estado_actual = '32')".$TipoAnalisis;
					}
					...
					{	
						...
						for ($i = 0; $i <= $NumPaginas; $i++)
						{
							...
							{
								$StrPaginas.=  "<a href=JavaScript:Recarga('cal_adm_ingreso_leyes_frx.php?LimitIni=".($i * $LimitFin)."&CmbEstado=".$CmbEstado."&Mostrar=".$Mostrar."');>";
								$StrPaginas.= ($i + 1)."</a>&nbsp;-&nbsp;\n";
							}
						}
