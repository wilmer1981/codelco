...
{	
	global $CookieRut;
	if(ObtenerCorreos($Producto,$SAz,$link))
	{
...
function IngresaCobre($Prod,$SubPro,$NumSA,$FechaR,$RutFun,$RutQuim,$link)
{
	...
	if ($Fila["candados_imp_abiertos"] == 0)
	{
		...
		$Consulta.= " from cal_web.leyes_por_solicitud ";
		$Consulta.= " where nro_solicitud=".$NumSA;
		if (($Prod==16 || $Prod==17 || $Prod==19 || $Prod==48) || ($Prod==18 && ($SubPro== '16' || $SubPro=='17' || $SubPro=='49' || $SubPro=='02' || $SubPro=='57')))
			$Consulta.= " and (cod_leyes <> '02')";
		else
															...
															if($Fila=mysqli_fetch_array($Respuesta))
															{
																...
																if ($PonerCandado=="S")
																{
																	...
																	if (($Producto == "18" && ($SubProducto == "6" || $SubProducto == "7" || $SubProducto == "8"  || 
																		...
																		|| $Subproducto == "50" || $SubProducto == "51" || $SubProducto == "52" || $SubProducto =="54" || $SubProducto =="57")) 
...
function TraspasarDatoPI($Producto,$SubProducto,$SA,$link)
{
	...
	if($Fila=mysqli_fetch_array($Resp))
	{
		...
		while($FilaLey=mysqli_fetch_array($RespLey))
		{
			...
			if(trim($Valor_Ley)=='')
				$Valor_Ley=0;
			if(trim($FechaCosecha)=='')
				$FechaCosecha='0000-00-00';	
