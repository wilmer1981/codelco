...
<input type="hidden" name="LimitIni" value="<?php echo $LimitIni; ?>">
  ...
  <table width="760" border="0" class="TablaDetalle">
    ...
    <tr> 
      <td height="24">Flujos</td>
      <td><select name="CmbFlujos">
	  <option value="-1">Seleccionar</option>
	  <?php
	  $Consulta="select descripcion,cod_flujo,if(length(cod_flujo)=2,concat('0',cod_flujo),cod_flujo) ";
	  $Consulta.="	as cod_flujo_ordenado from proyecto_modernizacion.flujos ";
	  $Consulta.=" where  sistema='CAL' order by cod_flujo_ordenado ";
	  $Respuesta=mysqli_query($link, $Consulta);
	  while($Fila=mysqli_fetch_array($Respuesta))
	  {
	  	if ($CmbFlujos == $Fila["cod_flujo"])
		{
			echo "<option value='".$Fila["cod_flujo"]."' selected>".$Fila["cod_flujo"].".-".$Fila["descripcion"]." </option>";
		}
		else
		{
			echo "<option value='".$Fila["cod_flujo"]."'>".$Fila["cod_flujo"].".-".$Fila["descripcion"]." </option>";
		}
	  }
	  ?>
      </select> </td>
...
<?php
	...
	$Consulta = $Consulta." inner join proyecto_modernizacion.flujos t5 on t4.flujos = t5.cod_flujo ";
...
?>
    ...
    <?php	
	$Consulta = "select STRAIGHT_JOIN fecha_muestra,nro_solicitud,recargo, if(length(recargo)=1,concat('0',recargo),recargo) as recargo_ordenado, id_muestra, ";
	$Consulta = $Consulta." rut_funcionario, fecha_hora,t1.agrupacion,t3.descripcion ";
	...
	$Consulta = $Consulta." inner join proyecto_modernizacion.flujos t3 on t2.flujos = t3.cod_flujo ";
	...
	if ($CmbPeriodo == "-1")//todos los periodos
	{
		$Consulta = $Consulta." and (t2.flujos = '".$CmbFlujos."') order by t1.nro_solicitud,recargo_ordenado ";
	}
	else
	{
		$Consulta = $Consulta." and (t2.flujos = '".$CmbFlujos."') and (t1.cod_periodo = '".$CmbPeriodo."')order by t1.nro_solicitud,recargo_ordenado ";
	}
...
?>
  ...
  <table width="760" border="0" cellpadding="0" cellspacing="0">
          <tr>
			...
			$Consulta.=" inner join proyecto_modernizacion.flujos t3 on t2.flujos = t3.cod_flujo ";
			...
			if ($CmbPeriodo == "-1")//todos los periodos
			{
				$Consulta = $Consulta." and (t2.flujos = '".$CmbFlujos."')  ";
			}
			else
			{
				$Consulta = $Consulta." and (t2.flujos = '".$CmbFlujos."') and (t1.cod_periodo = '".$CmbPeriodo."') ";
			}
			//echo $Consulta."<br>"; 
			$Respuesta = mysqli_query($link, $Consulta);
			...
			for ($i = 0; $i <= $NumPaginas; $i++)
			{
				...
				{
					$StrPaginas.=  "<a href=JavaScript:Recarga('cal_con_enabal.php','".($i * $LimitFin)."');>";
					$StrPaginas.= ($i + 1)."</a>&nbsp;-&nbsp;\n";
				}
			}
