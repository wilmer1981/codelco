...
<tr class="ColorTabla01">
	<td align="center"><a href="JavaScript:Proceso('O','PD');" class="LinksBlancoRaya">Sub-Producto</a></td>
	<td align="center"><a href="JavaScript:Proceso('O','PV');" class="LinksBlancoRaya">Proveedor</a></td>
	<td align="center"><a href="JavaScript:Proceso('O','L');" class="LinksBlancoRaya">Lote</a></td> 
	<td align="center"><a href="JavaScript:Proceso('O','FR');" class="LinksBlancoRaya">Rec.Lote</a></td>
	<td>Recep.Mue</td>
	<td>Recep.Lab</td>
...
<?php
	...
	$Consulta = "select STRAIGHT_JOIN distinct t1.cod_producto, t1.cod_subproducto,t1.lote, t1.muestra_paralela, t1.rut_proveedor, t3.nomprv_a, ";
	...
	if($Ano<2009 && $Ano>0)
		$Consulta.= " left join cal_histo.solicitud_analisis_a_".$Ano." t4 on t1.lote=t4.id_muestra  ";
		else
	...
	switch ($Orden)
	{
		...
		case "FR":
			...
			break;
		default:
			$Consulta.= " order by t1.cod_producto,t1.cod_subproducto, t1.lote  ";
			break;
	}		
	...
	{
		//SOLICITUD DEL LOTE
		$Consulta = "select STRAIGHT_JOIN distinct t2.nro_solicitud ,t2.recargo , t2.estado_actual, t3.nombre_subclase";
		//$Consulta.= " ";
		if($Ano<2009 && $Ano>0)
			$Consulta.= " from age_web.lotes t1 inner join cal_histo.solicitud_analisis_a_".$Ano." t2 on t1.lote=t2.id_muestra  ";
			else
			$Consulta.= " ,t2.nro_sa_lims from age_web.lotes t1 inner join cal_web.solicitud_analisis t2 on t1.lote=t2.id_muestra  ";
		...
		//RETALLA
		$Consulta = "select STRAIGHT_JOIN distinct t2.nro_solicitud, t2.estado_actual, t3.nombre_subclase";
		$Consulta.= " from age_web.lotes t1 ";
		if($Ano<2009 && $Ano>0)
			$Consulta.= " inner join cal_histo.solicitud_analisis_a_".$Ano." t2 on t1.lote=t2.id_muestra  ";
			else
		...
		$Consulta.= " from age_web.lotes t1 ";
		if($Ano<2009 && $Ano>0)
			$Consulta.= " inner join cal_histo.solicitud_analisis_a_".$Ano." t2 on t1.muestra_paralela=t2.id_muestra    and year(t1.fecha_recepcion)=year(t2.fecha_muestra)  ";
			else
		...
		echo "<td>".substr($Fila["fecha_recepcion"],8,2)."/".substr($Fila["fecha_recepcion"],5,2)."/".substr($Fila["fecha_recepcion"],0,4)." ".substr($Fila["fecha_recepcion"],10,6)."</td>\n";
		...
		if 	($SA_Retalla==""){
			...
			if ($SALims!='') {
					echo "<td><a href=\"JavaScript:Historial('".$SA."','R')\" class=\"LinksAzul\">".$SALims."</a></td>\n";
				}else{
