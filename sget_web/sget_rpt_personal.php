...
function Procesos(TipoProceso)
{
	...
	{
		case "R":	
			f.action ="sget_rpt_personal.php?Recarga=S";
			f.submit();
		break;
		case "C"://BUSCAR
			if(f.TxtFInicio.value!=''&&f.TxtFTermino.value=='')
				f.TxtFTermino.value=f.TxtFInicio.value;
			f.action = "sget_rpt_personal.php?Buscar=S";
			f.submit();
		break;
		case "E"://EXCEL
			URL='sget_rpt_personal_excel.php?TxtRutPrv='+f.TxtRutPrv.value+"&TxtDv="+f.TxtDv.value+"&TxtApellido="+f.TxtApellido.value+"&CmbSexo="+f.CmbSexo.value+"&CmbTipoPersona="+f.CmbTipoPersona.value;
			URL=URL+"&CmbAfp="+f.CmbAfp.value+"&CmbSalud="+f.CmbSalud.value+"&CmbCiudad="+f.CmbCiudad.value+"&CmbComunas="+f.CmbComunas.value+"&CmbCargo="+f.CmbCargo.value+"&TxtFInicio="+f.TxtFInicio.value+"&TxtFTermino="+f.TxtFTermino.value;
			URL=URL+"&TxtCtto="+f.TxtCtto.value+"&CmbSindicato="+f.CmbSindicato.value+"&CmbEmpresa="+f.CmbEmpresa.value+"&TxtTarjeta="+f.TxtTarjeta.value+"&CmbEstado="+f.CmbEstado.value+"&CmbCertAnt="+f.CmbCertAnt.value+"&CmbTipoCtto="+f.CmbTipoCtto.value;
...
<input name='TxtRutPrv' type='text'   value='<? $TxtRutPrv;?>' size='12' maxlength='8' onBlur=CalculaDv(this.form,'TxtRutPrv','TxtDv') onKeyDown="ValidaIngreso('S',false,this.form,'TxtDv')">-<input name="TxtDv" type="text"  id="TxtDv" value="<? $TxtDv;?>"  size="3" maxlength="1">      
  ...
  <tr>
    ...
    <td class='formulario2'>N&ordm; Tarjeta 
    <td class='formulario2'><input name="TxtTarjeta" type="text" value="<? echo $TxtTarjeta; ?>" size="25">  </tr>
  <tr>
    <td height="17" class='formulario2'>Sexo</td>
    <td width="349" class='formulario2'><SELECT name="CmbSexo" >
      <?
		if ($CmbSexo=="-1")	
		{	
  ...
  <tr>
   ...
   <tr>
    ...
    <td class='formulario2' ><SELECT name="CmbCargo">
		<option value="-1" SELECTed="SELECTed">Todos</option>
		<?
  ...
  <tr>
    ...
    <td colspan="2" class='formulario2' >Fecha Desde
      <input name="TxtFInicio" type="text" readonly   size="10" value="<? echo $TxtFInicio; ?>"  >
      <img src="archivos/calendario.gif" alt="Pulse Aqui ParaSeleccionar Fecha" width="22" height="18" border="0" align="absmiddle" onClick="popFrame.fPopCalendar(TxtFInicio,TxtFInicio,popCal);return false">&nbsp;<a href="JavaScript:Limpia(1)"></a> Hasta 
  ...
  <tr>
    <td width="155"class='formulario2'>Empresa</td>
...
<SELECT name="CmbEmpresa" id="CmbEmpresa" >
  ...
  <tr>
    <td class='formulario2'>Cert. Antecedentes</td>
    <td class='formulario2'><SELECT name="CmbCertAnt">
      <option value="-1" SELECTed="SELECTed">Todos</option>
      <?
		switch($CmbCertAnt)
		{
			case "D":
				echo "<option value='D' SELECTed>Al Dia</option>";
				echo "<option value='V' >Vencidos</option>";
				echo "<option value='S' >Si</option>";
				echo "<option value='N'>No</option>";
			break;
			case "V":
				echo "<option value='D' >Al Dia</option>";
				echo "<option value='V' SELECTed>Vencidos</option>";
				echo "<option value='S' >Si</option>";
				echo "<option value='N'>No</option>";
			break;
			case "S":
    ...
    <td class='formulario2'><SELECT name="CmbTipoCtto">
  ...
  if($Buscar=='S')
  {
		...
		<!--<td width="4%" align="center"  class="TituloTablaVerde">AFP</td>-->
		<td width="9%" align="center"  class="TituloTablaVerde">Nro. Tarjeta</td>
		<td width="8%" align="center"  class="TituloTablaVerde">Cert.Anteced.</td>
		</tr>
		<?
		...
		if($TxtFInicio!=''&&$TxtFTermino)
			...
			$Consulta.= " and t1.fec_ini_ctto between '".$TxtFInicio."' and  '".$TxtFTermino."'";	
		if($CmbEmpresa != "-1")
		...
		while($FilaMod=mysql_fetch_array($RespMod))
		{
			...
			$CertAntecedentes=$FilaMod[num_cert_antecedentes];
			...
			<td><? echo number_format($CertAntecedentes,0,",","."); ?>&nbsp;</td>
			</tr>
		<?
			$Cont++;
		}
		?>
		<tr><td width="12%" align="left"  class="TituloTablaVerde" colspan="11" >Cantidad Registros:&nbsp;<? echo number_format($Cont,0,'','.');?></td></tr>
		</table>
	  </td>
