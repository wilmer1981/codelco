		...
		case "G":
			...
			if($Archivo_name!='none')
			{
				...
				if(strtoupper($Extension[1])!='EXE'&&strtoupper($Extension[1])!='ZIP'&&strtoupper($Extension[1])!='RAR')
				{
					...
					if($Acento==false)
					{
							$NombreArchivo="N".str_pad($CmbTipoDoc,3,0,STR_PAD_LEFT).str_pad($ID,7,0,STR_PAD_LEFT)."_".$Archivo_name;
							if (copy($Archivo, $Directorio."/".$NombreArchivo))
							{
								...
								$Insertar.="'$FechaHora','N','','$CmbTipoDoc',";
			...
			header('location:sget_doc_normativa.php?Formulario='.$Formulario.'&Pagina='.$Pagina.'&Proceso='.$Proceso."&ID=".$ID."&Mensaje=".$Mensaje."&TipoVolver=".$TipoVolver."&CodEstO=".$CodEstO."&NumReq=".$ID."&Acento=".$Acento);
			break;
	}
