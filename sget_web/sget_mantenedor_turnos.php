...
function Proceso(Opc)
{
	...
	switch(Opc)
	{
		...
		case "C":
			f.action="sget_mantenedor_turnos.php?&Buscar=S";
			f.submit();
		break;
		case "N":
			URL="sget_mantenedor_turnos_proceso.php?Opc="+Opc;
		...
		case "M":
			if(SoloUnElemento(f.name,'CheckTipoDoc','M'))
			{
				...
				URL="sget_mantenedor_turnos_proceso.php?Opc="+Opc+"&Valores="+Datos;
				opciones='top=30,toolbar=0,resizable=0,menubar=0,status=1,width=660,height=200,scrollbars=1';
				verificar_popup(popup);
				popup=window.open(URL,"",opciones);
				popup.focus();
		...
		case "E":
			if(SoloUnElemento(f.name,'CheckTipoDoc','E'))
			{
				...
				{
					Datos=Recuperar(f.name,'CheckTipoDoc');
					f.action='sget_mantenedor_turnos01.php?Opcion=E&Valor='+ Datos; //Datos; //+"&Pagina="+f.Pagina.value;
					f.submit();
				}
			}	
		break;
...
<?
 $IP_SERV = $HTTP_HOST;
 EncabezadoPagina($IP_SERV,'mant_turnos.png')
  ...
  <tr>
   ...
   <td align="center"><table width="930" border="1" cellpadding="4" cellspacing="0" >
	  ...
	  <tr align="center">
        ...
        <td width="55%" class="TituloTablaVerde">Descripci&oacute;n Turno </td>
		<td width="55%" class="TituloTablaVerde">Estado</td>
     </tr>
<?
if($Buscar=='S')
{
	$Consulta = "SELECT t1.cod_turno,t1.descrip_turno,t1.estado,t2.nombre_subclase as estado_turno ";
	$Consulta.= " from sget_turnos t1 ";
	$Consulta.=" left join  proyecto_modernizacion.sub_clase t2  on t1.estado=t2.cod_subclase and t2.cod_clase='30007'";
	$Consulta.=" where t1.cod_turno<>''";
	if($TxtDescripcion!='')
	...
	{
		$CodTipoDoc=$Fila["cod_turno"];
		$DescripDoc =$Fila["descrip_turno"];
		$Estado =$Fila["estado_turno"];
		
?>		
      	<tr >
        <td  align="center"><? echo "<input name='CheckTipoDoc' class='SinBorde' type='checkbox'  value='".$Fila["cod_turno"]."'>" ?></td>
