...
if ($Opc=='M')
{
	$Consulta="SELECT * from sget_cargos t1 ";
	$Consulta.=" where t1.cod_cargo='".$Valores."' ";
	
	$Resp=mysql_query($Consulta);
	if($Fila=mysql_fetch_array($Resp))
	{
		$TxtCodigo=$Fila["cod_cargo"];
		$TxtDescripcion=$Fila["descrip_cargo"];
		$CmbEstado=$Fila["estado"];
	}
}
else
{
	$Consulta="SELECT max(cod_cargo+1) as maximo from sget_cargos ";
	$Resp=mysql_query($Consulta);
	if($Fila=mysql_fetch_array($Resp))
	{
...
<?
	if ($Opc=='N')
		echo "<title>Nuevo Cargo</title>";
		else	
			echo "<title>Modifica Cargo</title>";
?>
...
function Proceso(Opcion)
{
	...
	switch(Opcion)
	{
		case "N":
			...
			if (Veri==true)
			{
				f.action = "sget_mantenedor_cargos01.php?Opcion="+ Opcion+"&Codigo="+f.TxtCodigo.value+"&Descri="+f.TxtDescripcion.value;
				f.submit();
			}
		break;
		case "M":
...
<input type="hidden" name="Form" value="<? echo $Form;?>">
  ...
  <tr>
   ...
   <td><table width="100%" border="0" cellpadding="0" cellspacing="0">
     <tr>
       <td width="74%" align="left"><?	if($Opc=='N'){?><img src="archivos/sub_tit_cargo_n.png"><? }else{?><img src="archivos/sub_tit_cargo_m.png"><?	}?></td>
