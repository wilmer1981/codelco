...
function Proceso(Opc)
{
	...
	{
		case "C":
			f.action="sget_mantenedor_mutuales.php?&Buscar=S";
			f.submit();
		break;
	
		case "N":
			URL="sget_mantenedor_mutuales_proceso.php?Opc="+Opc;
		...
		case "M":
			if(SoloUnElemento(f.name,'CheckTipoDoc','M'))
			{
				...
				URL="sget_mantenedor_mutuales_proceso.php?Opc="+Opc+"&Valores="+Datos;
				opciones='top=30,toolbar=0,resizable=0,menubar=0,status=1,width=660,height=200,scrollbars=1';
				verificar_popup(popup);
				popup=window.open(URL,"",opciones);
				popup.focus();
		...
		case "E":
			if(SoloUnElemento(f.name,'CheckTipoDoc','E'))
			{
				...
				{
					Datos=Recuperar(f.name,'CheckTipoDoc');
					f.action='sget_mantenedor_mutuales01.php?Opcion=E&Valor='+ Datos; //Datos; //+"&Pagina="+f.Pagina.value;
					f.submit();
				}
			}	
		break;
...
<?
 $IP_SERV = $HTTP_HOST;
 EncabezadoPagina($IP_SERV,'mant_mutuales.png')
 ?>
   ...
   <td>	 
	<table width="100%" border="1" cellpadding="4" cellspacing="0" >
	  ...
	  <tr align="center">
        ...
        <td width="55%" class="TituloTablaVerde">Descripci&oacute;n</td>
		 <td width="30%" class="TituloTablaVerde">Abreviatura</td>
		 <td width="30%" class="TituloTablaVerde">Estado</td>
        </tr>
<?
if($Buscar=='S')
{
	$Consulta = "SELECT t1.cod_mutual,t1.descripcion,t1.abreviatura,t2.nombre_subclase as estado_mutual ";
	$Consulta.= " from sget_mutuales_seg t1 ";
	$Consulta.=" left join  proyecto_modernizacion.sub_clase t2  on t1.estado=t2.cod_subclase and t2.cod_clase='30007'";
	$Consulta.=" where t1.cod_mutual<>''";
	if($TxtDescripcion!='')
		$Consulta.=" and  upper(t1.descripcion) like ('%".strtoupper(trim($TxtDescripcion))."%')";
	if($TxtAbreviatura!='')
		$Consulta.=" and  upper(t1.abreviatura) like ('%".strtoupper(trim($TxtAbreviatura))."%')";
	$Consulta.= " order by descripcion ";
	$Resp = mysql_query($Consulta);
	echo "<input name='CheckTipoDoc' type='hidden'  value=''>";
	while ($Fila=mysql_fetch_array($Resp))
	{
		$CodTipoDoc=$Fila["cod_mutual"];
		$DescripDoc =$Fila["descripcion"];
		$Abrev =$Fila["abreviatura"];
		$Estado =$Fila["estado_mutual"];
		
?>		
      	<tr >
        <td  align="center"><? echo "<input name='CheckTipoDoc' class='SinBorde' type='checkbox'  value='".$Fila["cod_mutual"]."'>" ?></td>
