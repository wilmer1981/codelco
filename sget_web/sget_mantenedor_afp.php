...
function Proceso(Opc)
{
	...
	{
		case "C":
			f.action="sget_mantenedor_afp.php?&Buscar=S";
			f.submit();
		break;
		case "N":
			URL="sget_mantenedor_AFP_proceso.php?Opc="+Opc;
		...
		case "M":
			if(SoloUnElemento(f.name,'CheckTipoDoc','M'))
			{
				...
				URL="sget_mantenedor_AFP_proceso.php?Opc="+Opc+"&Valores="+Datos;
				opciones='top=30,toolbar=0,resizable=0,menubar=0,status=1,width=660,height=200,scrollbars=1';
				verificar_popup(popup);
				popup=window.open(URL,"",opciones);
				popup.focus();
...
<?
 $IP_SERV = $HTTP_HOST;
 EncabezadoPagina($IP_SERV,'mant_afp.png')
 ?>
...
if($Buscar=='S')
{
	$Consulta = "SELECT t1.cod_afp,t1.descripcion_afp,t1.abreviatura_afp,t2.nombre_subclase as estado_afp ";
	$Consulta.= " from sget_afp t1 ";
	$Consulta.=" left join  proyecto_modernizacion.sub_clase t2  on t1.estado=t2.cod_subclase and t2.cod_clase='30007'";
	$Consulta.=" where t1.cod_afp<>''";
	if($TxtDescripcion!='')
		$Consulta.=" and  upper(t1.descripcion_afp) like ('%".strtoupper(trim($TxtDescripcion))."%')";
	if($TxtAbreviatura!='')
	...
	{
		$CodTipoDoc=$Fila["cod_afp"];
		$DescripDoc =$Fila["descripcion_afp"];
		$Abrev =$Fila["abreviatura_afp"];
		$Estado =$Fila["estado_afp"];
?>		
      	<tr >
        <td  align="center"><? echo "<input name='CheckTipoDoc' class='SinBorde' type='checkbox'  value='".$Fila["cod_afp"]."'>" ?></td>
...
?>			
  ...
  <td width="10" background="archivos/images/interior/form_der.gif">&nbsp;</td>
  </tr>
   <tr>
