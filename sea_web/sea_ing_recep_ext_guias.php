<?php 
	...
	if(isset($_REQUEST["GuiaAnulada"])) {
		$GuiaAnulada = $_REQUEST["GuiaAnulada"];
	}else{
		$GuiaAnulada = '';
	}

	if(isset($_REQUEST["AnularGuia"])) {
		$AnularGuia = $_REQUEST["AnularGuia"];
	}else{
		$AnularGuia = '';
	}
	if(isset($_REQUEST["EliminarGuia"])) {
		$EliminarGuia = $_REQUEST["EliminarGuia"];
	}else{
		$EliminarGuia = '';
	}
	if(isset($_REQUEST["GuiaEliminada"])) {
		$GuiaEliminada = $_REQUEST["GuiaEliminada"];
	}else{
		$GuiaEliminada = '';
	}
	...
	while($FilaGuia=mysqli_fetch_array($Resp))
	{
		...
		while($FilaDetalle=mysqli_fetch_array($RespDetalle))
		{
			...
			{	
				...
				if(isset($FilaEst["estado"])){
					$estadofil = $FilaEst["estado"];
				}else{
					$estadofil = "";
				}
				//if($FilaEst["estado"]=='X')
				if($estadofil=='X')
					...
					echo "<td align='center'><input type='radio' name='OptAnulaGuia' onclick=AnularGuiaTTE('".$FilaDetalle["guia"]."','".$estadofil."','".$ExisteMov."')></td>";
					echo "<td align='center'><input type='radio' name='OptEliminarGuia' onclick=EliminarGuiaTTE('".$FilaDetalle["guia"]."','".$estadofil."')></td>";
