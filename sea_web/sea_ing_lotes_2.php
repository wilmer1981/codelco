...
function Consultar(f)
{
	if (f.cmbanodos.value == -1)
	{
		...
		window.open("sea_con_lotes.php?cmbanodos=" + f.cmbanodos.value, "","menubar=no resizable=no width=600 height=320");
} 
//******************//
function Recargar(f)
{
...
<table width="558" border="0" cellspacing="0" cellpadding="3" class="TablaInterior">
          <tr> 
			...
			$consulta = "SELECT * FROM proyecto_modernizacion.subproducto WHERE cod_producto = 17 AND cod_subproducto not in (4,8,11) AND mostrar_sea = 'S' ORDER BY descripcion";
