	...
	for ($k=0; $k < $largo_h; $k++)
	{
		...
		{				
			...
			$Insertar = "$Insertar (tipo_movimiento,cod_producto,cod_subproducto,hornada,numero_recarga,fecha_movimiento,campo1,
								   campo2,unidades,flujo,peso,hora)";
		    $Insertar = "$Insertar VALUES(1,17,0,$valor_h,0,'$fecha','$guia','$patente',0,0,0,'$fecha_hora')";
			...
			for ($j=0; $l < $largo_a; $j++)
			{
				...
				{				
					...
					$Actualizar1 = "UPDATE movimientos set cod_subproducto = $valor_a, flujo = $flujo where fecha_movimiento='$fecha' and hornada= $valor_h and cod_subproducto = 0 and flujo=0 and tipo_movimiento = 1";
