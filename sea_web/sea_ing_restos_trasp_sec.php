<?php 
	...
	if(isset($_REQUEST["CmbLote"])) {
		$CmbLote = $_REQUEST["CmbLote"];
	}else{
		$CmbLote =  "";
	}
	...
	if(isset($_REQUEST["TxtPesoTot"])) {
		$TxtPesoTot = $_REQUEST["TxtPesoTot"];
	}else{
		$TxtPesoTot =  "";
	}
	...
	if(isset($_REQUEST["TxtPesoSelec"])) {
		$TxtPesoSelec = $_REQUEST["TxtPesoSelec"];
	}else{
		$TxtPesoSelec =  "";
	}
...
function EliminaTrasp(Opt)
{
	var frm=document.frm1;
	var Valores = "";
	if(confirm('Se eliminara Restos traspasado a SEC seleccionado'))
	{
		Valores = Opt;
		frm.action="sea_ing_restos_trasp_sec01.php?Proceso=EP&Valores=" + Valores;
		frm.submit();
	}
	return;
}
  ...
  <table width="770" border="0" cellpadding="5" cellspacing="0" class="TablaPrincipal">
    <tr>
	  <td width="762" height="316" align="center" valign="top"> 
        <table width="400" border="0" cellspacing="0" cellpadding="3" class="TablaInterior">
          ...
          <tr>
			...
			if($cmbproducto!='0-0')
			{
				...
				$Consulta.=" where t2.cod_producto='".$Prod[0]."' and t2.cod_subproducto='".$SubProd."'";// (t2.cod_subproducto=22 || t2.cod_subproducto = 23 || t2.cod_subproducto = 25) "; // MODIFICADOR POR ADAN 04-02-2011
				if ($letra=='A' or $letra == 'B' or $letra=='C')
				 	$Consulta.=" and (LEFT(t1.fecha_creacion_lote,4)='".$CmbAno."' or (LEFT(t1.fecha_creacion_lote,4)='".$CmbAno2."' and t1.cod_bulto in('M')))";// SE AGREGA and t1.cod_bulto in('M') MODIFICADOR POR ADAN 04-02-2011
					else
			...
			<?php //echo $var1; ?>
			<input type="hidden" name="CmbAno" value="<?php $CmbAno; ?>">
			</tr>
			<tr>
            <td>Peso Total Lote</td>
              <td><input type="text" align="right" name="TxtPesoTot" size="12" value="<?php $TxtPesoTot; ?>">
              (Kg.)</td>
		 </tr>
		<tr>
         <td>Peso Embarque Parcial </td>
			<td> <input type="text" align="right" name="pparcial"   size="12" value="<?php $pparcial; ?>" onChange="RevisaPeso(this.form);">
              (Kg.)</td>
		 </tr>
		...
		<?php
		$TotalStockFinUnid=0; //WSO
		$TotalStockFinPeso=0; //WSO
			if($recargapag=='S')
			{
				...
				while ($row = mysqli_fetch_array($rs))
				{
					...
					{				
						...
						echo "<td align='center'><a href=\"JavaScript:EliminaTrasp('".$SubProd."~".$Grupo."~".$hornadas."~".$FechaMov."')\">$hornadas</a></td>";
