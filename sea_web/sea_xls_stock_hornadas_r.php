		...
		{	
			...
			if ($recargapag == "S")
			{
				...
				else
				{
					...
					$consulta.= " WHERE t1.cod_producto = '".$arreglo[0]."' ";
					$consulta.= " AND t1.cod_subproducto = '".$arreglo[1]."' ";
					$consulta.= " AND t1.ano = YEAR(SUBDATE('".$FechaInicio."', INTERVAL 1 MONTH)) and t1.mes >= MONTH(SUBDATE('".$FechaInicio."', INTERVAL 1 MONTH)) and t1.mes <= MONTH('".$FechaInicio."')";
					$consulta.= " ORDER BY SUBSTRING(t1.hornada,7,6)";
				...
				while ($row = mysqli_fetch_array($rs))
				{
					...
					$row_aux = mysqli_fetch_array($rs_aux);
						if ($arreglo[0] == "19")
							$clave = $row_aux["grupo"].'-'.$row["cod_producto"].'-'.$row["cod_subproducto"].'-'.substr($row[hornada],6,6);
						else
				...
				while (list($clave,$valor) = each($array_hornadas))
				{
					...
					if ($PProducto=='17')
					{
							...
							{
							    if (($Arsenico < 300 || $Arsenico > 1500 || $Antimonio > 400) && ($Arsenico > 0))
							    {
								      echo '<tr bgcolor="#FF0000">';
							    }
							    else
