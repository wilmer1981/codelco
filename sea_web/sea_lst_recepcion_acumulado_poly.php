...
<?php
	...
	{
		$arreglo[] = array($row1["flujo"], $row1["cod_producto"], $row1["cod_subproducto"], $row1[horno_inicial], $row1[nodo]);
	}
...
*/	
	...
	// Saca todos los movimientos de recepcion afectados.
		reset($arreglo);
		while (list($clave, $valor) = each($arreglo)) // (0: flujo, 1: cod_producto, 2: cod_subproducto, 3: horno_inicial, 4: nodo)
		{	
			...
			{
				$consulta = "SELECT descripcion AS nombre FROM proyecto_modernizacion.flujos WHERE nodo= '".$valor[4]."' and cod_flujo = ".$valor[0];
				echo "FLUJO: ".$valor[0]." ";
			}
			...
			{		
				...
				echo '<td width="225">Limites Establecidos</td>';
		...
		$TotalPesoAcum = $TotalPesoAcum +  $row3["peso"];
			...
			/*echo '<table width="'.$largo.'" border="0" cellspacing="0" cellpadding="0" align="center" class="Detalle02">';
						echo '<tr class="Detalle02"><td colspan="2"</td>'; */
