...
<table width="320" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr class="ColorTabla01">
    <td align="center" colspan="4">RECHAZADOS A RAF</td>
  </tr>
...
<?php
	...
	{
		$consulta = "SELECT * FROM proyecto_modernizacion.relacion_prod_flujo_nodo WHERE cod_producto = 17 AND cod_proceso = 4";		
	}
	else
	{
		if ($radio == "P") //Radio Producto.
		{
			...
			{
			$consulta = "SELECT * FROM proyecto_modernizacion.relacion_prod_flujo_nodo WHERE cod_producto = 17 AND cod_proceso = 4 AND";
...
//
	...
	echo '<td width="40" align="center">Cant.</td>';
	...
	while (list($clave, $valor) = each($arreglo)) // (0: flujo, 1: cod_producto, 2: cod_subproducto, 3: horno_inicial)
	{
		...
		$consulta = $consulta." WHERE tipo_movimiento = 4 AND cod_producto = 17 AND cod_subproducto = ".$valor[2];
		...
		if(mysqli_num_rows($rs2) != 0) //Si la Consulta devuelve datos.
		{
			...
			echo '<tr class="ColorTabla01"><td colspan="4">';
			...
			//Crea el detalle.					
			echo '<table width="'.$largo.'" border="0" cellspacing="0" cellpadding="0">'; 
			while ($row2 = mysqli_fetch_array($rs2))
			{												  

				$consulta = "SELECT fecha_movimiento FROM movimientos";
				$consulta = $consulta." WHERE tipo_movimiento = 4 AND cod_producto = 17 AND cod_subproducto = ".$valor[2]." AND flujo = ".$valor[0];
