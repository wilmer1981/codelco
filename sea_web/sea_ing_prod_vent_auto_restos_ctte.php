<?php	
	...
	if ($Modif=="S")
	{
		...
		if(isset($Fila["fecha_movimiento"])){
...
?>
  ...
  <tr class="boxcontent">
    <td><font color="#000000"><b>NUMERO DE CARRO</font></b></td>
    <td width="12%"><font color="#000000"><b><input name="NumCarro" type="text" onBlur="BuscaPeso('C')" onKeyDown="TeclaPulsada('NumRack')" value="<?php echo $NumCarro; ?>" size="10"></td>
    <td colspan="2"><font color="#000000"><b><input name="PesoCarro" type="text"  onKeyDown="TeclaPulsada('NumRack')" value="<?php echo $PesoCarro; ?>" size="10">
        ...
        <input type="hidden" name="NuevoCarro" value="N"></b></font></td>
  ...
  <tr class="boxcontent">
    <td><font color="#000000"><b>NUMERO DE RACK</b></font></td>
    <td><font color="#000000"><b><input name="NumRack" type="text"  onBlur="BuscaPeso('R')" onKeyDown="TeclaPulsada('NumCubas')" value="<?php echo $NumRack; ?>" size="10"></td>
    <td colspan="2"><font color="#000000"><b><input name="PesoRack"  type="text"  onKeyDown="TeclaPulsada('NumCubas')" value="<?php echo $PesoRack; ?>" size="10" >
     <img src="../principal/imagenes/ing_rack.gif" width="90" height="18" name="ImgRack" style="visibility:hidden ">      <input type="hidden" name="NuevoRack" value="N"></b></font></td>
    <td width="16%">&nbsp;</td>
...
<table width="95%" border="1" cellpadding="2" cellspacing="0" class="TablaInterior">  
  <tr align="center" class="boxcontent">
	...
	if ($Modif == "S")	
	{
		...
		{
			$FechaG = $FilaAux["fecha"];
			echo "<input name='BtnGuardar' type='button' onClick=\"Proceso('M_RestosAnodos')\" value='Modificar' style='width:100px'>\n";
		}
		else
		{
			$FechaG = $FechaHora;
			echo "<input name='BtnGuardar' type='button' onClick=\"Proceso('M_RestosAnodos')\" value='Guardar' style='width:100px'>\n";
		}
	}
	else
	{
  ...
  <tr class="boxcontent">
	...
	<?php					
		...
		while ($Fila = mysqli_fetch_array($Respuesta))
		{
			...
			if(isset($tabla[$Fila["valor"]][0])){
				$fila01 = $tabla[$Fila["valor"]][0];
			}else{
				$fila01 = 0;
			}
			if(isset($tabla[$Fila["valor"]][1])){
				$fila02 = $tabla[$Fila["valor"]][1];
			}else{
				$fila02 = 0;
			}
			...
			echo "<td align='center'>".number_format($fila01,0,",",".")."</td>";
         	echo "<td align='center'>".number_format($fila02,0,",",".")."</td>";
	    	echo "</tr>";	
		}
		echo "<tr>";
...
<?php
	...
	/*if ($activar=="S")
		...
		if (isset($Grupo) && $Grupo!="")
			echo '<script language="javascript"> document.formulario.NumCarro.focus(); </script>';
		else
			echo '<script language="javascript"> document.formulario.Grupo.focus(); </script>';
?>
