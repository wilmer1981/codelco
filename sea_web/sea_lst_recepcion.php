...
<?php
	...
	$FechaFin=date('Y-m-d', mktime(1,0,0,intval(substr($fecha_ini, 5, 2)) ,intval(substr($fecha_ini, 8, 2)),intval(substr($fecha_ini, 0, 4))));
	$FechaFin2=date('Y-m-d', mktime(1,0,0,intval(substr($fecha_ini, 5, 2)) ,intval(substr($fecha_ini, 8, 2))+1,intval(substr($fecha_ini, 0, 4))));
	$FechaHoraIni=$fecha_ini." 08:00:00";
	$FechaHoraFin=date('Y-m-d',mktime(1,0,0,intval(substr($FechaFin2, 5, 2)) ,intval(substr($FechaFin2, 8, 2)),intval(substr($FechaFin2, 0, 4))))." 07:59:59";
	...
	{	
		...
		if (mysqli_num_rows($rs9) != 0) //Si existen datos escribir flujo.
		{
			...
			{				
				...
				//$FechaFin=date($fecha_aux,mktime(;
				$FechaFin=date('Y-m-d', mktime(1,0,0,intval(substr($fecha_aux, 5, 2)) ,intval(substr($fecha_aux, 8, 2)),intval(substr($fecha_aux, 0, 4))));
				$FechaFin2=date('Y-m-d', mktime(1,0,0,intval(substr($fecha_aux, 5, 2)) ,intval(substr($fecha_aux, 8, 2))+1,intval(substr($fecha_aux, 0, 4))));
				...
				$consulta = $consulta." FROM sea_web.movimientos AS t1";
				$consulta = $consulta." WHERE t1.tipo_movimiento = 1 AND t1.cod_producto = 17 AND t1.cod_subproducto = ".$valor[2];
				//$consulta = $consulta." AND t1.flujo = ".$valor[0]." AND t1.fecha_movimiento = '".$fecha_aux."'";		
