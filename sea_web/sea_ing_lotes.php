...
///////////// Agregado por WSO  /////////
	...
	if (isset($_REQUEST["txtorigen"])) {
		$txtorigen = $_REQUEST["txtorigen"];
	}else {
		$txtorigen = '';
	}
	if (isset($_REQUEST["txtmarca"])) {
...
function Consultar(f)
{
	...
	if (f.cmbanodos.value == -1)
	{
		...
		window.open("sea_con_lotes.php?mesl=" + f.mes.value + "&anol=" + f.ano.value +"&cmbanodos=" + f.cmbanodos.value +"&txtlote=" + f.txtlote.value, "","menubar=no resizable=no width=600 height=320");
  ...
  <table width="770" height="330" border="0" cellpadding="5" cellspacing="0" class="TablaPrincipal">
    <tr>
        <td width="776" align="center" valign="middle">
			<table width="558" border="0" cellspacing="0" cellpadding="3" class="TablaInterior">
				...
				<td width="135"> <SELECT name="cmbanodos" id="SELECT4" onChange="JavaScript:Recargar(this.form)">
					<?php
						...
						//$consulta = "SELECT * FROM proyecto_modernizacion.subproducto WHERE ((cod_producto = 17 AND cod_subproducto not in (4,8,11)) or (cod_producto='16' and (cod_subproducto='4' or cod_subproducto='24'))) AND mostrar_sea = 'S' ORDER BY cod_producto,descripcion";
						$consulta = "SELECT t1.cod_producto,t1.descripcion,t1.cod_subproducto FROM proyecto_modernizacion.subproducto t1 inner join proyecto_modernizacion.sub_clase t2 on t1.cod_producto=t2.valor_subclase1 and t1.cod_subproducto=t2.valor_subclase2 and t2.cod_clase='2015' ";
						$consulta.= "ORDER BY t1.cod_producto,t1.descripcion";
				...
				<tr> 
					...
					<td><input name="txthornada" type="text" size="10" value="<?php if(isset($txthornada)){echo $txthornada; }else{echo '';}?>"></td>
				</tr>
				<tr> 
					<td>Lote Origen</td>
					<td> 
			...
			<br>
			<table width="558" border="0" cellspacing="0" cellpadding="3">
				<tr>
			...
			<?php
				...
				//Agregado por WSO
				echo '<input name="mostrar" type="hidden" value="'.$mostrar.'">';
			?>
		</td>
	</tr>
</table>
