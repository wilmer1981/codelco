...
if($Proceso == 'B')
{
	...
	$TotalCantidad = 0;
	$TotalPeso = 0;
	$Consulta="SELECT distinct t1.cod_subproducto,t2.descripcion from sea_web.traspasos_sec t1 inner join proyecto_modernizacion.subproducto t2 on t1.cod_producto=t2.cod_producto and t1.cod_subproducto=t2.cod_subproducto ";
	...
	while($FilaProd=mysqli_fetch_array($RespProd))
	{
		...
		$consulta.= " WHERE tipo_movimiento = 10 AND cod_producto = 19 AND cod_subproducto='".$SubProd."' and fecha_movimiento = '".$fecha."' ";//and hora between '$FechaInicio' and '$FechaTermino'";
		$consulta.= " GROUP BY campo2, fecha_benef";
		...
		{												 
			...
			$consulta = $consulta." WHERE tipo_movimiento = 10 AND cod_producto = 19 AND cod_subproducto='".$SubProd."'";
			$consulta = $consulta." AND campo2 = '".$row2[campo2]."' AND fecha_movimiento = '".$fecha."' AND fecha_benef = '".$row2[fecha_benef]."'";// and hora between '$FechaInicio' and '$FechaTermino'";
			...
			{ 
				...
				$TotProdCantidad = $TotProdCantidad + $row_u["unidades"];
			}
			echo '<td width="100" align="center">';
			...
			{ 
				echo $row_p["peso"].'</td>';
				$TotProdPeso = $TotProdPeso + $row_p["peso"];
				$TotalPeso = $TotalPeso + $row_p["peso"];
			}
		}
		echo '<tr class="ColorTabla02">';
		echo '<td width="300" align="right" colspan="3">SUB-TOTAL DIA</td>';
