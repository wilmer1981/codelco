<?php 
	...
	$tipos = array(1=> "Recepcion", 2=> "Beneficio", 3=> "Produccion", 4=> "Traspaso");
?>
...
function Buscar(f)
{
    ...
    vector = f.cmbsubprod1.value.split("-");
   	f.TCodigo.value = vector[0];
	f.cmbsubprod.value = vector[1];
	...
	linea = "ok=S&proceso=B&radio=" + f.tipo_mov.value  + "&cmbsubprod1=" + f.cmbsubprod1.value + "&TCodigo=" + f.TCodigo.value + "&cmbsubprod=" + f.cmbsubprod.value + "&txtbuscar=" + f.busqueda.value;
	f.action = "sea_modificador_movimientos01_res.php?" + linea;
	f.submit();
}
/*********************/
function Buscar2(f) //Busca el promedio de la Hornada.
{
	...
	{
		linea = "recargapag2=S&recargapag1=S&radio=" + f.tipo_mov.value  + "&TCodigo=" + f.TCodigo.value + "&cmbsubprod=" + f.cmbsubprod.value + "&txtbuscar=" + f.txthornada.value;	
		f.action = "sea_modificador_movimientos_res.php?" + linea;
	}
	else
	{
		linea = "proceso=B";
...
//recargapag3=S&recargapag2=S&recargapag1=S&radio1=1&cmbsubprod=4&peso_prom=276.52&cmbhornada=2003101026&txtbuscar=1026&mostrar=S&tipo_prod=C
    vector2 = f.cmbsubprod1.value.split("-");
   	f.TCodigo.value = vector2[0];
	f.cmbsubprod.value = vector2[1];
	linea = "recargapag5=S&recargapag3=S&recargapag2=S&recargapag1=S&radio=" + f.tipo_mov.value + "&TCodigo=" + f.TCodigo.value + "&cmbsubprod=" + f.cmbsubprod.value + "&txtbuscar=" + f.txthornada.value;	
...
{
	f.action = "sea_modificador_movimientos_res.php?recargapag1=S&radio1=" + r.value;
	f.submit();
}
/********************/
...
function Grabar(f)
{
	...
	if (ValidaCampos(f))
	{
		...
		/***  Fecha/Tipo_mov/Unidades/Campo2/Campo1/Flujo/Numero_Recarga/Cod_producto/Fecha_Benef/Peso/cod_subproducto  ***/ 	
...
//		return;
		...
		//f.action = "sea_modificador_movimientos01_res.php?proceso=G&parametros=" + parametros;
		//f.submit();
	}
}
/*******************/
function Eliminar(f)
{
	...
	arreglo = f.radio[pos].value.split('/');
	alert (f.elements[parseInt(arreglo[0])+8].value);
	...
	if (valor == "S")		
	{
		...
		if (confirm("Esta Seguro de Eliminar el Movimiento Seleccionado"))
		{
			...
			parametros = parametros + arreglo[7] + '/' + arreglo[8] + '/' + arreglo[9] + '/' + arreglo[10];
		alert (parametros);
			//f.action = "sea_modificador_movimientos01_res.php?proceso=E&parametros=" + parametros;
			//f.submit();
		}
	}
	else 
...
<?php 
   echo '<input type="hidden" name="TCodigo" value="'.$TCodigo.'">';
   echo '<input type="hidden" name="cmbsubprod" value "'.$cmbsubprod.'">';
?>
	  ...
	  <table width="750" border="0" cellspacing="0" cellpadding="3" class="TablaInterior">
        ...
        <table width="750" border="0" cellspacing="0" cellpadding="3" class="TablaInterior">
          <tr> 
            ...
            <td><SELECT name="cmbsubprod1" id="cmbsubprod1">
              <option value="-1">SELECCIONAR</option>
              <?php					
					if ($recargapag1 == "S")
					{
						if ($radio1 == 1 or $radio1 == 3) 
							$consulta = "SELECT * FROM proyecto_modernizacion.subproducto WHERE cod_producto in ('16','17') AND mostrar_sea = 'S'";
						else 
						...
						{
						    $TmpPaso = $row["cod_producto"]."-".$row["cod_subproducto"];
							if ($TmpPaso == $cmbsubprod1)
								echo '<option value="'.$TmpPaso.'" SELECTed>'.$row["descripcion"].'</option>';
							else
								echo '<option value="'.$TmpPaso.'">'.$row["descripcion"].'</option>';
						}							
					}
			...
			<?php
				...
				{					
					if ($radio1 == 1 or $radio1 == 3) //Para Anodos.
						...
						echo '<input name="txtstock" type="text" size="10" value="'.StockActual($cmbhornada,$TCodigo,$cmbsubprod).'" readonly>';
			...
			<?php
				...
				{					
					if ($radio1 == 1 or $radio1 == 3)//Para Anodos.
					{
						...
						$Consulta.= " where cod_producto = '".$TCodigo."'";			
...
<?php
	...
	if ($activar == "S")
	{
		...
		echo 'frm1.action = "sea_modificador_movimientos01_res.php?proceso=B";';
		echo 'frm1.submit();';
		echo '</script>';
	}
...
<br>
	...
	if ($recargapag3 == "S")
	{
		...
		if ($radio1 == 1)				
		{
			...
			$consulta = "SELECT * FROM sea_web.movimientos WHERE trim(cod_producto) = '".$TCodigo."' AND cod_subproducto = ".$cmbsubprod;
		...
		while ($row2 = mysqli_fetch_array($rs2))
		{
			...
			if ($pos_aux == $posicion)
				...
				echo '<input type="radio" name="radio" value="'.$fila.'/'.$row2[fecha_movimiento].'/'.$row2[tipo_movimiento].'/'.$row2["unidades"].'/'.$row2[campo2].'/'.$row2[campo1].'/'.$row2["flujo"].'/'.$row2[numero_recarga].'/'.$row2["cod_producto"].'/'.$row2[fecha_benef].'/'.$row2["peso"].'/'.$row2["cod_subproducto"].'" onClick="JavaScript:Verificar(this.form)" checked>&nbsp;';
			else 
				echo '<input type="radio" name="radio" value="'.$fila.'/'.$row2[fecha_movimiento].'/'.$row2[tipo_movimiento].'/'.$row2["unidades"].'/'.$row2[campo2].'/'.$row2[campo1].'/'.$row2["flujo"].'/'.$row2[numero_recarga].'/'.$row2["cod_producto"].'/'.$row2[fecha_benef].'/'.$row2["peso"].'/'.$row2["cod_subproducto"].'" onClick="JavaScript:Verificar(this.form)">&nbsp;';
...
//				echo $consulta."<br>";
			...
			else 
			{
				...
				if (($row2[tipo_movimiento] == 2) or ($row2[tipo_movimiento] == 1) or ($row2[tipo_movimiento] == 4) )
