...
function CierreTTE(Ck,SP,Lote,Rec,Guia,UnidR,PesoR,Origen,UnidO,PesoO,FechaGuia,Marca)
{
	...
	if(Ck.checked==true)
	{
		...
		else
		{
			...
			{
				f.action="sea_ing_recep_inter_tte01.php?Proceso=EL&SubProducto="+SP+"&LoteVentana="+Lote+"&Rec="+Rec+"&Guia="+Guia;
				f.submit();
			}
		}	
	}	
  ...
  if($Est=='A')
  {
	  ...
	  echo'<tr class="ColorTabla01"> 
		...
		while($FilaGuia=mysqli_fetch_array($RespGuia))
		{
			...
			echo '</tr>'; 
			$Consulta="SELECT guia,lote_ventana from sea_web.recepcion_externa where ";
			if($Est=='A')
			...
			while($FilaGuia2=mysqli_fetch_array($RespGuia2))
			{
				...
				while($row = mysqli_fetch_array($rs))
				{
					...
					echo'<td align="center"><input type="checkbox" name="CheckElimina" onclick=CierreTTE(this,"'.$producto.'","'.$lote_ventana.'","'.$recargo.'","'.$Guia.'","'.$unidades.'","'.$PesoRecep.'","'.$lote_origen.'","'.$Unid.'","'.$peso_origen.'","'.$FechaGuiaTTE.'","'.$marca.'")></td>';
					echo'</td>
					 </tr>';
