            ...
            <td width="229"> 
              <?php	
			   ...
			   if($codigo == 16)
			   {
				   ...
				   $Consulta = "SELECT distinct hornada FROM sea_web.movimientos WHERE cod_producto = 16 AND cod_subproducto = $subproducto AND fecha_movimiento <= '$fecha'";
				   $rs = mysqli_query($link, $Consulta);
				   while($fila = mysqli_fetch_array($rs))
				   {			   
						$hornada = substr($fila[hornada],3,6);
						//Recep
						$Consulta = "SELECT SUM(unidades) as unid, SUM(peso) as peso FROM sea_web.movimientos WHERE tipo_movimiento = 1 AND cod_producto  = 16 AND hornada = $fila[hornada]";
						...
						//Trasp-Benef
						$Consulta = "SELECT SUM(unidades) as unid, SUM(peso) as peso FROM sea_web.movimientos WHERE tipo_movimiento IN (2,4) AND cod_producto  = 16 AND hornada = $fila[hornada]";
			   ...
			   if($codigo == 18)
			   {
					//echo "holaaaaaaaaaaaaaaaaaaaaaaaa".$cmbproducto;
				   $fecha = $ano1."-".$mes1."-".$dia1;	
		           $subproducto = substr($cmbproducto,2,2);
				   echo'<select name="txthornada" onChange="Buscar(this.form,2)">';
				   echo'<option value="0" selected>Seleccionar</option>';
	...
	if ($recargapag2 == "S")
	{
		...
		if ($boton == "1")
		{
			...
			if($codigo == "16") 			
			{
				...
				$consulta = $consulta." WHERE tipo_movimiento = 1 AND fecha_movimiento = '".$fecha."' AND cod_producto = 16"; 
		...
		else
		{
			...
			if($codigo == "16")//BLISTER 			
			{
				...
				$consulta = $consulta." WHERE cod_producto = 16 AND cod_subproducto = ".$subproducto." AND right(hornada_ventana,6) = ".$txthornada;
				//echo $consulta."<br>";
			}
			if($codigo != "16" && $codigo != "48" && $codigo != "18" && $codigo != "66")//ANODOS			
			{
		...
		if($codigo == 16)//BLISTER
		{
			...
			if ($row2 = mysqli_fetch_array($rs2))
			{
				...
				$consulta = "SELECT SUM(unidades) as unidades, SUM(peso) as peso_unidades FROM sea_web.movimientos WHERE fecha_movimiento <= '$fecha2' AND tipo_movimiento = 1 AND cod_producto = 16 AND hornada = ".$row2[hornada] ;
				$rs4 = mysqli_query($link, $consulta);
	
				 if($row4 = mysqli_fetch_array($rs4))
				 {			

					$consulta = "SELECT SUM(unidades) as unidades, SUM(peso) as peso FROM sea_web.movimientos WHERE fecha_movimiento <= '$fecha2' AND cod_producto = 16 AND tipo_movimiento IN(4,2) AND hornada = ".$row2[hornada];
					...
					echo '<tr>';
						echo '<td align="center"><input name="hornada['.$i.']" type="hidden" value="'.$row2[hornada].'">'.substr($row2[hornada],3,6);			
