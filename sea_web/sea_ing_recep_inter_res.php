...
var tope = i;
	if(i != '')	
	{
	    ...
	    if (f.proveedor.value=='A-61704005-0' || f.proveedor.value=='A-90132000-4')
		{
			f.action="sea_ing_recep_inter03.php?Proceso=G";
			f.submit();
		}
		else
		{
...
var f = frmPrincipal;
	    if (f.proveedor.value=='A-61704005-0' || f.proveedor.value=='A-90132000-4')
		{
			f.action="sea_ing_recep_inter03.php?Proceso=M";
			f.submit();
		}
		else
		{
...
{
var f = frmPrincipal;
		f.action="sea_ing_recep_inter_res.php?Proceso=V";
        f.submit();
...
var f = frmPrincipal;
	...
	f.action="sea_ing_recep_inter_res.php?mostrar=S";
	f.submit();
...
<?php //Anodos HVL
 ...
 if ($mostrar=="S" && $proveedor == "A-00001100-2")	
 {
	  ...
	  if($proveedor != "-1")
	  {
			...
			$consulta.= " WHERE FECHA = '".$fecha."' AND COD_PRODUCTO='1' AND COD_SUBPRODUCTO = '17' AND RUT_PRV = '".$prov."' and peso_neto <> '0' ORDER BY HORA_ENTRADA";
	  ...
	  else
	  {
			...
			$consulta.= " FROM SIPA_WEB.recepciones WHERE FECHA = '".$fecha."' AND COD_PRODUCTO='1' AND COD_SUBPRODUCTO = '17' ORDER BY GUIA_DESPACHO ";
			include("../principal/conectar_rec_web.php");
			$rs = mysqli_query($link, $consulta);
	  }	
	  while($row = mysqli_fetch_array($rs))
 	  {
			...
			$consulta.= " AND LOTE = '".$row[LOTE_A]."' AND PATENTE = '".$row[PATENT_A]."'";
			...
			if($row1 = mysqli_fetch_array($result))
			{
				 ...
				 $consulta_s.= " AND PATENTE = '".$row[PATENT_A]."' ";
				 ...
				 $consulta_u = "SELECT SUM(unidades) AS unid FROM movimientos WHERE fecha_movimiento = '$fecha' AND campo1 = '$row[GUIADP_A]' AND campo2 = '$row[PATENT_A]'";
...
<?php //Proveedores TTE y Disputada
 ...
 if ($mostrar=="S" && $proveedor != "A-00001100-2" && $codigo != "B")	
 {
	  ...
	  echo'<tr class="ColorTabla01"> 
		...
		$consulta = "SELECT distinct LOTE AS LOTE_A FROM SIPA_WEB.recepciones WHERE COD_PRODUCTO='1' AND COD_SUBPRODUCTO = '17' AND RUT_PRV = '$prov'";
		$consulta.="and  ((FECHA = '".$fecha."' and hora_entrada > '".$horaI."') or ( fecha= '".$Fecha2."' and hora_entrada < '".$horaF."'))";
		include("../principal/conectar_rec_web.php");
		$rs = mysqli_query($link, $consulta);
		while($row = mysqli_fetch_array($rs))
		{
			...
			$consulta.= " WHERE COD_PRODUCTO='1' AND COD_SUBPRODUCTO = '17' AND LOTE = $row[LOTE_A] AND RUT_PRV = '$prov'";
			$consulta.=" and ((fecha = '".$fecha."' and hora_entrada > '".$horaI."') or (fecha = '".$Fecha2."' and hora_entrada < '".$horaF."'))";
			$result = mysqli_query($link, $consulta);
			if($row1 = mysqli_fetch_array($result))
			{
				...
				$consulta_s = "SELECT SUM(PESO_NETO) AS peso_t FROM SIPA_WEB.recepciones WHERE LOTE = $row[LOTE_A] AND COD_PRODUCTO='1' AND COD_SUBPRODUCTO = 17 AND RUT_PRV = '$prov'";
				$consulta_s.=" and ((fecha = '".$fecha."' and hora_entrada > '".$horaI."') or (fecha = '".$Fecha2."' and hora_entrada < '".$horaF."'))";
				$result_s = mysqli_query($link, $consulta_s);
				...
				if($lote_ventana != '')
				{
					  ...
					  if($hornada != '')
					  {
						   $consulta_u = "SELECT SUM(unidades) AS unid, SUM(peso) AS peso, SUM(peso_origen) AS peso_origen FROM movimientos WHERE tipo_movimiento = 1  AND hornada = $hornada";
						   $consulta_u.=" and ((fecha_movimiento = '".$fecha."' and  hora > '".$horaI."') or (fecha_movimiento = '".Fecha2."' and hora < '".$horaF."'))";
						   include("../principal/conectar_sea_web.php"); 
						   $result_u = mysqli_query($link, $consulta_u);
