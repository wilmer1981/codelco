...
function Recarga(f)
{
	...
	f.action = "sea_con_stock_hornadas_res.php?recargapag=S&cmbproducto=" + f.cmbproducto.value + "&activar=" + chequeado;		
	f.submit();	
}
		...
		if (($activar == "S") and (($RadioTipoFecha == "A") or ($RadioTipoFecha == "D")) and ($RadioTipoCons == "H")) // Restos por Grupo.
		{
			...
			if ($recargapag == "S")
			{
				...
				while (list($clave,$valor) = each($array_hornadas))
				{
					...
					$consulta.= " AND hora between '".$FechaInicio2."' and '".$FechaTermino."'";
					//echo "1-trece".$consulta."</br>";;
					$rs1 = mysqli_query($link, $consulta);						
					if ($row1 = mysqli_fetch_array($rs1))
					{
						$tras_unidades = $row1["unidades"];
						$tras_peso = $row1["peso"];
						$consulta_sec = "select sum(unidades)as unidades,sum(peso) as peso from sea_web.restos_a_sec where tipo_movimiento = '1' and ";
						$consulta_sec.=" cod_producto = ".$arreglo[0];
						$consulta_sec.=" and hornada IN (".$hornadas.")";
						$consulta_sec.=" and fecha_movimiento between '".$FechaInicio."' and '".$FechaConsulta2."'";
						$rsec =  mysqli_query($link, $consulta_sec);
						//echo "l-trece2".$consulta_sec."</br>";
						if ($rows = mysqli_fetch_array($rsec))
						{
							$tras_unidades = $tras_unidades - $rows["unidades"];
							$tras_peso = $tras_peso  - $rows["peso"];
						}
						//echo '<td align="right">'.$row1["unidades"].'</td>';
						//echo '<td align="right">'.$row1["peso"].'</td>';
						echo '<td align="right">'.$tras_unidades.'</td>';
						echo '<td align="right">'.$tras_peso.'</td>';
						$TotalOtrosUnid = $TotalOtrosUnid + $tras_unidades;
						$TotalOtrosPeso = $TotalOtrosPeso + $tras_peso;
						
						$unid_aux = $unid_aux - $tras_unidades;						
						$peso_aux = $peso_aux - $tras_peso;					
					}
					else
					{
						echo '<td align="right">&nbsp;</td>';
...
//------------- agrego blister, catodos, despuntes
					...
					if ($row1 = mysqli_fetch_array($rs1))
					{
						// ver solo lo realmente embarcado
						$tras_unidades = $row1["unidades"];
						$tras_peso = $row1["peso"];
						$consulta_sec = "select sum(unidades) as unidades ,sum(peso) as peso from sea_web.restos_a_sec where tipo_movimiento = '1' and ";
						$consulta_sec.=" cod_producto = '".$arreglo[0]."' and cod_subproducto = '".$arreglo[1]."'";
						$consulta_sec.=" and hornada = '".$valor[0]."'";
						$consulta_sec.=" and fecha_movimiento between '".$FechaInicio."' and '".$FechaConsulta2."'";
						$rsec =  mysqli_query($link, $consulta_sec);
						if ($rows = mysqli_fetch_array($rsec))
						{
...
// agrego catodos despuntes
				...
				if ($row1 = mysqli_fetch_array($rs1))
				{
						...
						$tras_peso = $row1["peso"];
						$consulta_sec = "select sum(unidades) as unidades ,sum(peso) as peso  from sea_web.restos_a_sec where tipo_movimiento = '1' and ";
						$consulta_sec.=" cod_producto = '".$row["cod_producto"]."' and cod_subproducto = '".$row["cod_subproducto"]."'";
						$consulta_sec.=" and fecha_movimiento between '".$FechaInicio."' and '".$FechaConsulta2."'";
						$rsec =  mysqli_query($link, $consulta_sec);
						//echo $consulta_sec."</br>";
						if ($rows = mysqli_fetch_array($rsec))
						{
							$tras_unidades = $tras_unidades - $rows["unidades"];
							$tras_peso = $tras_peso  - $rows["peso"];
						...
						$TotalOtrosUnid = $TotalOtrosUnid + $tras_unidades;
						$TotalOtrosPeso = $TotalOtrosPeso + $tras_peso;
				  		$SubTotalOtrosUnid = $SubTotalOtrosUnid + $tras_unidades;
				  		$SubTotalOtrosPeso = $SubTotalOtrosPeso + $tras_peso;
						$unid_aux = $unid_aux - $tras_unidades;						
						$peso_aux = $peso_aux - $tras_peso;					
