...
if($Proceso == "G")
{
	...
	while (list($c,$v) = each($vector))
	{
		...
		$consulta.= " WHERE ano = ".$row[ano]." AND mes = ".$row[mes]." AND cod_producto = 19 AND hornada IN (".$v.")";
		...
		$consulta.= " WHERE tipo_movimiento IN (2,4,3) AND cod_producto = 19 AND hornada IN (".$v.")";
		//echo $consulta."<br>";
		...
		$consulta = $consulta." AND cod_subproducto = ".$row3["cod_subproducto"];
		...
		if ($unidades > 0)
		{
			...
			$insertar = $insertar." VALUES(4,19,".$row3["cod_subproducto"].",".$v.",0,'".$fecha_r."','".$Lado_Aux."','".$cmbgrupo."',";
			if($cmbgrupo=='8')
			{
    ...
    header("Location:sea_ing_restos_trasp_raf_res.php?".$valores); 
