...
function Eliminar(f)
{
	...
	/*
	Validacion de Eliminacion en forma descendente
	if(pos!=((f.radio.length)-1))
	{
			alert("Debe eliminar los movimiento de forma descendente");
		return;
	}
	*/
	arreglo = f.radio[pos].value.split('/');
...
<form name="frm1" action="" method="post">
	<?php echo '<input type="hidden" name="valprod" value="'.$cmbsubprod.'">' ?>
				...
				{					
					if ($radio1 == 1 or $radio1 == 3) //Para Anodos.
						//echo '<input name="txtstock" type="text" size="10" value="'.StockActual($cmbhornada,17,$cmbsubprod).'" readonly>';
						 echo '<input name="txtstock" type="text" size="10" value="'.StockActual($cmbhornada,$cmbproducto,$cmbsubprod, $link).'" readonly>';
					else					
					{
						//Produccion.						
						if ($tipo_prod == "H") //Para Resto H.M.
							echo '<input name="txtstock" type="text" size="10" value="'.StockRestoHM($cmbhornada,19,$cmbsubprod, $link).'" readonly>';
						else //Para Ctte.
							echo '<input name="txtstock" type="text" size="10" value="'.StockRestoCTTE($cmbhornada,19,$cmbsubprod, $link).'" readonly>';
					}								
				}
				else
				...
				{					
					...
					{						
						if ($tipo_prod == "H")	 //Para Resto H.M. 
						{
							...
							echo '<input name="txtpeso" type="text" size="10" value="'.PesoFaltante(19,$cmbsubprod,$cmbhornada,$link).'" readonly>';
						}
						else 
						{
							$Consulta = "SELECT * from sea_web.hornadas ";
	...
	if ($recargapag3 == "S")
	{
		...
		if ($radio1 == 1)				
		{
			if ($cmbsubprod==4 || $cmbsubprod==8)
			{
				...
				$consulta = $consulta." AND (tipo_movimiento *1)  in (1,2,44) AND hornada = '".$cmbhornada."' ";
				//$consulta = $consulta." ORDER BY tipo_movimiento ASC, fecha_movimiento ASC";
				$consulta = $consulta." ORDER BY fecha_movimiento ASC, tipo_movimiento ASC";
			}
			else
			{
				$consulta = "SELECT * FROM sea_web.movimientos WHERE trim(cod_producto) =  '".trim($cmbproducto)."' AND cod_subproducto = '".$cmbsubprod."' ";
				$consulta = $consulta." AND (tipo_movimiento *1)  in (1,2) AND hornada = '".$cmbhornada."' ";
			//	$consulta = $consulta." ORDER BY tipo_movimiento ASC, fecha_movimiento ASC";
				$consulta = $consulta." ORDER BY fecha_movimiento ASC, tipo_movimiento ASC";
			}
		...
		else if ($radio1 == 2) 
			{
				...
				$consulta = $consulta." AND (tipo_movimiento * 1) in (2,3) AND hornada = '".$cmbhornada."' ";
				//	$consulta = $consulta." ORDER BY tipo_movimiento ASC, fecha_movimiento ASC";
				$consulta = $consulta." ORDER BY fecha_movimiento ASC, tipo_movimiento ASC";
