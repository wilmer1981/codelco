...
$ns="http://localhost/Proyecto/WSChagre";
$server->configurewsdl('Web Service Codelco-Chagres',$ns);
$server->wsdl->schematargetnamespace=$ns;
$server->register('Autentifica',array('user' => 'xsd:string','pwd' => 'xsd:string','dato' => 'xsd:string'),array('return' => 'xsd:string'),$ns);
...
//return $data['GUIA_SGDD_1']['NUM_EMIGUIA'];
//$response = simplexml_load_string($data, 'SimpleXMLElement', LIBXML_NOCDATA);
	if(is_null($data['GUIA_SGDD_1']['NUM_EMIGUIA']) || (trim($data['GUIA_SGDD_1']['NUM_EMIGUIA'])==''))
	{
			$buffer= '<Retorno><NroGuia>'.$NroGuia.'</NroGuia>';
			$buffer.='<Estado>100</Estado>';
			$buffer.='<Descripcion>Xml No Valido En Estructura</Descripcion>';
			$buffer.='<Fecha>'.date('Y-m-d H:i:s').'</Fecha></Retorno>';
			$buffer2=str_replace('"',chr (34),str_replace('/',chr (47),str_replace('<',chr (60),str_replace('>',chr (62),($buffer)))));
			return new soapval('return', 'xsd:string',$buffer2); 
	...
	{	
			try
			{
					...
					$fecha=$data['GUIA_SGDD_1']['DTE_CREACION'];//1202011
					$ANULAGUIA=$data['GUIA_SGDD_1']['COMPLEMENTO_PLANILLA']['G_ANULADA'];//1202011	
					$FechaGuia=substr($fecha,0,4)."-".substr($fecha,4,2)."-".substr($fecha,6,2);;
				...
				//return $Consulta."<br>";;
					...
					//	return $Consulta."<br>";
						if(!$Fila2 = mysql_fetch_assoc($Resp2))
						{
							...
							$Consulta2 = "SELECT IFNULL(MAX(ciclo),0) AS ciclo FROM sea_web.relaciones WHERE cod_origen = '".$CODORIGEN."'";
							...
							$Consulta3.=" WHERE cod_origen ='".$CODORIGEN."' AND ciclo ='".$CicloAux."'";
							...
							if($Fila5 = mysql_fetch_assoc($Resp5))
							{
								...
								$Insertar.=" VALUES ('".$CODORIGEN."','".$LoteVentana."','".$LoteOrigen."','".$Hornada."','".trim($Marca)."','".$Ciclo."')";
								mysql_query($Insertar);
								$Insertar = "INSERT INTO sea_web.recepcion_externa_anglo(guia,cod_producto,cod_subproducto,lote_origen,lote_ventana,peso,peso_recep,piezas,piezas_recep,marca,fecha,estado,fecha_guia)";
								$Insertar.= " values('".$NroGuia."','".$PRODUCTOSEA."','".$SUBPRODUCTOSEA."','".$LoteOrigen."','".$LoteVentana."','".$CantPeso."',0,'".$CantPiezas."','0','".trim($Marca)."','".date('Y-m-d')."','','".$FechaGuia."')";
								mysql_query($Insertar);
								...
								$Insertar.="'".$RUTPROV."','".$CODMINA."','".$CODGRUPO."','".$PRODUCTO."','".$SUBPRODUCTO."','".$NroGuia."','FEPASA','M','','CARGADO DESDE INTERFAZ CHAGRES','A','0')";//'".$CantPeso."')";
								mysql_query($Insertar);
								
								
						//	echo "SIPA ".$Insertar."<br>";//	return $Insertar;
							}
							else
							{
								if($Fila2["guia"] <> $NroGuia)
								{
									...
									{ 
										...
										$Insertar.= "values('".$NroGuia."','".$PRODUCTOSEA."',".$SUBPRODUCTOSEA."','".$LoteOrigen."','".$LoteVentana."',";
					...
					while($Fila = mysql_fetch_assoc($Resp))
					{
						...
						{	
							...
							if($Fila3 = mysql_fetch_assoc($Resp3))
							{
								...
								$Insertar.="'".$RUTPROV."','".$CODMINA."','".$CODGRUPO."','".$PRODUCTO."','".$SUBPRODUCTO."','".$Fila["guia"]."','FEPASA','M','','','A')";
								mysql_query($Insertar);
							}
						}
					}
					...
					if($ANULAGUIA!=0)
					{	
						$ConsultaGuiasAnuladas="SELECT t1.hornada,t3.lote_ventana,t3.guia from movimientos t1 inner join relaciones t2 ";
						$ConsultaGuiasAnuladas.=" on t1.hornada=t2.hornada_ventana inner join recepcion_externa_anglo t3 on ";
						$ConsultaGuiasAnuladas.="t2.lote_ventana=t3.lote_ventana where t3.guia='".$ANULAGUIA."'";
						$Resp = mysql_query($ConsultaGuiasAnuladas);
						if(!$Fila = mysql_fetch_assoc($Resp))
						{
							$ElimiHornada="SELECT t2.hornada_ventana from sea_web.recepcion_externa_anglo t1 inner join sea_web.relaciones  t2 on t1.lote_ventana=t2.lote_ventana where t1.guia='".$ANULAGUIA."'";
							$RespHornada = mysql_query($ElimiHornada);
							while($FilaHornada = mysql_fetch_assoc($RespHornada))
							{
								$Eliminar="delete from sea_web.hornadas where hornada_ventana='".$FilaHornada[hornada_ventana]."'";
								mysql_query($Eliminar);	
							}		
							$Eliminar="delete from sea_web.relaciones where lote_ventana in(SELECT lote_ventana from sea_web.recepcion_externa_anglo where guia='".$ANULAGUIA."') ";
							mysql_query($Eliminar);
							$Eliminar="delete from sea_web.recepcion_externa_anglo where guia='".$ANULAGUIA."' ";
							mysql_query($Eliminar);
							$Eliminar="delete from sea_web.recepcion_externa_detalle_anglo  guia='".$ANULAGUIA."' ";
							mysql_query($Eliminar);
							$Eliminar="delete from sipa_web.recepciones where guia_despacho='".$ANULAGUIA."' ";
							mysql_query($Eliminar);
						}
						else
						{
								$Insertar="INSERT INTO guias_solicitud_anulacion(guia,fecha_hora,estado)";
								$Insertar.="values('".$ANULAGUIA."','".date('Y-m-d G:i:s')."','P')";
								mysql_query($Insertar);	
					...
					$EstadoSolicitud=1;
					$buffer= '<Retorno><NroGuia>'.$NroGuia.'</NroGuia>';
					$buffer.='<Estado>'.$EstadoSolicitud.'</Estado>';
					$buffer.='<Descripcion>Registro Ingresado Satisfactoriamente</Descripcion>';
					$buffer.='<Fecha>'.date('Y-m-d H:i:s').'</Fecha></Retorno>';
					$buffer2=str_replace('"',chr (34),str_replace('/',chr (47),str_replace('<',chr (60),str_replace('>',chr (62),($buffer)))));
					//return new soapval('return', 'xsd:string',$buffer2); 
					return $buffer2;
				...
				$buffer= '<Retorno><NroGuia>'.$NroGuia.'</NroGuia>';
				$buffer.='<Estado>'.$EstadoSolicitud.'</Estado>';
				$buffer.='<Descripcion>'.$e->getLine()."  - ".$e->getMessage().'</Descripcion>';
				$buffer.='<Fecha>'.date('Y-m-d H:i:s').'</Fecha></Retorno>';
				$buffer2=str_replace('"',chr (34),str_replace('/',chr (47),str_replace('<',chr (60),str_replace('>',chr (62),($buffer)))));
				return $buffer2;
				//	return new soapval('return', 'xsd:string',$buffer2); 
