...
function Recarga()
{
	...
	Frm.action="sec_consulta_pesaje_ie2.php?TipoIE="+TipoIE;
	Frm.submit();
		...
		<?php
			...
			switch ($TipoIE)
			{
				case "Normal":
					...
					$Consulta=$Consulta." left join sec_web.lote_catodo t6 on t1.cod_producto=t6.cod_producto ";
				...
				case "Virtual":
					...
					$Consulta="SELECT sum(t4.peso_paquetes) as peso,t6.descripcion as nombre_producto,t2.descripcion as nombre_subproducto ";
					$Consulta=$Consulta." from sec_web.instruccion_virtual t1";
					$Consulta=$Consulta." inner join sec_web.lote_catodo t3 on t1.corr_virtual= t3.corr_enm";
					$Consulta=$Consulta." inner join sec_web.paquete_catodo t4 on t3.cod_paquete= t4.cod_paquete and";
					$Consulta=$Consulta." t3.num_paquete=t4.num_paquete and t4.cod_estado='a'";
					$Consulta=$Consulta." left join proyecto_modernizacion.productos t6 on t1.cod_producto=t6.cod_producto ";
					$Consulta=$Consulta." left join proyecto_modernizacion.subproducto t2 on t1.cod_producto=t2.cod_producto and t1.cod_subproducto=t2.cod_subproducto ";
					$Consulta=$Consulta." where t1.estado='P' or t1.estado='T' group by t1.cod_producto,t1.cod_subproducto";
					$Resultado=mysqli_query($link, $Consulta);
					while ($Fila=mysqli_fetch_array($Resultado))
					{
