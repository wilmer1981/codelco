...
function Recarga1()
{
	...
	if (f.cmbmovimiento.value == -1)
		...
		f.action = "sec_ing_produccion_0809.php?recargapag1=S";
...
function Recarga2()
{
	...
	document.location = "sec_ing_produccion_0809.php?" + linea;
...
function AjustarPeso()
{
	...
	{	
		...
		f.action = "sec_ing_produccion01_0809.php?proceso=B12";
		f.submit();
	}
}
/****************/
...
*/	
	...
	if (f.checkpeso1.checked == true)
		linea = linea + "&pcontrol1=checked";
		else
		linea = linea + "&pcontrol1=";
		
	linea = linea + "&cmbbascula=" + f.cmbbascula.value;
	
	f.action = "sec_ing_produccion01_0809.php?proceso=B8" + linea;
	f.submit();
}
/***************/
function CreaVirtual()
{
	...
	f.action = "sec_ing_produccion01_0809.php?proceso=V" + linea;
	f.submit();	
}
function AgregaPeso()
{
	...
	f.action = "sec_ing_produccion01_0809.php?proceso=AP" + linea;
	f.submit();	
}
/***************/
	...
	if (ValidaCampos())
	{
		...
		if (f.cmbmovimiento.value == 3)
		{
			if(f.cmbcodpaq.value!='-1')
			{
				...
				//08-04 StrDatosEtiqueta=StrDatosEtiqueta+f.cmbcodpaq.options[f.cmbcodpaq.SELECTedIndex].text+"-"+f.txtnumpaq.value+"\r\n"+f.txtmarca.value+"\r\n"+""+"\r\n"+f.dia.value+"-"+f.mes.value+"-"+f.ano.value+" "+f.hh.value+":"+f.mm.value+"\r\n"+f.txtpeso.value+"\r\n"+""+"\r\n"+f.txtnommarca.value;
		...
		if ((f.cmbmovimiento.value == 3) && (f.cmbproducto.value == 57) && (f.cmbsubproducto.value == 12))
			f.action = "sec_ing_produccion01_0809.php?proceso=GL" + linea;
		else 
		//alert (linea);
			f.action = "sec_ing_produccion01_0809.php?proceso=G" + linea;
		f.submit();
	}	
}     
...
function Modificar()
{
	...
	if ((f.cmbmovimiento.value == 3) || (f.cmbmovimiento.value == 1))
	{
		...
		{
			alert("El Peso Del Paqute No Puede Ser Mayor Al Peso Faltante");
			return;
		}		
	...
	if ((f.cmbmovimiento.value == 3) && (f.cmbproducto.value == 57))
		f.action = "sec_ing_produccion01_0809.php?proceso=ML" + linea;
	else
		f.action = "sec_ing_produccion01_0809.php?proceso=M" + linea;
	f.submit();
...
function Eliminar()
{
	...
	//alert(linea);
		...
		f.action = "sec_ing_produccion01_0809.php?proceso=E" + linea;
		f.submit();					
...
function Buscar()
{
	...
	f.action = "sec_ing_produccion01_0809.php?proceso=B" + linea;
	f.submit();
}
/***************/
function BuscarIE()
{	
	...
	document.location = "sec_ing_produccion01_0809.php?proceso=B7" + linea;
}
/***************/
function BuscarIE_Lodo() //Solo para los Lodos.
{
	...
	document.location = "sec_ing_produccion01_0809.php?proceso=B9" + linea;
}
/***************/
function BusarSerieLodo()
{
	...
	f.action = "sec_ing_produccion01_0809.php?proceso=B11" + linea;
	f.submit();
}
/***************/
function VerDatos()
{	
	...
	switch (f.cmbmovimiento.value)
	{
		...
		case '2' :  window.open("sec_ing_produccion_popup2.php?"+linea,"","top=195,left=180,width=540,height=350,scrollbars=no,resizable=no");
					break;
		case '3' :	window.open("sec_ing_produccion_popup3_0809.php?"+linea,"","top=195,left=180,width=650,height=360,scrollbars=yes,resizable=no");
					break;
	}
}
/***************/
...
function Limpiar()
{
	document.location = "sec_ing_produccion_0809.php";	
}
/***************/
function Salir()
{		
...
function RecargaGrupo()
{
	...
	f.action = "sec_ing_produccion_0809.php?" + linea;
	f.submit();
}
/************/
...
function ValBascula()
{
	...
	linea = linea + "&cmbmovimiento=" + f.cmbmovimiento.value + "&cmbproducto="	+ f.cmbproducto.value + "&cmbsubproducto=" + f.cmbsubproducto.value;
	linea = linea + "&cmbbascula=" + f.cmbbascula.value;
	f.action = "sec_ing_produccion01_0809.php?proceso=BB" + linea;
	f.submit();
}
/****************/
function VeControl1()
...
function VeControl2()
{
	...
	linea = "recargapag1=S&recargapag2=S&recargapag3=S";
	linea = linea + "&cmbmovimiento=" + f.cmbmovimiento.value + "&cmbproducto=" + f.cmbproducto.value + "&cmbsubproducto=" + f.cmbsubproducto.value
	linea = linea + "&cmbbascula=" + f.cmbbascula.value + "&ano="+ f.ano.value + "&mes=" + f.mes.value +"&dia=" + f.dia.value;

	if (f.checkpeso.checked == true)
		linea = linea + "&peso_auto=checked";
	else 
		linea = linea + "&peso_auto=";
	window.open("sec_ing_produccion_popupPC.php?"+linea,"","top=195,left=180,width=540,height=350,scrollbars=no,resizable=no");	
}
</script>
</head>
...
<?php
	echo '<input name="TxtIp" type="hidden" value="'.$TxtIp.'">';
	echo '<input name="pcontroll" type="hidden" value="'.$pcontroll.'">';
	echo '<input name="pcontrol2" type="hidden" value="'.$pcontrol2.'">';
	echo '<input name="cmbnumero" type="hidden" value="'.$cbmnumero.'">';
	echo '<input name="txtfechaC" type="hidden" value="'.$fechaC.'">';
		...
		switch ($cmbmovimiento) {
			...
			case 3:
				if ($cmbproducto == '57') //Lodos
					...
					include("sec_ing_produccion_3_0809.php");	
				break;
		}	 
	}
?>  
