...
// PROCESO  1
	...
	if ($proceso == "B") //Busqueda de Recepciones.
	{
		...
		$linea.= "&peso_auto=".$peso_auto."&hh=".$hh."&mm=".$mm."&pcontrol1=".$pcontrol1."&cmbbascula=".$cmbbascula;
		header("Location:sec_ing_produccion_0809.php?".$linea);
	}
...
// PROCESO 7	
	...
	if ($proceso == "B7") //Busca I.E en programa_enami � programa_codelco � Virtual.
	{
		...
		$linea.= "&pcontrol1=".$pcontrol1."&cmbbascula=".$cmbbascula;
...
// PROCESO 15
	...
	if ($proceso == "G")
	{
		...
		{		
			...
			else
			{
				...
				$linea = "recargapag1=S&recargapag2=S&recargapag3=S&cmbmovimiento=".$cmbmovimiento."&cmbproducto=".$cmbproducto;
				$linea.= "&cmbsubproducto=".$cmbsubproducto;
				$linea.= "&peso_auto=".$peso_auto."&cmbbascula=".$cmbbascula;				
				header("Location:sec_ing_produccion_0809.php?".$linea);				
			}
		}
	...
	//echo "RRR".$consulta;
		...
		{						
			...
			{
				$existe = 0;
				$consulta = "SELECT * FROM sec_web.sec_control_pesada";
				$consulta.= " WHERE cod_paquete = '".$cod_paq[$cmbcodpaq]."' AND num_paquete = '".$txtnumpaq."'";
				...
				if ($existe==0)
				{
						$inserta ="insert into sec_web.sec_control_pesada (cod_paquete,num_paquete, fecha, bascula5,bascula6,operador) ";
						$inserta.=" values ('".$cod_paq[$cmbcodpaq]."','".$txtnumpaq."','".$fecha."',";
						if ($cmbbascula==5)
							$inserta.="'".$txtpeso."',0,'".$CookieRut."')";
						if ($cmbbascula==6)
							$inserta.="0,'".$txtpeso."','".$CookieRut."')";
...
if ($proceso == "E")
	{	
		...
		if ($cmbmovimiento == "3") //PAQUETES.
		{
			...
			//aca para control de romanas
			$consulta="SELECT * from sec_web.sec_control_pesada where cod_paquete = '".$cod_paq[$cmbcodpaq]."' and ";
			$consulta.=" num_paquete = '".$txtnumpaq."' and fecha = '".$fecha."'";
			$resp1=mysqli_query($link, $consulta);
			if ($fila1=mysqli_fetch_array($resp1))
			{
				$elimina="delete  from sec_web.sec_control_pesada where cod_paquete = '".$cod_paq[$cmbcodpaq]."' and ";
				$elimina.=" num_paquete = '".$txtnumpaq."' and fecha = '".$fecha."'";
				mysqli_query($link, $elimina);
			}
						
			$linea = "recargapag1=S&recargapag2=S&recargapag3=S&cmbmovimiento=".$cmbmovimiento."&cmbproducto=".$cmbproducto;
			$linea.= "&cmbsubproducto=".$cmbsubproducto;
			$linea.= "&peso_auto=checked&cmbbascula=".$cmbbascula;			
			header("Location:sec_ing_produccion_0809.php?".$linea);			
		}
	}
// PROCESO 18
...
// PROCESO 21
	...
	// nuevo
	if ($proceso=="BB") //Valida el Lote.
	{
		$ExisBas="N";
    	$IP_USUARIO = $REMOTE_ADDR;
		$consulta = "SELECT * FROM sec_web.sec_ip_bascula_pc";
		$consulta.= " WHERE iden_ip = '".$IP_USUARIO."' AND iden_bascula = '".$cmbbascula."'";
		$resu= mysqli_query($link, $consulta);
		if ($fila = mysqli_fetch_array($resu)) 
		{
			$ExisBas = "S";
		}		
		if ($ExisBas=="N")	
		{
			$mensaje = "Bascula no Corresponde a PC";
			echo '<script language="JavaScript">';
			echo 'alert("'.$mensaje.'");';
			echo 'window.history.back()';
			echo '</script>';
    ...
    // Graba control de Peso
	if ($proceso=="GC2")
	{
		$consulta="SELECT * from sec_web.sec_control_pesada where cod_paquete = '".$TxtCodigo."' and num_paquete = '".$TxtPaquete."' and ";
		$consulta.=" fecha = '".$TxtFecha."'";
		$respta = mysqli_query($link, $consulta);
		if ($Fila=mysqli_fetch_array($respta))
		{
			$actualiza="UPDATE sec_web.sec_control_pesada set ";
			if ($cmbbascula==5)
				$actualiza.=" bascula5 = '".$TxtPesada."' ";
				else
				$actualiza.=" bascula6 = '".$TxtPesada."' ";
			$actualiza.=" where cod_paquete = '".$TxtCodigo."' and num_paquete = '".$TxtPaquete."' and fecha = '".$TxtFecha."'";
			mysqli_query($link, $actualiza);
		}
		$linea ="cmbbascula=".$cmbbascula;
		header("Location:sec_ing_produccion_popupPC.php?".$linea);
