...
?>
	...
	$Color = "";
	$TotalPeso = 0;	
	$Consulta = "SELECT ifnull(t2.cod_bulto,'') as cod_bulto, ifnull(t2.num_bulto,'0') as num_bulto, sum(t1.peso_paquetes) as peso, t2.corr_enm ";
	...
	$Consulta.= " where (year(t1.fecha_creacion_paquete) = year('".$FechaAux."') or";
	$Consulta.= " year(t1.fecha_creacion_paquete) = year('".$Fecha_menor."'))";
	...
	while ($Fila = mysqli_fetch_array($Respuesta))
	{
		...
		else
		{
			...
			$Consulta.= " and (year(fecha_creacion_paquete) = year('".$FechaAux."') or";
			$Consulta.= " year(fecha_creacion_paquete) = year('".$Fecha_menor."'))";
		...
		{
			//CREA CERTIFICADO VIRTUAL			
			CertifVirtual($Fila["cod_bulto"],$Fila["num_bulto"]);			
