...
{	
	...
	f.action = "sec_ing_produccion_popup3.php?mostrar=S";
	f.submit();
}
/***************/
...
//&agrega_paq=S&&hh=15&mm=25&&listar_ie=V&recargapag4=S
	...
	linea = 'from=S&recargapag1=S&recargapag2=S&recargapag3=S&recargapag4=S&cmbmovimiento=3&cmbproducto=' + f.cmbproducto.value + '&cmbsubproducto=' + f.cmbsubproducto.value; 
	...
	if(f.cmbsubproducto.value == 16 || f.cmbsubproducto.value == 17 || f.cmbsubproducto.value ==49 || f.cmbsubproducto.value ==57)
		linea = linea + '&SA_C_STD2=' + vector[14];
	window.opener.document.frm1.action = "sec_ing_produccion.php?" + linea;
...
<table width="500" height="25" border="0" cellspacing="0" cellpadding="3" class="TablaInterior">
 ...
 <div style="position:absolute; left: 15px; top: 50px;" id="div1">  
  <table width="500" height="25" border="0" cellspacing="0" cellpadding="0" class="TablaInterior">
    <tr>
      <td width="154">Fecha Recepci&oacute;n</td>
...
<?php 
	...
	echo '<div style="position:absolute; left: 15px; top: 85px; width:518px; height:200px; OVERFLOW: auto;" id="div2">';
	echo '<table width="500" height="25" border="0" cellspacing="0" cellpadding="0" class="ColorTabla01">';
	echo '<tr>';
	echo '<td width="100" align="center">Cod. Serie</td>';
	echo '<td width="100" align="center">N&deg; Serie</td>';
	...
	if ($cmbproducto == 57)	
	{
		...
		echo '<td width="100" align="center">Peso Tara</td>';		
	...
	echo '<div style="position:absolute; left: 15px; top: 110px; width:518px; height:200px; OVERFLOW: auto;" id="div5">';
	...
	else
	{
		...
		$consulta.= " t1.unidades, t1.peso_neto  AS peso_paquetes, t1.peso_bruto, t1.peso_tara,nro_solicitud";
	...
	while ($row = mysqli_fetch_array($rs))
	{
		...
		if ($row["cod_estado"] == 'a')
		{
			...
			if($cmbsubproducto =='16' || $cmbsubproducto =='17' || $cmbsubproducto =='49' || $cmbsubproducto =='57')	
				$linea.= "/".$row["nro_solicitud"];
				//echo "DD".$linea;
				echo '<td width="100"><input type="radio" name="radiobutton" value="'.$linea.'" onClick="Chequear(this,\''.$row["cod_estado"].'\')">'.$row["cod_paquete"].'</td>';
		}
		else
			echo '<td width="100"><input type="radio" name="radiobutton" value="" onClick="Chequear(this,\''.$row["cod_estado"].'\')">'.$row["cod_paquete"].'</td>';
		...
		echo '<td width="100" align="center">'.$row["peso_paquetes"].'</td>';
		
		if ($cmbproducto == 57)
		{
			echo '<td width="100" align="center">'.$row["peso_tara"].'</td>';		
			echo '<td width="100" align="center">'.$row[peso_bruto].'</td>';					
		}
	...
	if ($cmbproducto == 18)
	{
		...
		echo '<td width="100" align="center">'.$total_neto.'</td>';
	}
	if ($cmbproducto == 48)
		echo '<td width="100" align="center">'.$total_neto.'</td>';
	...
	{
		echo '<td width="100" align="center">'.$total_neto.'</td>';
		echo '<td width="100" align="center">'.$total_tara.'</td>';	
		echo '<td width="100" align="center">'.$total_bruto.'</td>';		
	}
