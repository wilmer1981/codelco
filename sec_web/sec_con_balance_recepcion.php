...
<form name="frmPrincipal" action="" method="post">
  <table width="523" border="1" cellpadding="3" cellspacing="0" class="TablaInterior">
    <tr align="center"> 
      <td height="30" colspan="2"><strong> TIPO DE MOVIMIENTO RECEPCION</strong></td>
    </tr>
    <tr> 
...
?>
	...
	if ($Color == "")
		...
		$Color = "";		
	$Consulta = "SELECT STRAIGHT_JOIN ifnull(t2.cod_bulto,'') as cod_bulto, ifnull(t2.num_bulto,'0') as num_bulto, year(t2.fecha_creacion_lote) as AnoLote, sum(t1.peso_paquetes) as peso";
	...
	$Consulta.= " and t1.cod_paquete = '".$MesConsulta."' and t1.fecha_creacion_paquete = t2.fecha_creacion_paquete";
	$Consulta.= " and t1.cod_producto = '".$Producto."' and t1.cod_subproducto = '".$SubProducto."'";
	$Consulta.= " group by t2.cod_bulto,  t2.num_bulto, year(t2.fecha_creacion_lote)";
	$Respuesta = mysqli_query($link, $Consulta);
	//echo "otro".$Consulta."<br>";
	while ($Fila = mysqli_fetch_array($Respuesta))
	{
		...
		$Consulta.=" and year(fecha_creacion_lote) = '".$Fila["AnoLote"]."'";
		//if ($Fila["cod_bulto"]=='G' && $Fila["num_bulto"]=='2200')
		//	echo $Consulta;
		...
		$AnoLote = substr($FechaCreacionLote,0,4);
...
/**/	$Consulta.= " and year(t1.fecha_hora)= '".$AnoLote."'";
		...
		{
			echo "<td align='center' bgcolor='yellow'><a href=\"JavaScript:Informacion('".$Fila["cod_bulto"]."','".$Fila["num_bulto"]."','".$Fila["AnoLote"]."');\">";				
			echo $NumCertificado."</a></td>\n";						
		}
		else
		{
			echo "<td align='center'><a href=\"JavaScript:Informacion('".$Fila["cod_bulto"]."','".$Fila["num_bulto"]."');\">";
