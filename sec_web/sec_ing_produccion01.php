<?php
	...
	$txtpesobolsa  = isset($_REQUEST["txtpesobolsa"])?$_REQUEST["txtpesobolsa"]:0;
	...
	$pesotara      = isset($_REQUEST["pesotara"])?$_REQUEST["pesotara"]:0;
	$pesoneto   = isset($_REQUEST["pesoneto"])?$_REQUEST["pesoneto"]:0;	
	...
	$pesoprog    = isset($_REQUEST["pesoprog"])?$_REQUEST["pesoprog"]:0;
...
switch($proceso)
{
	case "ELICUBA"://ELIMINA CUBAS DEL GRUPO SELECCIONADO EN POPUP DE MODIFICACION
		...
		foreach($Datos as $Clave => $Valor)
		{
			...
			$lado           = $Datos2[2];
			$grupo          = $Datos2[0];
			$cmbproducto    = $Datos2[4];
			$cmbsubproducto = $Datos2[5];
	
			if ($grupo == "99")
			{
				$Eliminar="delete FROM sec_web.produccion_desc_normal";
				$Eliminar.= " WHERE cod_grupo = '".$Datos2[0]."' AND cod_lado = '".$Datos2[2]."' and cod_cuba='".$Datos2[1]."'";
	...
	case "TERMINAIECOM"://ACTUALIZA ESTADO TERMINADO
		$Datos=explode('~~',$Valores);
		$IE=$Datos[0];	 	
...
//echo "Proceso ".$proceso."<br>";
	...
	if ($proceso == "B7") //Busca I.E en programa_enami  programa_codelco  Virtual.
	{
		...
		{		
			...
			{	
				...
				if ($cmbmovimiento == "3")
				{
					if($cmbsubproducto=='16' || $cmbsubproducto=='17' || $cmbsubproducto=='49')//INGRESAMOS SOLICITUD DE ANALISIS SOLO A CATODOS EW STD 1,2 Y 3.
					{
						$linea.= "&SA_C_STD=".$row4["nro_solicitud"];					
					}
				}	
	...
	{	
		...
		//ULTIMA MODIFICACION REFERNTE A SOLICITUD DE ANALISIS PARA CATODOS EW STD 1,2 Y 3.
		if($cmbmovimiento== 3)
		{
			if($cmbsubproducto=='16' || $cmbsubproducto=='17' || $cmbsubproducto=='49')//INGRESAMOS SOLICITUD DE ANALISIS SOLO A CATODOS EW STD 1,2 Y 3.
				$linea.= "&SA_C_STD=".$NroSolAnalisis;
		}			
		header("Location:sec_ing_produccion.php?".$linea);				
	}
	...
	if ($proceso == "G")
	{
		...
		echo "<br>pesoprog:".$pesoprog;
		echo "<br>peso origen:".$txtpeso;
		echo "<br>cmbmovimiento:".$cmbmovimiento;
		echo "<br>cmbproducto:".$cmbproducto;
		echo "<br>cmbsubproducto:".$cmbsubproducto;
		//exit();
		$fecha = $ano.'-'.$mes.'-'.$dia;
		...
		{	
			session_start();
			$_SESSION['ses_promedio'] = 0;
			$_SESSION['ses_quitar']   = 0;
			$_SESSION['ses_resto']    = 0;
			$_SESSION['ses_numero']   = 0;
			echo "<br>Entró a RECEPCION";			
            //exit();			
			$linea = "recargapag1=S&recargapag2=S&recargapag3=S&cmbmovimiento=".$cmbmovimiento."&cmbproducto=".$cmbproducto;			
			...
			else
			{
				...
				{ 
                    echo "<br>Entrooooo a ELSE 1 ";	
					//echo "<br>Mensaje:".$mensaje;
					//exit();					
					//Consulta si hay un paquete abierto el ao anterior, con este codigo.
					$consulta = "SELECT * FROM sec_web.paquete_catodo";
					//$consulta.= " WHERE cod_paquete = '".$cod_paq[$cmbcodigo]."' AND num_paquete = '".$txtnumero."'";					
					$consulta.= " WHERE cod_paquete = '".$cod_paq[$cmbcodpaq]."' AND num_paquete = '".$txtnumpaq."'";
					$consulta.= " and cod_producto ='".$cmbproducto."' and cod_subproducto = '".$cmbsubproducto."'";
					$consulta.= " AND cod_estado = 'a' AND YEAR(fecha_creacion_paquete) = YEAR(SUBDATE(NOW(), INTERVAL 1 YEAR))";
					echo "<br>PPPPP".$consulta;
					$rs = mysqli_query($link, $consulta);
					if ($row = mysqli_fetch_array($rs))
					{
					...
					else
					{
						...
						//$ses_numero = $txtpaquete;
						*/
						echo "<br>Entrooooo a ELSE 2 ";
						echo "<br>txtpaquete = ".$txtpaquete;						
						//exit();
						if($txtpaquete>0){
							$ses_promedio = ($txtzuncho / $txtpaquete);	
						}else{
						...
						echo "<br>---------------------------------";
						echo "<br>ses_promedio = ".$_SESSION['ses_promedio'];
						echo "<br>ses_quitar   = ".$_SESSION['ses_quitar'];
						echo "<br>ses_resto    = ".$_SESSION['ses_resto'];
						echo "<br>ses_numero   = ".$_SESSION['ses_numero'];
						echo "<br>---------------------------------<br>";
						//exit();
						...
						echo "<br>row2[peso]: ".$row2["peso"];
						echo "<br>txtpesoprog: ".$txtpesoprog;
						//exit();
						
						if ($row2["peso"] > $txtpesoprog) 
						{	
                           echo "<br>peso > txtpesoprog";					
		...
		{						
				...
				{	
					...
					else
					{
						...
						else
						{
							...
							{
								$actualizar = "UPDATE sec_web.paquete_catodo SET nro_solicitud = '".$NroSolAnalisis."'";
								$actualizar.= " WHERE cod_existencia = '05' AND cod_paquete = '".$cod_paq[$cmbcodpaq]."'";
								$actualizar.= " AND num_paquete = '".$txtnumpaq."' and fecha_creacion_paquete='".$fecha."' ";
								$actualizar.= " AND cod_producto='".$cmbproducto."' and cod_subproducto='".$cmbsubproducto."' ";
						...
						else
						{
							...
							$insertar.= " VALUES ('".$cod_paq[$cmbcodlote]."','".$txtnumlote."','".$cod_paq[$cmbcodpaq]."','".$txtnumpaq."','".$fecha_lote."','".$txtmarca."','".$cmbinstruccion."','a','P','".$cliente."', '".$fecha."','0')";
							mysqli_query($link, $insertar);
			...
			//{
				...
				if ($mensaje=="")
				{
					...
					$linea.= "&agrega_paq=N"; 
			if($cmbsubproducto=='16' || $cmbsubproducto=='17' || $cmbsubproducto=='49' || $cmbsubproducto=='57')//INGRESAMOS SOLICITUD DE ANALISIS SOLO A CATODOS EW STD 1,2 Y 3.
				$linea.= "&SA_C_STD=".$NroSolAnalisis;
			//}
			header("Location:sec_ing_produccion.php?".$linea);
		}				
	...
	if ($proceso == "M")
	{
		...
		{
			$consulta = "SELECT t2.peso_paquetes FROM sec_web.lote_catodo AS t1";
			$consulta.= " INNER JOIN sec_web.paquete_catodo AS t2";
			$consulta.= " ON t1.cod_paquete = t2.cod_paquete AND t1.num_paquete = t2.num_paquete";
			$consulta.= " AND t1.cod_estado = t2.cod_estado AND t1.fecha_creacion_paquete = t2.fecha_creacion_paquete ";
			...
			//echo ('MF Retorno UPDATE: ' . mysql_error()) ." <br>";
			...
			{
				$actualizar = "UPDATE sec_web.lote_catodo SET nro_solicitud = '".$NroSolAnalisis."'";
				$actualizar.= " WHERE cod_bulto = '".$cod_paq[$cmbcodlote]."' AND num_bulto = '".$txtnumlote."'";
				$actualizar.= " AND corr_enm = '".$cmbinstruccion."'";
				//echo $actualizar."<br>";					
				mysqli_query($link, $actualizar);
...
{
	require "../principal/includes/class.phpmailer.php";	
	
	$Asunto='Modificacin de Peso Paquete '.$CodPaquete."-".$Paquete.', Nmero de Lote: '.$CodPaquete."-".$Lote.'';
	$Titulo='Sec_web - Sistema de Catodos';
	...
	if($Producto=='64')
		$NomProd="SALES";
	if($Producto=='48')
		$NomProd="DESPUNTES Y LAMINAS";
	if($Producto=='57')
		$NomProd="BARROS REFINERIA";
	if($Producto=='66')
		$NomProd="OTROS PESAJES";
	if($Producto=='19')
		$NomProd="RESTOS ANODOS";
	if($Producto=='17')
		$NomProd="ANODOS";
	...
	if($Fila=mysqli_fetch_array($Resul))
	{
		$Mensaje='<table width="90%" align="center"  border="0" cellpadding="0"  cellspacing="0">';
			$Mensaje.='<tr>';
			  $Mensaje.='<td height="15"><font face="Arial, Helvetica, sans-serif" size="2"><font size="2">Estimado Usuario:</font></font></td>';	
			$Mensaje.='</tr>';
			$Mensaje.='<tr>';
			  $Mensaje.='<td height="15"><font face="Arial, Helvetica, sans-serif" size="2"><font size="2">Para su informacin se ha modificado el peso del siguiente Paquete.</font></font></td>';	
			$Mensaje.='</tr>';
		$Mensaje.='</table><br>';
			 
		$Mensaje.='<table width="90%" align="center"  border="0" cellpadding="0"  cellspacing="0">';
			$Mensaje.='<tr>';
			  $Mensaje.='<td><table width="100%" border="0" cellpadding="3" cellspacing="0" >';
					$Mensaje.='<tr>';
					  $Mensaje.='<td height="27" style="FONT-WEIGHT: bolder; FONT-SIZE: 10px; background: #CEC18D; COLOR:#804000; FONT-FAMILY:Arial; border-top:1px;border-top-style:solid;border-top-color:#FFFFFF;border-bottom:1px;border-bottom-style:solid;border-bottom-color:#FFFFFF;border-left:1px;border-left-style:solid;border-left-color:#FFFFFF;border-right:1px;border-right-style:solid;border-right-color:#FFFFFF;">Paquete</td>';
					  $Mensaje.='<td height="27"><font size="2">'.$CodPaquete." - ".$Paquete.'</font></td>';
					$Mensaje.='</tr>';
					$Mensaje.='<tr>';
					  $Mensaje.='<td style="FONT-WEIGHT: bolder; FONT-SIZE: 10px; background: #CEC18D; COLOR:#804000; FONT-FAMILY:Arial; border-top:1px;border-top-style:solid;border-top-color:#FFFFFF;border-bottom:1px;border-bottom-style:solid;border-bottom-color:#FFFFFF;border-left:1px;border-left-style:solid;border-left-color:#FFFFFF;border-right:1px;border-right-style:solid;border-right-color:#FFFFFF;">Peso</td>';
					  $Mensaje.='<td><font size="2">'.$Peso.'</font></td>';
					$Mensaje.='</tr>';
					$Mensaje.='<tr>';
					  $Mensaje.='<td style="FONT-WEIGHT: bolder; FONT-SIZE: 10px; background: #CEC18D; COLOR:#804000; FONT-FAMILY:Arial; border-top:1px;border-top-style:solid;border-top-color:#FFFFFF;border-bottom:1px;border-bottom-style:solid;border-bottom-color:#FFFFFF;border-left:1px;border-left-style:solid;border-left-color:#FFFFFF;border-right:1px;border-right-style:solid;border-right-color:#FFFFFF;">Lote</td>';
					  $Mensaje.='<td><font size="2">'.$CodPaquete." - ".$Lote.'</font></td>';
					$Mensaje.='</tr>';
					$Mensaje.='<tr>';
					  $Mensaje.='<td width="22%" style="FONT-WEIGHT: bolder; FONT-SIZE: 10px; background: #CEC18D; COLOR:#804000; FONT-FAMILY:Arial; border-top:1px;border-top-style:solid;border-top-color:#FFFFFF;border-bottom:1px;border-bottom-style:solid;border-bottom-color:#FFFFFF;border-left:1px;border-left-style:solid;border-left-color:#FFFFFF;border-right:1px;border-right-style:solid;border-right-color:#FFFFFF;">Instruccin de Embarque</td>';
					  $Mensaje.='<td width="78%"><font size="2">'.$Instruccion.'</font></td>';
					$Mensaje.='</tr>';
					$Mensaje.='<tr>';
					  $Mensaje.='<td width="22%" style="FONT-WEIGHT: bolder; FONT-SIZE: 10px; background: #CEC18D; COLOR:#804000; FONT-FAMILY:Arial; border-top:1px;border-top-style:solid;border-top-color:#FFFFFF;border-bottom:1px;border-bottom-style:solid;border-bottom-color:#FFFFFF;border-left:1px;border-left-style:solid;border-left-color:#FFFFFF;border-right:1px;border-right-style:solid;border-right-color:#FFFFFF;">Producto/SubProducto</td>';
					  $Mensaje.='<td width="78%"><font size="2">'.$NomProd." / ".$NomSubProducto.'</font></td>';
					$Mensaje.='</tr>';
				   $Mensaje.='</table>';
				 $Mensaje.='<p>&nbsp;</p></td>';
			 $Mensaje.='</tr>';
		...
		$cuerpoMsj.= '</td></tr>';
		$cuerpoMsj.= '</table>';
		$cuerpoMsj.= '<table  width="90%"  border="0" align="center">';
		$cuerpoMsj.= '<tr><td><font size="2">';
		$cuerpoMsj.="(*) Este Correo ha sido Generado en forma automatica, por favor no responda este mensaje. Gracias.";
		$cuerpoMsj.= "<br>";
		$cuerpoMsj.="Servicio Automatico de Sistema Catodos";
		$cuerpoMsj.= "<br>";
		$cuerpoMsj.= '</font></td></tr>';
		$cuerpoMsj.= '</table>';
		$cuerpoMsj.= '</body></html>';
		...
		$mail->SMTPAuth=true;
		$mail->FromName = "Sec_web - Sistema Catodos";
		$mail->Subject = $Asunto;
		$mail->Body=$cuerpoMsj;
		$mail->IsHTML(true);
		$mail->AltBody =str_replace('<br>','\n',$cuerpoMsj);
		$mail->AddAddress($Fila["nombre_subclase"]);	
		$mail->Timeout=120;
		//$mail->AddAttachment($Doc,$Doc);
		//echo "nombre".$Fila["nombre_subclase"];
		$exito = $mail->Send();
		$intentos=1; 
