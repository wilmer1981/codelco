...
<br>
<table width="476" border="1" align="center" cellpadding="2" cellspacing="0">
  <tr align="center" class="ColorTabla01"> 
    <td width="124" rowspan="2">SEMANA</td>
    <td width="124" rowspan="2">PESO</td>
    <td colspan="3" align="center">LEYES</td>
    <!--<td colspan="3" align="center">FINO</td>-->
  </tr>
  <tr class="ColorTabla01"> 
    <td width="69" height="25" align="center">Cu</td>
    <td width="68" align="center">Ag</td>
  ...
  <?php
	...
	for ($i=1;$i<=4;$i++)
	{
		...
		{			
			...
			$FinoAg = 0;
			$FinoAu = 0;
			$PesoSemana = $FilaAux["peso"];
			$PesoTotal = $PesoTotal +$PesoSemana;
			...
			$FinoCu = $FinoCu + ($PesoSemana * 99.99); // Cu							
			...
			echo "<td align='right'>".number_format($PesoSemana,0,",",".")."</td>\n";
			if ($FinoCu > 0 && $PesoSemana > 0)
			{					
				echo "<td align='right'>".substr(number_format(($FinoCu/$PesoSemana),4,",","."),0,5)."</td>\n";
				$FinoCu = $FinoCu / 100;
			}
			else
			{
			...
			if ($FinoAg > 0 && $PesoSemana > 0)					
			{
				echo "<td align='right'>".number_format(($FinoAg/$PesoSemana),2,",",".")."</td>\n";
				$FinoAg = $FinoAg / 1000;
			}
			else
			{
			...
			if ($FinoAu > 0 && $PesoSemana > 0)					
			{
				echo "<td align='right'>".number_format(($FinoAu/$PesoSemana),2,",",".")."</td>\n";
				$FinoAu = $FinoAu / 1000;
			}
			else
			{
...
<table width="650" border="1" align="center" cellpadding="2" cellspacing="0">
  ...
  <?php
  	$Cu = ($TotalFinoCu/$PesoTotal) * 100;
	$Ag = ($TotalFinoAg/$PesoTotal) * 1000;
	$Au = ($TotalFinoAu/$PesoTotal) * 1000;
	$FechaInicio = $Ano."-".$Mes."-01";
	$FechaTermino = $Ano."-".$Mes."-31";	
	$AnoConsulta = $Ano; 
	$MesConsulta = $Mes;
