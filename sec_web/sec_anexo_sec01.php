	...
	switch ($Proceso)
	{
		case "G":
			...
			if ($Fila["valor"] == "0")
			{
				...
				$Insertar.= " values('3','".$Ano."','".$Mes."','".date("Y-m-d H:i:s")."','1', 'C', '".$CookieRut."')";
				mysqli_query($link, $Insertar);
				//ACTUALIZO REGISTRO EN TABLA EXISTENCIA NODO
				$Actualizar = "UPDATE sec_web.existencia_nodo ";
				$Actualizar.= " set bloqueado = '1' ";
				$Actualizar.= " where ano = '".$Ano."' ";
				$Actualizar.= " and mes = '".$Mes."' ";
				mysqli_query($link, $Actualizar);
				header("location:sec_anexo_sec.php?Mostrar=S&CodSistema=".$CodSistema."&Ano=".$Ano."&Mes=".$Mes."");
			}			
			break;			
		case "AM":
			//VALIDA PASSWORD
			...
			else
			{
				//INSERTA REGISTRO EN TABLA CIERRE MES
				$Insertar = "insert into proyecto_modernizacion.cierre_mes (cod_sistema, ano, mes, fecha_cierre, cod_bloqueo, estado, rut_funcionario) ";
				$Insertar.= " values('3','".$Ano."','".$Mes."','".date("Y-m-d H:i:s")."','1', 'A', '".$CookieRut."')";
				mysqli_query($link, $Insertar);
				//ACTUALIZO REGISTRO EN TABLA EXISTENCIA NODO
				$Actualizar = "UPDATE sec_web.existencia_nodo ";
				...
				echo "window.opener.document.frmPrincipal.action='sec_anexo_sec.php?Mostrar=S&CodSistema=".$CodSistema."&Ano=".$Ano."&Mes=".$Mes."';";
