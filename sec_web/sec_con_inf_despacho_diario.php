...
<input name="FechaTermino" type ="hidden"    value="<?php echo  $FechaTermino ?>">


  <table width="850" border="0" align="center" cellpadding="2" cellspacing="1" class="TablaInterior">
    <tr> 
      <td width="86"><strong>Fecha Inicio:</strong> </td>
      ...
      <td width="119"><strong>Fecha Termino:</strong></td>
	...
	//echo $Consulta;
		...
		{	
			...
			if ($Fila3 = mysqli_fetch_array($Respuesta3))
			{
				...
				/*$Consulta="SELECT distinct patente_guia,t1.corr_enm";
				$Consulta.=" from sec_web.guia_despacho_emb t1, sec_web.embarque_ventana  t2";
				$Consulta.=" where fecha_guia = '".$FechaInicio."' and t1.cod_estado <>'A'";
				$Consulta.=" and t2.cod_puerto != '' and t2.tipo_embarque != 'T' and t1.corr_enm = t2.corr_enm";
				$Consulta." and t1.corr_enm <9999 and t1.num_envio = t2.num_envio";
				$Consulta.=" group by patente_guia order by corr_enm";
				
				
			/*esto estaba con comentario 	$Consulta= " SELECT distinct patente_guia,corr_enm";
				$Consulta.=" from sec_web.guia_despacho_emb";
				$Consulta.=" where fecha_guia = '".$FechaInicio."'"; 
				$Consulta.=" and cod_estado <>'A'  group by patente_guia order by corr_enm";*/
				
				/*echo "FF".$Consulta;
				$Resp20 = mysqli_query($link, $Consulta);
				while ($Fpatente = mysqli_fetch_array($Resp20))
				{
					if ($Fpatente["corr_enm"]==$Fila3["corr_codelco"])
					{
						$pate = $pate + 1;
