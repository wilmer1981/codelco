...
var valor = valida_valores();
   ...
   {
      f.action="sea_stock_anodos_piso_raf01.php?Proceso=G&cmbsubproducto="+valor; 
      f.submit();
   }		
}
...
f.action="sea_stock_anodos_piso_raf.php?Proceso=B";
f.submit();
...
//			echo '<option value="-1">'.$consulta.'</option>';
			...
			<?php
				...
				if($codigo == 16 || $codigo == 18 || $codigo == 48)
				{
					...
					if($codigo == 18)
					{
						$Consulta="select * from proyecto_modernizacion.subproducto where cod_producto = 18 and cod_subproducto in(2,4,5,6,8,9,10)"; 
						$rs = mysqli_query($link, $Consulta);
						while ($Fila=mysqli_fetch_array($rs))
						{
					...
					if($codigo == 48)
					{
						$Consulta="select * from proyecto_modernizacion.subproducto where cod_producto = 48 and cod_subproducto in(2,3,7)"; 
						$rs = mysqli_query($link, $Consulta);
						while ($Fila=mysqli_fetch_array($rs))
						{
