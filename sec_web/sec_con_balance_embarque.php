...
<form name="frmPrincipal" action="" method="post">
  <table width="523" border="1" cellpadding="3" cellspacing="0" class="TablaInterior">
    <tr align="center">
      <td height="30" colspan="2"><strong>TIPO DE MOVIMIENTO EMBARQUE</strong></td>
    </tr>
    <tr> 
...
<?php
	...
	if (($Producto == "48") || ($Producto == "18" && $SubProducto != "5"))
	{
		...
		if (($Producto == "18") || ($Producto == "48")) //modificado por WSO
		{
...
?>	
  ...
  <?php  
	...
	$Consulta = "SELECT STRAIGHT_JOIN sum(t1.peso_paquetes) as peso,t2.cod_bulto,t2.num_bulto, t2.corr_enm ";
	$Consulta.= "from sec_web.paquete_catodo t1 ";
	$Consulta.=" inner join sec_web.guia_despacho_emb t2 on t1.num_guia=t2.num_guia	 and t2.fecha_guia=t1.fecha_embarque ";
	...
	$Consulta.= " and t2.cod_estado != 'A'  group by t2.cod_bulto,t2.num_bulto";
	//echo "CON".$Consulta;
	$Respuesta = mysqli_query($link, $Consulta);
	while ($Fila = mysqli_fetch_array($Respuesta))
	{
		...
		$Consulta.= " and (year(fecha_creacion_lote) = '".$Ano."' || year(fecha_creacion_lote) = '".$AnoAux."')";
 ...
 //echo "FF".$FechaCreacionLote;
		...
		$Consulta.= " where t1.cod_bulto = '".$Fila["cod_bulto"]."' and t1.num_bulto = '".$Fila["num_bulto"]."')";
		$Consulta.= " and t1.corr_enm = '".$Fila["corr_enm"]."'";
