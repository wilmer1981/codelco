<?php
	...
	if ($opcion == "M")
	{
		...
		$consulta = "SELECT * FROM sec_web.totales_produccion WHERE fecha LIKE '".$ano."-".$mes."%' AND revision = '".$arreglo[1]."'";
	...
	if ($opcion == "N")
	{
		...
		$consulta = "SELECT IFNULL(MAX(revision)+1,0) AS revision FROM sec_web.totales_produccion";
		$consulta = $consulta." WHERE fecha LIKE '".$fecha_aux."%'";
		$rs = mysqli_query($link, $consulta);
		...
		?></select>
          ...
          <tr> 
            ...
            <td width="65"> 
              <?php
				if ($mostrar == "S")
					echo '<input name="txtrevision" type="text" size="10" value="'.$row[revision].'" disabled></td>';
				else 
					echo '<input name="txtrevision" type="text" size="10" value="'.$row[revision].'"></td>';
			?>
            <td width="150">
			<?php
				if ($row["estado"] == 1)
          ...
          <tr> 
            ...
            <td colspan="2"><input name="txtcatodos" type="text" size="10" value="<?php echo $row[cant_catodos_comerciales] ?>"></td>
          </tr>
          <tr> 
            <td>Descobrizacion Normal</td>
            <td colspan="2"><input name="txtdescobrizacion" type="text" size="10" value="<?php echo $row[cant_descobrizacion_normal] ?>"></td>
          </tr>
          <tr> 
            <td>Despuntes y L�minas</td>
            <td colspan="2"><input name="txtdespuntes" type="text" size="10" value="<?php echo $row[cant_despuntes_laminas]?>"></td>
          </tr>
        </table> 
        <br>
		<?php
