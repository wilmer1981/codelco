...
<form name="frmPrincipal" action="" method="post">
  <table width="523" border="1" cellpadding="3" cellspacing="0" class="TablaInterior">
    <tr> 
      <td colspan="2"><strong>PESAJE PAQUETES</strong></td>
    </tr>
    <tr> 
      <td colspan="2">&nbsp;</td>
    </tr>
...
?>
  <table width="<?php echo $LargoTabla; ?>" border="1" cellpadding="3" cellspacing="0" class="TablaDetalle">
    <tr align="center" class="ColorTabla01"> 
      <td width="121" rowspan="2">LOTE</td>
      <td width="121" rowspan="2">N.ENVIO</td>
      <td width="40" rowspan="2">N.CERT</td>
      <td width="73" rowspan="2" align="center">PESO Kg</td>
      <?php	
	reset($ArrLeyes);
...
?>
	...
	while ($Fila = mysqli_fetch_array($Respuesta))
	{
		...
		if ($Encontro == false)
		{
			$Consulta = "select distinct t1.cod_grupo, sum(t1.peso_paquetes) as peso_paquetes";
			$Consulta.= " from sec_web.paquete_catodo t1 inner join sec_web.lote_catodo t2 ";
			$Consulta.= " on t1.cod_paquete = t2.cod_paquete and t1.num_paquete = t2.num_paquete ";
			$Consulta.= " where t1.fecha_creacion_paquete between '".$FechaAux."' and '".$FechaAux."' ";
			$Consulta.= " and t1.cod_producto = '".$Producto."' and t1.cod_subproducto = '".$SubProducto."'";
			...
			while ($Fila3 = mysqli_fetch_array($Respuesta3))
			{
				...
				else
				{
					if (($Producto == "18") && (intval($Fila3["cod_grupo"]) >= 50))
					{
						...
						if (intval(substr($Fila3["fecha_creacion_paquete"],0,4)) >= 2004)															
					...
					{
						$Fecha1 = date("Y-m-d",mktime(0,0,0,substr($FechaAux,5,2),(substr($FechaAux,8,2)-4),substr($FechaAux,0,4)));
						$Fecha2 = date("Y-m-d",mktime(0,0,0,substr($FechaAux,5,2),(substr($FechaAux,8,2)+4),substr($FechaAux,0,4)));
					}
				}
