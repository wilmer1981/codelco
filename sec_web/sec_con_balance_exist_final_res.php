...
?>	
  ...
  <?php  
	...
	$Consulta.= " and ((t1.cod_estado = 'a' and t1.fecha_embarque = '00-00-0000') or (t1.cod_estado = 'c' and t1.fecha_embarque >= '".$FechaAux."'))";
	...
	if (strtoupper($MesConsulta) == "A")
	{
			...
			$Consulta.= " and year(t1.fecha_creacion_paquete) < '".$ano_7."' and t1.cod_paquete <= 'M') or ";
			$Consulta.= " (year(t1.fecha_creacion_paquete) = '".$ano_7."' and t1.cod_paquete = 'M'))";
	}
	else
	{
	...
	$Consulta.= " INNER join sec_web.traspaso t4 on t3.cod_bulto = t4.cod_bulto AND t3.num_bulto = t4.num_bulto";
	...
	$Consulta = "SELECT * from sec_web.tmp_stock_ini where";
	if ($ano_7 > $AnoFin)
	{
		 $Consulta.=" ano_creacion = ".$AnoFin." or ano_creacion= ".$ano_7." order by cod_bulto, num_bulto";
	}
	else
	{
		 $Consulta.=" ano_creacion  <= ".$AnoFin."  order by cod_bulto, num_bulto";
	}
	...
	while ($Fila = mysqli_fetch_array($Respuesta))
	{
		...
		//-------------------------------------------------------------------------------------------
		// ASIGNACION
	/*poy $Consulta = "SELECT * from sec_web.programa_codelco where corr_codelco='".$FilaAux["corr_enm"]."'";
		/*poly $Consulta = "SELECT * from sec_web.programa_codelco where corr_codelco='".$Fila["ie"]."'";*/
