...
function BuscaLeyesProduccion($CodBulto, $NumBulto, $Arreglo, $link)
{
	...
	$Consulta = "SELECT STRAIGHT_JOIN t2.cod_paquete, SUM(t2.peso_paquetes) as peso_sublote, year(t2.fecha_creacion_paquete) as ano, t3.cod_subclase as mes ";
	...
	{		
		...
		while(date($fecha_aux) <= date($fecha_ter)) //Recorre los dias.
		{
			...
			{		
				$Consulta = "SELECT STRAIGHT_JOIN t2.nro_solicitud, ifnull(t3.valor,0)as valor,t3.cod_leyes";
				...
				$Consulta = " SELECT STRAIGHT_JOIN t1.fecha_produccion, t2.nro_solicitud, t3.cod_leyes, ifnull(t3.valor,0) as valor, t3.cod_unidad, t4.conversion ";
				$Consulta.= " from sec_web.produccion_catodo t1";
		...
		//LEYES MENSUALES
		$Consulta = " SELECT STRAIGHT_JOIN t2.nro_solicitud, t3.cod_leyes, ifnull(t3.valor,0) as valor, t4.conversion";
		$Consulta.= " from cal_web.solicitud_analisis t2 ";
		$Consulta.= " left join cal_web.leyes_por_solicitud t3 on t2.fecha_hora=t3.fecha_hora ";
		$Consulta.= " and t2.nro_solicitud=t3.nro_solicitud and t3.cod_producto=t2.cod_producto ";
		$Consulta.= " and t3.cod_subproducto=t2.cod_subproducto inner join proyecto_modernizacion.unidades t4 ";
