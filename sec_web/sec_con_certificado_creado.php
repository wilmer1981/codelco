<?php 
	...
	if(isset($Fila["cod_producto"])){
		$CodProducto 	= $Fila["cod_producto"];
	}else{
		$CodProducto 	= "";
	}
	if(isset($Fila["cod_subproducto"])){
		$CodSubProducto = $Fila["cod_subproducto"];
	}else{
		$CodSubProducto	= "";
	}
	if(isset($Fil["cod_cliente"])){
		$CodCliente 	= $Fil["cod_cliente"];
	}else{
		$CodCliente 	= "" ;
	}
	if(isset($Fila["paquetes"])){
	...
	$Consulta.= " and corr_enm = '".$NumEnvio."'";
...
<?php	
	if (isset($Lote))
	{
		...
		{	
			...
			{*/
		//$Consulta2 = "SELECT * from sec_web.programa_codelco where corr_codelco = '".$Fila["corr_enm"]."'";
		$Consulta2 = "SELECT * from sec_web.programa_codelco where corr_codelco = '".$NumEnvio."'";
		$Respuesta2 = mysqli_query($link, $Consulta2);
		if ($Fila2 = mysqli_fetch_array($Respuesta2))
			$CodCliente = $Fila2["cod_cliente"];
		else
