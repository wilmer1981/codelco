...
if ($Opc=='M')
{
	$Consulta="select * from pcip_ere_productos t1 ";
	...
	if($Fila=mysql_fetch_array($Resp))
	{
		...
		$TxtIngreso=$Fila["cuenta_ingreso"];
		$TxtCostos=$Fila["cuenta_costos"];
		$CmbVig=$Fila["vigente"];		
	}
}
else
{
	$Consulta="select max(cod_producto+1) as maximo from pcip_ere_productos ";
	$Resp=mysql_query($Consulta);
	if($Fila=mysql_fetch_array($Resp))
	{
		$TxtCodigoPro=$Fila["maximo"];
	}
		$TxtProducto='';
		$TxtIngreso='';
		$TxtCostos='';
...
function Proceso(Opcion)
{
	...
	switch(Opcion)
	{
		case "N":
			...
			if(f.TxtIngreso.value=='')
			{
				alert("Debe Ingresar Cuenta Ingreso");
				f.TxtIngreso.focus();
				return;
			}
			if(f.TxtCostos.value=='')
			{
				alert("Debe Ingresar Cuenta Costos");
				f.TxtCostos.focus();
				return;
			}
				if(f.CmbVig.value=='-1')
			{
				...
				f.action = "pcip_mantenedor_productos_er_proceso01.php?Opcion="+Opcion+"&Codigo="+f.TxtCodigoPro.value+"&produc="+f.TxtProducto.value+"&produc="+f.TxtIngreso.value+"&produc="+f.TxtCostos.value+"&produc="+f.CmbVig.value;
				f.submit();
		        break;
		case "M":
			if(f.TxtCodigoPro.value=='')
			{
				alert("Debe Ingresar un Codigo Producto");
				f.TxtCodigoPro.focus();
				return;
			}
			if(f.TxtProducto.value=='')
			{
				...
				if(f.CmbVig.value=='-1')
			{
				...
				f.action = "pcip_mantenedor_productos_er_proceso01.php?Opcion="+Opcion+"&Codigo="+f.TxtCodigoPro.value+"&Produc="+f.TxtProducto.value+"&ingreso="+f.TxtIngreso.value+"&Costos="+f.TxtCostos.value+"&Vigente="+f.CmbVig.value;
				f.submit();
        		break;
		case "NI":
				f.action = "pcip_mantenedor_productos_er_proceso.php?Opc=N";
				f.submit();
				break;			
...
<?
if ($Opc=='N')
	echo '<body onLoad="document.FrmPopupProceso.TxtCodigoPro.focus();">';
	else
...
<table width="90%" align="center"  border="0" cellpadding="0"  cellspacing="0" class="TablaPricipalColor">
  ...
  <tr>
   ...
   <td><table width="100%" border="0" cellpadding="0" cellspacing="0">
     <tr>
       <td width="74%" align="left"><?	if($Opc=='N'){?><img src="../pcip_web/archivos/sub_tit_productos_ventas_n.png"><? }else{?><img src="../pcip_web/archivos/sub_tit_productos_ventas_m.png"><?	}?></td>
       <td align="right">
       ...
       <td align="center"><table width="100%" border="0" cellpadding="3" cellspacing="0" >
	     <tr>
           ...
           <td width="382" class="formulariosimple">
		    <input name="TxtCodigoPro" maxlength= "4" readonly="" onkeydown='TeclaPulsada(false)' size="3" type="text" id="TxtCodigoPro" value="<? echo $TxtCodigoPro; ?>">
            <span class="InputRojo">(*)</span></td>
         </tr>
	     <tr>
           <td width="111" class="formulario2">Producto</td>
           <td width="382" class="formulariosimple">
		    <input name="TxtProducto" maxlength= "80" size="80" type="text" id="TxtProducto" value="<? echo $TxtProducto; ?>">
            <span class="InputRojo">(*)</span></td>
         </tr>
	     <tr>
           <td width="111" class="formulario2">Cuenta Ingreso</td>
           <td width="382" class="formulariosimple">
		    <input name="TxtIngreso" maxlength= "6" size="20" onkeydown='TeclaPulsada(false)' type="text" id="TxtIngreso" value="<? echo $TxtIngreso; ?>">
            <span class="InputRojo">(*)</span></td>
         </tr>
	     <tr>
           <td width="111" class="formulario2">Cuenta Costos</td>
           <td width="382" class="formulariosimple">
		    <input name="TxtCostos" maxlength= "6" size="20" onkeydown='TeclaPulsada(false)' type="text" id="TxtCostos" value="<? echo $TxtCostos; ?>">
            <span class="InputRojo">(*)</span></td>
         </tr>
		 <tr>
...
<? 
	...
	if ($Mensaje==true)
		echo "alert('Producto Guardado Exitosamente');";
	if($Mensaje1==true)
	    echo "alert('Producto Modificado exitosamente');";			
	if($MensajeExiste==true)
	    echo "alert('Cuenta Ingreso Existente');";			
	echo "</script>";
?>