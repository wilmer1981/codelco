...
if ($Opc=='M')
{
	$Consulta="select * from pcip_fac_productos_facturas t1 ";
...
{
	$Consulta="select max(cod_producto+1) as maximo from pcip_fac_productos_facturas ";
	$Resp=mysql_query($Consulta);
	if($Fila=mysql_fetch_array($Resp))
	{
...
<?
	if ($Opc=='N')
		echo "<title>Nuevo Producto Facturas</title>";
		else	
			echo "<title>Modifica Producto Facturas</title>";
?>
	...
	switch(Opcion)
	{
		case "N":
				...
				if(f.CmbVig.value=='-1')
			{
				...
				f.action = "pcip_mantenedor_productos_facturas_proceso01.php?Opcion="+Opcion+"&Codigo="+f.TxtCodigo.value+"&Produc="+f.TxtProducto.value+"&Vigente="+f.CmbVig.value
				f.submit();
		        break;
		case "M":
			if(f.TxtProducto.value=='')
				...
				if(f.CmbVig.value=='-1')
			{
        		...
        		break;
		case "NI":
				f.action = "pcip_mantenedor_productos_facturas_proceso.php?Opc=N";
				f.submit();
				break;
...
<table width="90%" align="center"  border="0" cellpadding="0"  cellspacing="0" class="TablaPricipalColor">
  ...
  <tr>
   ...
   <td><table width="100%" border="0" cellpadding="0" cellspacing="0">
     <tr>
       <td width="74%" align="left"><?	if($Opc=='N'){?><img src="../pcip_web/archivos/sub_tit_productos_factura_n.png"><? }else{?><img src="../pcip_web/archivos/sub_tit_productos_factura_m.png"><?	}?></td>
       <td align="right">
