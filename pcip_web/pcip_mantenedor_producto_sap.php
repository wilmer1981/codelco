...
function Proceso(Opc)
{
	...
	{
		case "C":
			f.action="pcip_mantenedor_producto_sap.php?Buscar=S";
			f.submit();
		break;
		case "N":
			URL="pcip_mantenedor_producto_sap_proceso.php?Opc="+Opc;
		...
		case "M":
			if(SoloUnElemento(f.name,'CheckTipoDoc','M'))
			{
				...
				URL="pcip_mantenedor_producto_sap_proceso.php?Opc="+Opc+"&Valores="+Datos;
				opciones='top=30,toolbar=0,resizable=0,menubar=0,status=1,width=660,height=200,scrollbars=1';
				verificar_popup(popup);
				popup=window.open(URL,"",opciones);
				popup.focus();
		...
		case "E":
			if(SoloUnElemento(f.name,'CheckTipoDoc','E'))
			{
				...
				{
					Datos=Recuperar(f.name,'CheckTipoDoc');
					f.action='pcip_mantenedor_producto_sap_proceso01.php?Opcion=E&Valor='+ Datos; //Datos; //+"&Pagina="+f.Pagina.value;
					f.submit();
				}
			}	
		break;
		case "R":
			f.action = "pcip_mantenedor_producto_sap.php";
			f.submit();
		break;
		case "RE":
				URL="pcip_mantenedor_asigna_prodsap_matsvp.php";
...
{
	URL="pcip_relacion_mat_sap.php?Cod="+Cod;
...
<?
 $IP_SERV = $HTTP_HOST;
 EncabezadoPagina($IP_SERV,'mant_producto_sap.png')
 ?>
      ...
      <td>
		<table width="100%" cellpadding="2" cellspacing="0">
	     ...
	     <td width="81%" align="right" class='formulario2' >
				...
				<a href="JavaScript:Proceso('RE')"><img src="../pcip_web/archivos/btn_ingreso_obs2.png" border="0" alt="Relaci�n Codigo Sap - Codigo Materiales" align="absmiddle"></a> 
		  	  ...
		  	  <tr>
		  	    <td align="left" class='formulario2'>Codigo</td>
...
if($Buscar=='S')
{
	$Consulta = "select t1.cod_sap,t1.nom_sap,t2.nombre_subclase as nom_vig ";
	$Consulta.= " from pcip_svp_productos_sap t1 inner join proyecto_modernizacion.sub_clase t2 on t2.cod_clase='31007' and t2.cod_subclase=t1.vigente";
	$Consulta.=" where t1.cod_sap<>''";
	if($CmbProd!='T')
		$Consulta.=" and t1.cod_sap='".$CmbProd."'";
	if($Txtproducto!='')	
		$Consulta.=" and  upper(t1.nom_sap) like ('%".strtoupper(trim($Txtproducto))."%')";
	if($CmbVig!='T') 
...
?>		
      	<tr >
        <td  align="center"><? echo "<input name='CheckTipoDoc' class='SinBorde' type='checkbox'  value='".$Fila["cod_sap"]."'>" ?></td>
