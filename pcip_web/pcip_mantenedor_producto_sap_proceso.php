...
if ($Opc=='M')
{
	$Consulta="select * from pcip_svp_productos_sap t1 ";
	$Consulta.=" where t1.cod_sap='".$Valores."' ";
	$Resp=mysqli_query($link, $Consulta);
	if($Fila=mysql_fetch_array($Resp))
	{
		$TxtCodigo=$Fila["cod_sap"];
		$TxtProducto=$Fila["nom_sap"];
		$CmbVig=$Fila["vigente"];
	}
}
else
...
<?
	if ($Opc=='N')
		echo "<title>Nuevo Producto Sap</title>";
		else	
			echo "<title>Modifica Producto Sap</title>";
?>
...
function Proceso(Opcion)
{
	...
	switch(Opcion)
	{
		case "N":
			...
			if (Veri==true)
			{
				f.action = "pcip_mantenedor_producto_sap_proceso01.php?Opcion="+ Opcion+"&Codigo="+f.TxtCodigo.value+"&produc="+f.TxtProducto.value+"&vigente="+f.CmbVig.value;
				f.submit();
			}
		break;
		case "M":
			...
			if (Veri==true)
			{
				f.action = "pcip_mantenedor_producto_sap_proceso01.php?Opcion="+ Opcion+"&Codigo="+f.TxtCodigo.value+"&produc="+f.TxtProducto.value+"&vigente="+f.CmbVig;
				f.submit();
			}
		break;
		case "NI":
				f.action = "pcip_mantenedor_producto_sap_proceso.php?Opc=N";
				f.submit();
				break;
...
<input type="hidden" name="Volver" value="<? echo $Volver;?>">
  ...
  <tr>
   ...
   <td><table width="100%" border="0" cellpadding="0" cellspacing="0">
     <tr>
       <td width="74%" align="left"><?	if($Opc=='N'){?><img src="../pcip_web/archivos/sub_tit_producto_sap_n.png"><? }else{?><img src="../pcip_web/archivos/sub_tit_producto_sap_m.png"><?	}?></td>
       <td align="right">
           ...
           <td width="382" class="formulariosimple">
		    <?
			  if($Opc=='N')
			   {
			?>
		    <input name="TxtCodigo" maxlength= "8"  size="12" type="text" id="TxtCodigo" value="<? echo $TxtCodigo; ?>">
