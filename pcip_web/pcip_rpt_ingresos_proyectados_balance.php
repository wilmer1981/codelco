...
<html>
<head>
<title>Reporte Ingresos Proyectados Balance</title>
<style type="text/css">
<!--
body {
...
function Proceso(TipoProceso)
{
	...
	{
		case "C":
			f.action = "pcip_rpt_ingresos_proyectados_balance.php?Buscar=S";
			f.submit();
		break;
		case "E"://GENERA EXCEL
			URL='pcip_rpt_ingresos_proyectados_balance_excel.php?&Ano='+f.Ano.value+'&Mes='+f.Mes.value+'&MesFin='+f.MesFin.value;
			window.open(URL,"","top=30,left=30,width=1000,height=550,status=yes,menubar=yes,resizable=yes,scrollbars=yes");
		break;				
		case "R":
			f.action = "pcip_rpt_ingresos_proyectados_balance.php";
			f.submit();
			break;	
		case "I"://IMPRIMIR
			window.print();
...
<?
 $IP_SERV = $HTTP_HOST;
 EncabezadoPagina($IP_SERV,'mant_rpt_balance.png')
?>
    ...
    <table width="100%" align="center"  border="0" cellpadding="0"  cellspacing="0" class="TablaPricipalColor">
      <tr>
        ...
        <td><table width="100%" border="1" align="center" cellpadding="2" cellspacing="0" >
          <tr>
            <td class="LinkPestanaActiva" align="center" colspan="25">BALANCE DIVISIONAL</td>	
		  </tr>
          <?		
  			 if($Buscar=='S')
			 {				 
				...
				{	
					...
					?>
					<tr>
					<td class="TituloTablaVerde" align="center" colspan="2"><? echo strtoupper($NomArea);?></td>
					<?
					for($i=$Mes;$i<=$MesFin;$i++)
					{
					?>
					<td class="TituloTablaVerde"align="center" colspan="2"><? echo $Meses[$i-1]."&nbsp;".$Ano?></td>
					<?	
					}
					?>            
				    </tr>
					  <tr>
						<td class="TituloTablaVerde" align="center" colspan="2">Tipo/Calidad</td>						   	
						<?
						for($i=$Mes;$i<=$MesFin;$i++)
						{
						?>
						<td width="100" class="TituloTablaVerde" align="center" >Real</td>
						<td width="100" class="TituloTablaVerde" align="center">Proyectado</td>
						<?	
						}
						?>
					  </tr>
					...
					while(list($c,$v)=each($CodDiv))
					{
						...
						if($CodigoArea!='2')
						{
							...
							echo "<td class='TituloTablaNaranja'>TM</td>";
							for($i=$Mes;$i<=$MesFin;$i++)
							{
								$ValorTotalReal=$ArrTotalRealTM[$i][0];
								$ValorTotalPpto=$ArrTotalPptoTM[$i][0];
								echo "<td align='right' class='TituloTablaNaranja'>".number_format($ValorTotalReal,0,',','.')."</td>";
								echo "<td align='right' class='TituloTablaNaranja'>".number_format($ValorTotalPpto,0,',','.')."</td>";
							}	
							echo "</tr>";
							echo "<tr>";
							echo "<td class='TituloTablaNaranja'>LEY %</td>";
							for($i=$Mes;$i<=$MesFin;$i++)
							{	
								...
								if($ValorTotalPpto>0)		
									...
									$ValorTotalLeyPpto=0;
								echo "<td align='right' class='TituloTablaNaranja'>".number_format($ValorTotalLeyReal,1,',','.')."%</td>";
								echo "<td align='right' class='TituloTablaNaranja'>".number_format($ValorTotalLeyPpto,1,',','.')."%</td>";
							}	
							echo "</tr>";
							echo "<tr>";
							echo "<td class='TituloTablaNaranja'>TMF</td>";
							for($i=$Mes;$i<=$MesFin;$i++)
							{	
								$ValorTotalTMFReal=$ArrTotalRealTMF[$i][0];
								$ValorTotalTMFPpto=$ArrTotalPptoTMF[$i][0];
								echo "<td align='right' class='TituloTablaNaranja'>".number_format($ValorTotalTMFReal,0,',','.')."</td>";
								echo "<td align='right' class='TituloTablaNaranja'>".number_format($ValorTotalTMFPpto,0,',','.')."</td>";
							}	
						  echo "</tr>";
						}  
						if($CodigoArea=='1')
						{
						   ...
						   for($i=$Mes;$i<=$MesFin;$i++)
						   {
							   ...
							   echo "<td>&nbsp;</td>";
							   echo "<td align='right' class='titulo_rojo'>".number_format($Dato,0,',','.')."</td>";
						   }
						   echo "</tr>";
						}
				 ...
				 ?>
					<tr>
					<td class="titulo_rojo" align="center" colspan="2">PAGABLES</td>
					<?
					for($i=$Mes;$i<=$MesFin;$i++)
					{
					?>
					...
					?>            
					  ...
					  <tr>
						<td class="TituloTablaVerde" align="center" colspan="2">CATODOS</td>						   	
						<?
						for($i=$Mes;$i<=$MesFin;$i++)
						{
						?>
		    ...
		    <?
						//CONSULTA PARA OBTENER LOS CATODOS PRIMERA ETAPA
						...
						{	
						   ...
						   echo "<tr>";//SUBTOTALES PRIMEROS CATODOS
							 ...
							 for($i=$Mes;$i<=$MesFin;$i++)
							 {
								...
								echo "<td class='TituloTablaNaranja' align='right'>".number_format($ValorSubTotalReal,0,',','.')."</td>";
							    echo "<td class='TituloTablaNaranja' align='right'>".number_format($ValorSubTotalPpto,0,',','.')."</td>";
						...
						{	
						   ...
						   echo "<tr>";//SUBTOTALES SEGUNDA CARTODOS
							 ...
							 for($i=$Mes;$i<=$MesFin;$i++)
							 {
								...
								echo "<td class='TituloTablaNaranja' align='right'>".number_format($ValorSubTotalReal2,0,',','.')."</td>";
								echo "<td class='TituloTablaNaranja' align='right'>".number_format($ValorSubTotalPpto2,0,',','.')."</td>";
						...
						?>						
							<tr>
							<td class="titulo_rojo" align="center" colspan="2">RECHAZADOS</td>
							<?
							for($i=$Mes;$i<=$MesFin;$i++)
							{
							?>
							 ...
							 echo "<td align='center'>TMF</td>";
								   for($i=$Mes;$i<=$MesFin;$i++)
								   {
									...
									$ValorSubTotalPptoRecha=$ArrSubTotalRechaPpto[$i][0];
									 echo "<td align='right' class='TituloTablaNaranja'>".number_format($ValorSubTotalRealRecha,0,',','.')."</td>";
									 echo "<td align='right' class='TituloTablaNaranja'>".number_format($ValorSubTotalPptoRecha,0,',','.')."</td>";	
						   ...
						   echo "<tr>";
							 ...
							 echo "<td align='center'>TMF</td>";
								   for($i=$Mes;$i<=$MesFin;$i++)
								   {
									 ...
									 echo "<td align='right' class='TituloTablaNaranja'>".number_format($TotalRealRecha,0,',','.')."</td>";
									 echo "<td align='right' class='TituloTablaNaranja'>".number_format($TotalPptoRecha,0,',','.')."</td>";	
									 	
								   } 	 
						   echo "</tr>";
						      ?>
								<tr>
								<td class="titulo_rojo" align="center" colspan="2">SALIDA</td>
								<?
								for($i=$Mes;$i<=$MesFin;$i++)
								{
								?>
			  ...
			  <?
								   ...
								   echo "<tr>";
									 ...
									 echo "<td align='center'>TMF</td>";
										   for($i=$Mes;$i<=$MesFin;$i++)
										   {
											 ...
											 echo "<td align='right' class='TituloTablaNaranja'>".number_format($ValorDubclaseRealSalida,0,',','.')."</td>";
											 echo "<td align='right' class='TituloTablaNaranja'>&nbsp;</td>";
									 ...
									 echo "<td align='center'>TMF</td>";
										   for($i=$Mes;$i<=$MesFin;$i++)
										   {
										    $ValorTotalRealSalida2=$ArrSubTotalSalidaReal2[$i][0];
											 echo "<td align='right' class='TituloTablaNaranja'>".number_format($ValorTotalRealSalida2,0,',','.')."</td>";
											 echo "<td align='right' class='TituloTablaNaranja'>&nbsp;</td>";	
					    	...
					    	<?
								...
								while($Fila1=mysql_fetch_array($Resp1))
								{
								   ...
								   echo "<tr>";
									 ...
									 echo "<td align='center'>TMF</td>";
										   for($i=$Mes;$i<=$MesFin;$i++)
										   {
											 ...
											 echo "<td align='right' class='TituloTablaNaranja'>".number_format($ValorTotalRealSalida3,0,',','.')."</td>";
											 echo "<td align='right' class='TituloTablaNaranja'>&nbsp;</td>";	
								   ...
								   echo "<tr>";
									 ...
									 echo "<td align='center'>TMF</td>";
										   for($i=$Mes;$i<=$MesFin;$i++)
										   {
											 ...
											 echo "<td align='right' class='TituloTablaNaranja'>".number_format($ValorTotalRealSalida4,0,',','.')."</td>";
											 echo "<td align='right' class='TituloTablaNaranja'>".number_format($ValorTotalPptoSalida4,0,',','.')."</td>";											 
								  ...
								  echo"</tr>";	
								   echo "<tr>";
									 echo "<td class='TituloTablaVerde' colspan='2'>VENTAS SUBPRODUCTOS</td>";
									for($i=$Mes;$i<=$MesFin;$i++)
									{										
										echo "<td width='100' class='TituloTablaVerde' align='center' >Real</td>";
										echo "<td width='100' class='TituloTablaVerde' align='center'>Proyectado</td>";
									}
								   echo "</tr>";	 
								   echo "<tr>";
