...
<html>
<head>
<title>Productos Variaciones Inventarios</title>
...
function Proceso(Opc)
{
	...
	{
		case "C":
			f.action="pcip_mantenedor_variaciones_inventarios_productos.php?&Buscar=S";
			f.submit();
		break;
		case "N":
			URL="pcip_mantenedor_variaciones_inventarios_productos_proceso.php?Opc="+Opc;
		...
		case "M":
			if(SoloUnElemento(f.name,'CheckTipoDoc','M'))
			{
				...
				URL="pcip_mantenedor_variaciones_inventarios_productos_proceso.php?Opc="+Opc+"&Valores="+Datos;
		...
		case "E":
			if(SoloUnElemento(f.name,'CheckTipoDoc','E'))
			{
				...
				{
					Datos=Recuperar(f.name,'CheckTipoDoc');
					f.action='pcip_mantenedor_variaciones_inventarios_productos_proceso01.php?Opcion=E&Valor='+ Datos; //Datos; //+"&Pagina="+f.Pagina.value;
					f.submit();
				}
			}	
		break;
		case "R":
			f.action = "pcip_mantenedor_variaciones_inventarios_productos.php";
			f.submit();
		break;				
		case "S":
...
<?
 $IP_SERV = $HTTP_HOST;
 EncabezadoPagina($IP_SERV,'mant_productos_inventarios.png')
 ?>
...
if($Buscar=='S')
{
	...
	$Consulta.= " from pcip_svp_productos_inventarios t1 inner join proyecto_modernizacion.sub_clase t2 on t2.cod_clase='31007' and t2.cod_subclase=t1.vigente";
...
?>		
      	<tr >
        <td  align="center"><? echo "<input name='CheckTipoDoc' class='SinBorde' type='checkbox'  value='".$Cod."'>" ?></td>
