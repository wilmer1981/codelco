...
if ($Opc=='M')
{
	$Consulta="select * from pcip_cdv_productos_ventas t1 ";
...
function Proceso(Opcion)
{
	...
	switch(Opcion)
	{
		case "N":
		    ...
		    if(f.TxtCodigo.value==0)
			{
				alert("Debe Ingresar un Codigo Mayor a Cero");
				f.TxtCodigo.focus();
				return;
			}		    
				...
				if(f.CmbVig.value=='-1')
			{
				...
				f.action = "pcip_mantenedor_productos_ventas_proceso01.php?Opcion="+Opcion+"&Codigo="+f.TxtCodigo.value+"&Produc="+f.TxtProducto.value+"&Vigente="+f.CmbVig.value
				f.submit();
		        break;
		case "M":
			if(f.TxtCodigo.value=='')
				...
				if(f.CmbVig.value=='-1')
			{
        		...
        		break;
		case "NI":
				f.action = "pcip_mantenedor_productos_ventas_proceso01.php?Opcion="+Opcion;
				f.submit();
				break;
