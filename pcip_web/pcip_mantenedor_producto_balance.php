...
function Proceso(Opc)
{
	...
	{
		case "C":
			f.action="pcip_mantenedor_producto_balance.php?&Buscar=S";
			f.submit();
		break;
		case "N":
			URL="pcip_mantenedor_producto_balance_proceso.php?Opc="+Opc;
			opciones='top=30,toolbar=0,resizable=0,menubar=0,status=1,width=660,height=300,scrollbars=1';
		...
		case "M":
			if(SoloUnElemento(f.name,'CheckTipoDoc','M'))
			{
				...
				URL="pcip_mantenedor_producto_balance_proceso.php?Opc="+Opc+"&Valores="+Datos;
				opciones='top=30,toolbar=0,resizable=0,menubar=0,status=1,width=660,height=300,scrollbars=1';
				verificar_popup(popup);
				popup=window.open(URL,"",opciones);
				popup.focus();
		...
		case "E":
			if(SoloUnElemento(f.name,'CheckTipoDoc','E'))
			{
				...
				{
					Datos=Recuperar(f.name,'CheckTipoDoc');
					f.action='pcip_mantenedor_producto_balance_proceso01.php?Opcion=E&Valor='+ Datos; //Datos; //+"&Pagina="+f.Pagina.value;
					f.submit();
				}
			}	
		break;
		case "R":
			f.action = "pcip_mantenedor_producto_balance.php";
			f.submit();
		break;						
		case "S":
...
<?
 $IP_SERV = $HTTP_HOST;
 EncabezadoPagina($IP_SERV,'mant_productos_balance.png')
 ?>
      ...
      <td>
		<table width="100%" cellpadding="2" cellspacing="0">
	     ...
	     <td width="81%" align="right" class='formulario2' >
		  	  ...
		  	  <tr>
		  	    <td align="left" class='formulario2'>Codigo Producto</td>
				  ...
				  $Consulta = "select * from pcip_svp_productos_etapas order by cod_producto_etapa ";			
					...
					while ($FilaTC=mysql_fetch_array($Resp))
					{
						if ($CmbProd==$FilaTC["cod_producto_etapa"])
							echo "<option selected value='".$FilaTC["cod_producto_etapa"]."'>".str_pad($FilaTC["cod_producto_etapa"],3,'0',STR_PAD_LEFT)."&nbsp;&nbsp;".ucfirst($FilaTC["nom_producto_etapa"])."</option>\n";
						else
							echo "<option value='".$FilaTC["cod_producto_etapa"]."'>".str_pad($FilaTC["cod_producto_etapa"],3,'0',STR_PAD_LEFT)."&nbsp;&nbsp;".ucfirst($FilaTC["nom_producto_etapa"])."</option>\n";
					}
				   ?>
				</select>
				</td>
		  ...
		  <tr>
           ...
           <td width="382" class="formulariosimple" ><input name="Txtproducto" maxlength= "20" type="text" id="Txtproducto" style="width:150" value="<? echo $Txtproducto ?>">           </td>
         </tr>
		 <tr>
	...
	<table width="930" border="1" cellpadding="4" cellspacing="0" >
	  ...
	  <tr align="center">
        <td width="3%" class="TituloTablaVerde"><input class='SinBorde' type="checkbox" name="ChkTodos" value="" onClick="CheckearTodo(this.form,'CheckTipoDoc','ChkTodos');"></td>
          ...
          <td width="5%" class="TituloTablaVerde">Codigo</td>
          <td width="41%" class="TituloTablaVerde">Producto</td>
		  <td width="38%" class="TituloTablaVerde">Tipo Balance</td>
		  <td width="13%" class="TituloTablaVerde">Vigente</td>
		 </tr>
<?
if($Buscar=='S')
{
	$Consulta = "select t1.cod_producto_etapa,t1.nom_producto_etapa,t2.nombre_subclase as nom_balance,t3.nombre_subclase as nom_vig";
	$Consulta.= " from pcip_svp_productos_etapas t1 left join proyecto_modernizacion.sub_clase t2 on t2.cod_clase='31002' and t2.cod_subclase=t1.cod_tipo_balance";
	$Consulta.= " left join proyecto_modernizacion.sub_clase t3 on t3.cod_clase='31007' and t1.vigente=t3.cod_subclase";
	$Consulta.=" where t1.cod_producto_etapa<>''";
	if($CmbProd!='T')
		$Consulta.=" and t1.cod_producto_etapa='".$CmbProd."'";
	if($Txtproducto!='')	
		$Consulta.=" and  upper(t1.nom_producto_etapa) like ('%".strtoupper(trim($Txtproducto))."%')";
	if($CmbTipo!='T') 
	    $Consulta.=" and  t1.cod_tipo_balance ='".$CmbTipo."'";	
	if($CmbVig!='T') 
	...
	{
		$Cod=$Fila["cod_producto_etapa"];
		$Prod =$Fila["nom_producto_etapa"];
		$TipoBalance=$Fila["nom_balance"];
		$Vig =$Fila["nom_vig"];
?>		
      	<tr >
        <td  align="center"><? echo "<input name='CheckTipoDoc' class='SinBorde' type='checkbox'  value='".$Fila["cod_producto_etapa"]."'>" ?></td>
		<td align="center"><? echo $Cod; ?></td>
        <td><? echo $Prod; ?></td>
		<td><? echo $TipoBalance; ?></td>
