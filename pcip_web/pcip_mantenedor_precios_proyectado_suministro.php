...
<html>
<head>
<title>Mantenedor Precios Suministros</title>
<style type="text/css">
<!--
body {
...
function Procesos(TipoProceso)
{
	...
	{
		case 'N'://GRABAR
			var URL = "../pcip_web/pcip_mantenedor_precios_proyectado_suministro_proceso.php?Opcion=N";
		...
		case "M":
			if(SoloUnElemento(f.name,'CheckSumi','M'))
			{
				...
				{
					URL="../pcip_web/pcip_mantenedor_precios_proyectado_suministro_proceso.php?Opcion=M&Codigos="+Valores;
					window.open(URL,"","top=30,left=30,width=850,height=550,status=yes,menubar=no,resizable=yes,scrollbars=yes");
				}
			}
		break;
		case "C":
			f.action = "pcip_mantenedor_precios_proyectado_suministro.php?Buscar=S";
			f.submit();
		break;
		case "E"://ELIMINAR
			var Valores="";
		...
		case "I"://IMPRIMIR
			...
			break;
		case "R":
			f.action = "pcip_mantenedor_precios_proyectado_suministro.php";
			f.submit();
		break;
		case "S":
...
<?
 $IP_SERV = $HTTP_HOST;
 EncabezadoPagina($IP_SERV,'precio_suministros.png')
?>
	...
	while($Fila=mysql_fetch_array($Resp))
	{
			...
			<?
			 ...
			 for($i=1;$i<=12;$i++)
			 {
				...
				<td align="right">
				<? 
				echo number_format($FilaMes[valor],2,',','.');
			 ...
			 ?>
			 <td align="right"><? echo number_format($Acum,2,',','.');?></td>
		</tr>
	<?
		$Cont++;
	}
...
<? 
	...
	if($Cont==0&&$Buscar=='S')
		echo "alert('Informaci�n de Precios de Suministros No Encontrada');";	
	echo "</script>";
?>	
</body>
</html>