...
<html>
<head>
<title>Productos Facturas</title>
...
function Proceso(Opc)
{
	...
	{
		case "C":
			f.action="pcip_mantenedor_productos_facturas.php?&Buscar=S&CmbProducto=-1";
			f.submit();
		break;
		case "N":
			URL="pcip_mantenedor_productos_facturas_proceso.php?Opc="+Opc;
		...
		case "M":
			if(SoloUnElemento(f.name,'CheckTipoDoc','M'))
			{
				...
				URL="pcip_mantenedor_productos_facturas_proceso.php?Opc="+Opc+"&Valores="+Datos;
				opciones='top=30,toolbar=0,resizable=0,menubar=0,status=1,width=660,height=400,scrollbars=1';
				verificar_popup(popup);
				popup=window.open(URL,"",opciones);
				popup.focus();
		...
		case "E":
			if(SoloUnElemento(f.name,'CheckTipoDoc','E'))
			{
				...
				{
					Datos=Recuperar(f.name,'CheckTipoDoc');
					f.action='pcip_mantenedor_productos_facturas_proceso01.php?Opcion=E&Valor='+ Datos; //Datos; //+"&Pagina="+f.Pagina.value;
					f.submit();
				}
			}	
		break;
...
<?
 $IP_SERV = $HTTP_HOST;
 EncabezadoPagina($IP_SERV,'mant_productos_facturas.png')
 ?>
    ...
    <tr>
      ...
      <tr>
    	...
    	<td colspan="3" class="formulario2" ><select name="CmbProducto" onChange="Proceso('R')">
			  ...
			  <?
				$Consulta = "select cod_producto,nom_producto from pcip_fac_productos_facturas order by cod_producto ";			
...
<table width="950" align="center"  border="0" cellpadding="0"  cellspacing="0" class="TablaPricipalColor">
  ...
  <td ><img src="../pcip_web/archivos/images/interior/esq2em.gif" width="15" /></td>
	  ...
	  <tr align="center">
          ...
          <td width="14%" class="TituloTablaVerde">Codigo</td>
          <td width="65%" class="TituloTablaVerde">Producto Facturas</td>
...
if($Buscar=='S')
{
	...
	$Consulta.= " from pcip_fac_productos_facturas t1 inner join proyecto_modernizacion.sub_clase t2 on t2.cod_clase='31007' and t2.cod_subclase=t1.vigente ";
