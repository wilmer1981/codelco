...
<html>
<head>
<title> Mantenedor Asignaciones de Productos</title>
...
function Proceso(Opc)
{
	...
	{
		case "C":
			f.action="pcip_mantenedor_asignaciones_productos.php?&Buscar=S&CmbAsig=-1";
			f.submit();
		break;
		case "N":
			URL="pcip_mantenedor_asignaciones_productos_proceso.php?Opc="+Opc;
		...
		case "M":
			if(SoloUnElemento(f.name,'CheckTipoDoc','M'))
			{
				...
				URL="pcip_mantenedor_asignaciones_productos_proceso.php?Opc="+Opc+"&Valores="+Datos;
				opciones='top=30,toolbar=0,resizable=0,menubar=0,status=1,width=750,height=400,scrollbars=1';
				verificar_popup(popup);
				popup=window.open(URL,"",opciones);
				popup.focus();
		...
		case "E":
			if(SoloUnElemento(f.name,'CheckTipoDoc','E'))
			{
				...
				{
					Datos=Recuperar(f.name,'CheckTipoDoc');
					f.action='pcip_mantenedor_asignaciones_productos_proceso01.php?Opcion=E&Valor='+ Datos; //Datos; //+"&Pagina="+f.Pagina.value;
					f.submit();
				}
			}	
		break;
		case "R":
			f.action = "pcip_mantenedor_asignaciones_productos.php";
			f.submit();
		break;
		case "S":
...
<?
 $IP_SERV = $HTTP_HOST;
 EncabezadoPagina($IP_SERV,'mant_asignaciones_productos.png')
 ?>
    ...
    <tr>
      ...
      <tr>
    	...
    	<td colspan="3" class="formulario2" ><select name="CmbAsig" onChange="Proceso('R')">
			  ...
			  <?
				$Consulta = "select cod_asignacion,nom_asignacion from pcip_svp_asignacion  order by nom_asignacion ";			
...
<table width="950" align="center"  border="0" cellpadding="0"  cellspacing="0" class="TablaPricipalColor">
  ...
  <td ><img src="../pcip_web/archivos/images/interior/esq2em.gif" width="15" /></td>
	  ...
	  <tr align="center">
          ...
          <td width="11%" class="TituloTablaVerde">Codigo Producto </td>
          <td width="13%" class="TituloTablaVerde">Tipo Asignaci&oacute;n</td>		  
          <td width="23%" class="TituloTablaVerde">Nombre Asignaci&oacute;nes Productos</td>
		  <td width="7%" class="TituloTablaVerde">Orden</td>
		  <td width="8%" class="TituloTablaVerde">Unidad</td>
		  <td width="11%" class="TituloTablaVerde">Mostrar PPC</td>
  		  <td width="16%" class="TituloTablaVerde">Considerar en Reporte. Cobre Elect (TMF) </td>
		  <td width="8%" class="TituloTablaVerde">Vigente</td>
	  </tr>
<?
if($Buscar=='S')
{
	$Consulta = "select t1.cod_producto,t1.tipo,t1.mostrar_ppc,t1.mostrar_cu_elect,t2.nom_asignacion as nom_asig_tipo,t1.nom_asignacion as nom_asig_pro,t1.orden,t3.nombre_subclase as nom_vig,t1.cod_unidad";
	...
	while ($Fila=mysql_fetch_array($Resp))
	{
		...
		$Tipo=$Fila["tipo"];
		$MostrarPpc=$Fila["mostrar_ppc"];
		$MostrarCu=$Fila["mostrar_cu_elect"];
		$Vig=$Fila["nom_vig"];	
...
?>		
		...
		<td align="center">&nbsp;<? echo $MostrarCu; ?></td>
