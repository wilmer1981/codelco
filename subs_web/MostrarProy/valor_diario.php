...
<head>
	...
	<!---------------------------------  FUNCIONES  -------------------------------->
		...
		// Funcion  que me abre la ventana que me muestra el Imp. Unico de un mes dado
		{
			...
			window.open("detalle_iu.php?valor_afecto="+ valor_afecto + "&mes=" +palabrames+"&ano="+ano+"&rebaja="+rebaja,""," fullscreen=no,left=0,top=10,width=412,height=395,scrollbars=yes,resizable = no");					
		}
		<!------  Validar ----------->
		function Validar()
		...
		{
			var f = document.form1;
			if ((f.dia1.value=="") || (f.dia2.value=="")|| (f.dia3.value=="") || (f.imponible1.value=="") || (f.imponible2.value=="") || (f.imponible3.value=="")) 
			{
				alert("Tiene Alg�n Campo en Blanco");
				return('no_ok');
			} else {
				return('ok');
			}
		...
		// Funcion que me pasa el valor "total a pagar" al la pagina de licencias
		{
			...
			{
				window.close();
				window.opener.document.form1.action ="ing_trab.php?porc_salud="+porc_salud+"&opcion=AF&filasel="+filaselec+"&colocar_valor="+total_a_pagar+"&n="+numfila;
...
/*******Formatear_Fecha************/
	...
	/*    descuento de valor afectado neto   */
		// --- Primera fila --- //
		$fechadetalle1=$ano1."-".sacarmes($mes1)."-01";
		...
		if ($row = mysql_fetch_array($respuesta))
		{
		  ...
		  //$total_afecto[1]=(($valor_afecto_1*$row[factor])-$row[rebaja])-$valor_afecto_1;						
		...
		// --- Segunda fila --- //
		$fechadetalle2=$ano2."-".sacarmes($mes2)."-01";
		...
		{		
			$desc_valor_afecto[2]= $row[rebaja];
	   		//Valor total afectado (Incluido su descuento)	 
			$total_afecto[2]=round($valor_afecto_2-($row[factor]*$row[rebaja]));					
			$mostrar_total_afecto[2]=number_format($total_afecto[2],0,',','.');			
		} else {
			$muestra_msg2='s';
		}		
		// --- Tercera fila --- //
		$fechadetalle3=$ano3."-".sacarmes($mes3)."-01";
...
//		$respuesta = mysql_query($conssql);		//.$valor_afecto_1."
		...
		if ($row = mysql_fetch_array($respuesta3))
		{
		   ...
		   //Valor total afectado (Incluido su descuento)	 
			$total_afecto[3]=round($valor_afecto_3-($row[factor]*$row[rebaja]));							
        ...
        <table width="705" border="1" align="center"cellpadding="2"  cellspacing="0" class="TablaInterior">
          <tr align="left" class="ColorTabla01" >
						...
						<tr class="ColorTabla02" align="center">
							<td width="223"><b><? echo $mes1;?>&nbsp;&nbsp;<? echo $ano1; ?></b></td>
							<td width="230"><b><? echo $mes2;?>&nbsp;&nbsp;<? echo $ano2; ?></b></td>
							<td width="231"><b><? echo $mes3;?>&nbsp;&nbsp;<? echo $ano3; ?></b></td>				
						</tr>
						<tr>							
							<td>D&iacute;as Trabajados :&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" name="dia1" size="6" value="<? echo $dia1; ?>"  style="text-align:right" onKeyUp="puntitos(this,this.value.charAt(this.value.length-1))" tabindex="1" class="InputDer"></td>
							...
							<td>D&iacute;as Trabajados :&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" name="dia3"  size="6" value="<? echo $dia3;?>"  style="text-align:right" onKeyUp="puntitos(this,this.value.charAt(this.value.length-1))" tabindex="5" class="InputDer"></td>
							
						</tr>
						<tr>
							<td>Valor Imponible($):&nbsp&nbsp;&nbsp;&nbsp;<input type="text" name="imponible1" value="<? echo number_format($imponible1,0,',','.');?>" size="14"  style="text-align:right"  onKeyUp="puntitos(this,this.value.charAt(this.value.length-1))" tabindex="2" class="InputDer"></td>							
							...
							<td>Valor Imponible($):&nbsp&nbsp;&nbsp;&nbsp;<input type="text" name="imponible3" value="<? echo number_format($imponible3,0,',','.');?>" size="14"  style="text-align:right" onKeyUp="puntitos(this,this.value.charAt(this.value.length-1))" tabindex="6" class="InputDer"></td>							
						</tr>						
			  </table>	
       ...
       <!----------------------------------- Fin Tabla de Ingreso de imponibilidad  ------>
          ...
          <tr align="center" class="ColorTabla01">
    	      ...
    	      <tr align="center" class="ColorTabla02"> 				
				  ...
				  <td width="41"><div align="center"><b>Isapre<br>(UF)</b></div></td>
			...
			<tr> 			  
			  ...
			  <td align="right"><? echo $uf_isapre[1];?></td>
			...
			<tr> 
			  ...
			  <td align="right"><? echo $uf_isapre[2];?></td>
			...
			<tr> 
			  ...
			  <td align="right"><? echo $uf_isapre[3]; ?></td>
          ...
          <tr align="center">
					<td>	
						...
						<input type="button" name="BtnAceptar" value="Aceptar" style="width:70px" onClick="Aceptar(<? echo "'".$numfila."','".$filaselec."','".$total_a_pagar."','".$pac_uf."'";?>)"> 
