...
<head>
	...
	<!---------------------------------  FUNCIONES  -------------------------------->
		...
		function SIL (rut_func)
		{
			f = document.form1;
			window.open("PopUp_SIL.php?rut_completo="+ rut_func+"-"+f.dv.value,"SIL"," fullscreen=no,left=90,top=30,width=783,height=470,scrollbars=yes,resizable = no");			
		}
		<!--- Detalle --->		
		function Detalle()
			...
			for (i=1;i<f.elements.length;i++)
				{
					if (f.elements[i].name=="opt_detalle" && f.elements[i].checked)
					{
						...
						{
							F_Desde = f.elements[i-6].value;
							F_Hasta = f.elements[i-5].value;
							num_dias=f.elements[i-4].value;
							valor_licencia=f.elements[i-1].value;
							valor_diario=f.elements[i-2].value;
							window.open("detalle_licencia.php?valor_diario="+valor_diario+"&num_dias="+num_dias+"&valor_licencia="+valor_licencia+"&FechaDesde="+F_Desde+"&FechaHasta="+F_Hasta ,"verdetalle"," fullscreen=no,left=0,top=10,width=375,height=315,scrollbars=yes,resizable = no");			
						} else {						
		...
		// Funcion que  me oculta los botones
		{
			...
			for (i=1;i<f.elements.length;i++)			
			{
				// Si el radio tiene valor 's' entonces capturos sus datos 
				if (f.elements[i].checked && f.elements[i].name=="Chk_mostrar["+punt+"]")
				{
					f.elements[i+4].disabled=true;
					f.elements[i+4].value='s';
					f.elements[i+5].value=f.elements[i-3].value;					
					//f.elements[i+6].value=Math.round(parseFloat(f.elements[i-3].value)* parseInt(f.elements[i+3].value));
					F_Desde = f.elements[i+1].value;
					F_Hasta = f.elements[i+2].value;
					punt=i;			
				} 
				if (f.elements[i].checked==false && f.elements[i].name=="Chk_mostrar["+punt+"]")
				{
					f.elements[i+4].disabled=false;
					f.elements[i+4].value='n';
					f.elements[i+6].value="";
					f.elements[i+5].value=""; 	
					f.elements[i+1].value=""; 	
				}	
	
			} // fin : for..
/*			if (f.elements[punt].checked)
			{
				if (F_Desde == "" && F_Hasta == "")
				{
					alert("Debe Ingresar las 2 fechas");
		...
		function VerificaBotones(punt)
		{
			f = document.form1;
			var i;
			f.elements[19].disabled=true;			
			for (i=1;i<f.elements.length;i++)			
			{
				// Si el radio tiene valor 's' entonces capturos sus datos 
				if (f.elements[i].name=="Chk_mostrar"+punt && f.elements[i+4].value=='s' )
				{
					f.elements[i+4].disabled=true;
					f.elements[i+6].value="";
					f.elements[i+5].value="";
		...
		// Funcion que vuelve a la seleccion de funcionario
		{
			...
			Frm.action = "selecc_func.php";
			Frm.submit();
		}	
		<!------- Proceso (Guardar) ---------->
		function Guardar(opc, numero)
		// Funcion que vuelve a la seleccion de funcionario
		{
			var f = document.form1;
			f.action = "ing_trab.php?grabar="+opc+"porc_salud="+f.pac_uf.value+"&opcion=AF&n=" + numero +"#punto" ;
			f.submit();
		}
		
		function Desabilita(opcion)
		{
			var f = document.form1;	
			if (f.tipo_desc[0].checked==true)
				{
					f.tipod.value = f.tipo_desc[0].value;
					f.tipo_desc[1].checked=false;
				}
			if	(f.tipo_desc[1].checked==true)
				{
					f.tipo_desc[0].checked=false;
					f.tipod.value = f.tipo_desc[1].value;
				}
			if (f.tipo_desc[0].checked==false && f.tipo_desc[1].checked==false)
				{
	...
	<tr>
      <td height="313" valign="top">
  		  ...
  		  <table width="608" border="1" align="center" cellpadding="2" id="dv" cellspacing="0" class="TablaInterior">
				...
				?>			
					...
					<td>
						...
						<input type="radio" name="tipo_desc" value="U" onClick="Desabilita('U')"> 
						<label><? echo $nom_salud; ?></label>
						<input type="radio" name="tipo_desc"  value="P" onClick="Desabilita('P')">
						<label>%</label>
		  ...
		  <table width="608" border="1" align="center" cellpadding="2"  cellspacing="0" class="TablaInterior">
			<tr class="ColorTabla01">
			  <td colspan="8" ><strong>Licencias</strong></td>
			</tr>
			<tr class="ColorTabla02" align="center"> 
			  <td width="25%" colspan="2"><b>Desde&nbsp;<em>(dd/mm/aaaa)</em></b></td>
			  <td width="25%"><b>Hasta&nbsp;<em>(dd/mm/aaaa)</em></b></td>
			  <td width="16%" ><b>D&iacute;as de Licencias</b></td>
  			  <td width="10%"><b>Calcular</b></td>			  
			  <td width="26%"><b>Valor Diario</b></td>  
  			  <td width="26%"><b>Valor Licencia</b></td>  
				 ...
				 $v_diario[$numero]= $valor_diario;
				   for($i=0;$i<$n;$i++){
					  ...
					  echo '<tr align="center">';
					  	   //eval("if (isset(\$Chk_mostrar[".$i."]) && \$Chk_mostrar[".$i."]=='S') \$Chequeado='S'; else \$Chequeado='N';");
						  if (isset($Chk_mostrar[$i]) && $Chk_mostrar[$i]=='S') $Chequeado='S'; else $Chequeado='N';
						  if ($Chequeado=="S")
						   {
						   	  $Bloqueado="disabled";
							  $pagado_lic[$i]=$dias_lic[$i]*$v_diario[$i];
	  						  $ano_hasta=$fecha_hasta[6]."".$fecha_hasta[7]."".$fecha_hasta[8]."".$fecha_hasta[9];
						      $fecha_hasta=$hasta_lic[$i-1];
	  						  $ano_hasta=$fecha_hasta[6]."".$fecha_hasta[7]."".$fecha_hasta[8]."".$fecha_hasta[9];
							  $mes_hasta=$fecha_hasta[3]."".$fecha_hasta[4];
							  $num_dia_mes =date("t",mktime(1,0,0,$mes_hasta,02,$ano_hasta));
							  $dia_hasta=$fecha_hasta[0]."".$fecha_hasta[1]; 
							  if ($dia_hasta==30 || $dia_hasta==31 || $dia_hasta==28) 	
							  {
							  	if ($mes_hasta==12)
								{
							  ...
							  {
								$dia_hasta=$dia_hasta+1;					 	
							  }
							  if ($dia_hasta<10)
							  {
								  $dia_hasta="0".$dia_hasta;					 		
							  }						  
 							  $desde_lic[$i]=$dia_hasta."/".$mes_hasta."/".$ano_hasta;							
							  echo '<td><input type="checkbox" checked name="Chk_mostrar['.$i.']" onClick="Verificar('.$i.','.$n.')" value="S"></td>';
						  }
						  else
						  {
						  	  $Bloqueado="";							  
						  	  echo '<td><input type="checkbox" name="Chk_mostrar['.$i.']" onClick="Verificar('.$i.','.$n.')" value="S"></td>';							  
						  }
						  ...
						  if ($dias_lic[$i]==9) // Si la licencia tiene 10 dias entonces se le descuenta 3 dias
						  {
						  		
							$dias_lic[$i]=$dias_lic[$i]-3;	
						  }	
						
						  $dias_lic[$i]=$dias_lic[$i]+1;	

						  if (($desde_lic[$i]=="") || ($hasta_lic[$i]=="")) /*|| ($dias_lic[$i]-1<0))*/
						  {
							  $dias_lic[$i]=0;
						  }						  
						  echo '<td><input type="text" class="InputDer" name="dias_lic['.$i.']" value="'.$dias_lic[$i].'" size="4" maxlength="3" ></td>';						  						  				
						  ?><td><input <? echo $Bloqueado; ?> type="radio" name="opt_calcular" value="<? echo $i; ?>" onClick="Calcular(<? echo $n.",".$i; ?>)"  ></td><?						   						  
						  echo '<td><input type="text"  class="InputColor" name="v_diario['.$i.']" value="'.str_replace(',','.',$v_diario[$i]).'" size="11" maxlength="10" onKeyUp="'.$punt.'" readonly > </td>';						  						  
						  ...
						  ?><td><input  type="radio" name="opt_detalle" value="<? echo $ocultar ?>" onClick="Detalle()" ></td><?						    													
					  echo '</tr>'; 
			
					 // $num_dia_mes =date("t",mktime(0,0,0,$puntmes,02,$puntano));						    
					  echo '<script>';
					  	echo ' VerificaBotones();';		   
					  echo '</script>';
					  $num = $n;
					  $valor = "";
				   }/*** Fin del For...   ***/
			...
			<tr class="ColorTabla02"> 
			  <td colspan="8" >
				<div align="center">
					...
					<div align="center"><font face="Arial, Helvetica, sans-serif"> 
						<? if ($num=='')
							{$num=0;}?>
												...
												<!-- <input type="button" name="BotonGrabar" value="Grabar" class="ErrorSI" style="width:100" onClick="Grabar(<? //echo $num;?>)">-->
	
						<input type="button" name="BotonActualizar" value="Actualizar" class="ErrorSI" style="width:100" onClick="Actualizar(<? echo $num;?>)">
					...
					<!--	<input name="BtnSIL" class="ErrorSI" type="button" value="S.I.L" style="width:100" onClick="SIL(<? //echo $rut_trab;?>)">-->
					</font></div>
				</div>
			  </td>
			</tr>
	  ...
	  <br><br>
        <table width="400" border="0" cellspacing="0" cellpadding="3" class="TablaInterior" align="center">
          <tr align="center" valign="middle">
			...
			<!--	<td><input name="BtnGuardar" type="button" value="Guardar" style="width:70px" onClick="Guardar('s',<? //echo $num; ?>)"></td>-->
