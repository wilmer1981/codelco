...
<head>
	...
	<!---------------------------------  FUNCIONES  -------------------------------->
		...
		{		
			...
			{
				f.elements[col+4].value = Number(replace_all(f.elements[col+1].value,".",""))+1; // Me suma uno mas
				puntitos(f.elements[col+4],f.elements[col+4].value.charAt(f.elements[col+4].value.length-1)); //Separador de Miles
			}
			if((f.elements[col+5].value != "")&&(Number(replace_all(f.elements[col+5].value,".","")) > Number(replace_all(f.elements[col+1].value,".",""))))
			{
				f.elements[col+8].value = Number(replace_all(f.elements[col+5].value,".",""))+1; // Me suma uno mas
				puntitos(f.elements[col+8],f.elements[col+8].value.charAt(f.elements[col+8].value.length-1)); //Separador de Miles
			}
			if((f.elements[col+9].value != "")&&(Number(replace_all(f.elements[col+9].value,".","")) > Number(replace_all(f.elements[col+5].value,".",""))))
			{
				f.elements[col+12].value = Number(replace_all(f.elements[col+9].value,".",""))+1;// Me suma uno mas
				puntitos(f.elements[col+12],f.elements[col+12].value.charAt(f.elements[col+12].value.length-1)); //Separador de Miles
			}
			if((f.elements[col+13].value != "")&&(Number(replace_all(f.elements[col+13].value,".","")) > Number(replace_all(f.elements[col+9].value,".",""))))
			{
				f.elements[col+16].value = Number(replace_all(f.elements[col+13].value,".",""))+1;// Me suma uno mas
				puntitos(f.elements[col+16],f.elements[col+16].value.charAt(f.elements[col+16].value.length-1)); //Separador de Miles
			}
			if((f.elements[col+17].value != "")&&(Number(replace_all(f.elements[col+17].value,".","")) > Number(replace_all(f.elements[col+13].value,".",""))))
			{
				f.elements[col+20].value = Number(replace_all(f.elements[col+17].value,".",""))+1;// Me suma uno mas
				puntitos(f.elements[col+20],f.elements[col+20].value.charAt(f.elements[col+20].value.length-1)); //Separador de Miles
			}
			if((f.elements[col+21].value != "")&&(Number(replace_all(f.elements[col+21].value,".","")) > Number(replace_all(f.elements[col+17].value,".",""))))
			{
				f.elements[col+24].value = Number(replace_all(f.elements[col+21].value,".",""))+1;// Me suma uno mas
				puntitos(f.elements[col+24],f.elements[col+24].value.charAt(f.elements[col+24].value.length-1)); //Separador de Miles
			}
			if((f.elements[col+25].value != "")&&(Number(replace_all(f.elements[col+25].value,".","")) > Number(replace_all(f.elements[col+21].value,".",""))))
			{
				f.elements[col+28].value = Number(replace_all(f.elements[col+25].value,".",""))+1;// Me suma uno mas
				puntitos(f.elements[col+28],f.elements[col+28].value.charAt(f.elements[col+28].value.length-1)); //Separador de Miles
			}
		}
		<!-----------  Recargar -------->		
							   ...
							   while($row=mysql_fetch_array($respuesta)){
								  ...
								  if ($row[hasta]==-1){	
								  	echo '<td><input type="text" name="hasta['.$i.']" value="y M&aacute;s..." class="InputDer" size="10" maxlength="9" ></td>';
	 								  ...
	 								  echo '<td><input type="text" name="hasta['.$i.']" value="'.number_format($row[hasta],0,',','.').'"  size="10" maxlength="9" class="InputDer" onkeyup="puntitos(this,this.value.charAt(this.value.length-1))" onBlur="copiar()"></td>';//
								  }								
