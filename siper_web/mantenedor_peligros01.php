<?
	...
	switch($Proceso)
	{
		case "AP"://AGREGAR CONTACTO/PELIGRO
			...
			if($CheckSel==true)
				$Sel='1';	
			$Insertar="INSERT INTO sgrs_codcontactos (CCONTACTO,NCONTACTO,MOPCIONAL,QPROBHIST,QCONSECHIST,FCONTACTO,MVIGENTE,OBS) values('".$CodConta.$CmbCod."','".trim($TxtDescripcion)."','1','".$CmbProbH."','".$CmbConsH."','".date('Y-m-d')."','".$Vig."','".$OBS."')";
		...
		case "MP"://MODIFICAR CONTACTO/PELIGRO
			...
			$PH=$Fila[QPROBHIST];
			$PC=$Fila[QCONSECHIST];
			$Actualizar="UPDATE sgrs_codcontactos set NCONTACTO='".trim($TxtDescripcion)."',MOPCIONAL='1',QPROBHIST='".$CmbProbH."',QCONSECHIST='".$CmbConsH."',MVIGENTE='".$Vig."',OBS='".$OBS."' where CCONTACTO='".$CodConta."'";
