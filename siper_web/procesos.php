	...
	switch($Proceso)
	{
		...
		case "EI"://ELIMINAR ITEM ORGANICA
			...
			if(!$Fila=mysql_fetch_array($Resp))		
			{
				...
				if(!$Fila=mysql_fetch_array($Resp))		
				{
					...
					$RespAreaNom=mysql_query($ConsultaAreaNom);
					$FilaAreaNom=mysql_fetch_array($RespAreaNom);
		...
		case "MP"://MODIFICAR OBSERVACION A PELIGRO ASOCIADO A TAREA
			...
			foreach($Datos as $c => $v)
			{
				...
				//echo $Actualizar."<br>";
				mysql_query($Actualizar);
				$Obs='El Peligro: '.$DatosObs[1]." - ".$NOMPELI.' Se Modific�: Observaci�n '.$OBSPRINCIPAL.'. Asociado a La Tarea: '.$NOMAREA.'';	
				$Obs2='Por Observaci�n: '.$ObsAux.'. Asociado a La Tarea: '.$NOMAREA.'';	
		...
		case "GC"://AGREGAR CONTROLES AL PELIGRO
			...
			foreach($Datos as $c => $v)
			{
				...
				if($Fila=mysql_fetch_array($Resp))
				{
					...
					$Obs1='En el Control '.$NOMCONTROL.' del Peligro: '.$NOMCONTACT.', Se a Modificado OBSERVACION: '.$OBS.' y Especificaci�n de Control '.$ESPE.' de: '.$NOMAREA.'';	
					$Obs2='Reemplazado por:  OBSERVACION: '.$ObsAux.' y Especificaci�n de Control '.$EspCtrl.' de la �rea: '.$NOMAREA.'';	
				...
				else
				{
					...
					if($Datos2[1]!='NA'||($Datos2[2]!=''||$Datos2[3]!='NA'||$Datos2[4]!=''))//NO APLICA NO SE INSERTAR
					{
						...
						$Obs='Se a Ingresado el Siguiente Familia de Control: '.$NOMCONTROL.' en el Peligro: '.$NOMCONTACT.', Observaci�n: '.$Datos2[2].', Verificador: '.$NONVERI.' y Especificaci�n de Control '.$Datos2[4].' de la �rea: '.$NOMAREA.'';	
		...
		case "EC"://ELIMINAR CONTROLES
			...
			$Obs='Se a Eliminan Los Controles Asociados al Peligro '.$NOMCONTACT.' del �rea: '.$NOMAREA.'';	
			//echo $ObsEli."<br>";
		...
		case "VFIP"://VALIDAR FIN IDENTIFICACION DE PELIGROS
			...
			if($NOTIENECONTROLES!='S')// QUE UN CONTROL NO TENGA
			{
				...
				$Obs='A Finalizado identificaci�n de Peligros Para la Tarea: '.$NOMAREA.'.';	
		...
		case "ELUG"://ELIMINAR LUGAR MEDICION
			...
			while(list($clave,$Codigo)=each($Datos))
			{
				...
				{
					$Mensaje='No se puede Eliminar Lugar de Medici�n, Existen Mediciones asociadas';
					echo "<script languaje=javascript>";
					echo "alert('".$Mensaje."');";
					echo "</script>";
				}	
	...
	$Mensaje="<font face='Arial, Helvetica, sans-serif' size='2'>El Usuario ".strtoupper($Nombre)." a finalizado la identificaci�n  de peligros de la Tarea ".$NOMAREA.", para su Validaci�n.</font>";	
		 ...
		 $Mensaje.="<table width='100%' border='1' cellpadding='0' cellspacing='0' align='left'>";
			$Mensaje.="<tr>";
				...
				$Mensaje.="<td align='left' width='30%' bgcolor='#CCCCCC' ><table width='100%' border='1' cellspacing='0' cellpadding='0'><tr><td width='70%'><font face='Arial, Helvetica, sans-serif' color='#9c3031' size='1'>Famila de Controles</font></td><td width='30%'><font face='Arial, Helvetica, sans-serif' color='#9c3031' size='1'>Especificaci�n</font></td></tr></font></table></td>";
				$Mensaje.="<td width='35%' bgcolor='#CCCCCC'><table width='100%' border='1' cellspacing='0' cellpadding='0'><tr><td width='70%'><font face='Arial, Helvetica, sans-serif' color='#9c3031' size='1'>Famila de Verificadores</font></td><td width='30%'><font face='Arial, Helvetica, sans-serif' color='#9c3031' size='1'>Especificaci�n</font></td></tr></font></table></td>";
...
*/				//$Mensaje.="<td align='center' width='4%'><font face='Arial, Helvetica, sans-serif' size='1'>".$Descrip."</font></td>";
	...
	$cuerpoMsj.= '<table  width="100%"  border="0" align="center">';
	$cuerpoMsj.= '<tr><td>';
	$cuerpoMsj.= "<font face='Arial, Helvetica, sans-serif' size='2'>Seguir el link para realizar evaluaci�n de la Magnitud de Riesgo Residual: <a href='http://".$NomServ."/proyecto/siper_web/proceso_magnitud_riesgo.php?CodSelTarea=".$CodSelTarea."&MRCAL=".$MRCAL."&U=".$User."&UPrin=".$UserPrin."'><font size='2'>MRr.</font></a>";
	//$cuerpoMsj.= "<font face='Arial, Helvetica, sans-serif' size='2'>Seguir el link para realizar evaluaci�n de la Magnitud de Riesgo Residual: <a href='http://codelco.cl'><font size='2'>MRr.</font></a>";
	$cuerpoMsj.= "<br><br>";
