...
<html>
<head>
<title>Mantenedor Ex�menes</title>
...
function Proceso(Opc)
{
	...
	switch(Opc)
	{
		case "N":
				...
				f.Proc.value='N';
				f.Datos.value=Datos;
				f.action='mantenedor_examenes.php?VisibleDivProceso=S&Buscar=S';
				f.submit();		
		break;
		case "M":
			...
			if(f.TxtNombre.value!='')
			{
				if(f.CmbUnidad.value!='S')
				{
					if(f.TxtQPARAMETRO.value!='')
					{
						...
						f.action='mantenedor_examenes01.php?Proceso='+f.Proc.value;
						f.submit();
					}
					else
					{
					alert('Debe Ingresar Par�metro ')
					f.TxtQPARAMETRO.focus();
					}	
				}	
				else
				{
		...
		break;
		case "C":
				f.action='mantenedor_examenes.php?Buscar=S';
				f.submit();		
		break;
		case "E":
			if(SoloUnElemento(f.name,'CheckRut','E'))
			{
				...
				if(mensaje==true)
				{
					...
					f.action='mantenedor_examenes01.php?Proceso=E&DatosUni='+DatosUni;
					f.submit();
				}
			}	
		break;
...
<table width="71%" align="center"  border="0" cellpadding="0"  cellspacing="0">
  ...
  <tr>
   ...
   <td align="center"><table width="96%" border="0" cellpadding="0"  cellspacing="0">
     <tr>
	 <td align="center" colspan="2" class="TituloCabecera2">Mantenedor de Ex�menes</td>
	 </tr>
	 <tr>       
    ...
    <td><table width="100%" border="1" align="center" cellpadding="2" cellspacing="0">
      <tr>
	    ...
	    <td width="65%" align="center" class="TituloCabecera">Descripci&oacute;n </td>
          <td width="10%" align="center" class="TituloCabecera">Par�metro</td>
      ...
      <?
		//if($Buscar=='S')
		//{
			$Consulta = "SELECT *,t2.NUNIDAD from sgrs_codexlaboratorio t1 left join sgrs_unidades t2 on t1.CUNIDAD=t2.CUNIDAD ";
			$Consulta.=" where not isnull(CTEXAMEN)  ";
			if($TxtDescripcion!='')
				$Consulta.= " and upper(NEXAMEN) like('%".strtoupper($TxtDescripcion)."%') ";
		...
		?>
			 	<tr>
				<td align="center" ><? echo "<input name='CheckRut' class='SinBorde' type='checkbox'  value='".$Fila["CTEXAMEN"]."'"; ?></td>
				<td ><? echo $Fila["NEXAMEN"]."&nbsp;"; ?></td>
				<td align="center" ><? echo $Fila["QPARAMETRO"]; ?>&nbsp;</td>
  ...
  <tr>
    ...
    <td align="center"><table width="403" border="0" cellpadding="0" cellspacing="0">
	  ...
	  <? 
	  	if($Proc=='M')
		{
			$Consulta="Select * from sgrs_codexlaboratorio where CTEXAMEN='".$Datos."' ";
			$Resp1 = mysqli_query($link, $Consulta);
			if ($Fila1=mysql_fetch_array($Resp1))
			{
			 $TxtNombre=$Fila1[NEXAMEN];
			 $TxtQPARAMETRO=$Fila1[QPARAMETRO];
			 $CmbUnidad=$Fila1[CUNIDAD];
	  ...
	  ?>
	    ...
	    <tr>
        <td class="formulario">Par�metro</td>
		<td > <input name="TxtQPARAMETRO" type="text" value="<? echo $TxtQPARAMETRO; ?>" size="15"  maxlength="5" ><span class="InputRojo">(*)</span></td>
      </tr>
	   <tr>
