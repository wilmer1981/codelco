<?
	if ($Elim=="S")
	{
		...
		if (unlink($Dir.$ArchivoElim)) // Borra un fichero
		{
...
function Generar()
{
	...
	/*if(f.select1.value=='Fundicion')
	
	{
	Tabla='enabal';
	}
	if(f.select1.value=='PMN')
	{
	Tabla='enabalpmn';
	}*/
	//alert(Tabla);
...
//<a href="ingresobd.php">ingresobd.php</a>
function DelFile(arch)
{
	...
	if (msg==true)
	{
		f.action="leerarchivos_finales.php?Elim=S&ArchivoElim="+arch;
		f.submit();
	}
	else
	{
...
<table width="173" border="1" class="TablaDetalle">
    <tr>
      ...
      <td><select name="select1">
        <option value="enabal" selected>Fundicion</option>
        <option value="enabalpmn">PMN</option>
      </select></td>
    </tr>
  </table>
...
<?
$ArrArchivos = array(); // Define un arreglo 
$Dir = '../met_web/archivos/';
$Directorio=opendir($Dir); // Abre un gestor de directorio
$i=0;
while ($Archivo = readdir($Directorio)) //  Lee una entrada desde un gestor de directorio
{
	if($Archivo != '..' && $Archivo !='.' && $Archivo !='')
	{ 		
		$FechaHora = date("d-m-Y", filemtime($Dir."/".$Archivo)); // Devuelve la hora en que el archivo fue modificado por última vez
		$Peso = filesize($Dir."/".$Archivo); // Devuelve el tamaño del archivo en bytes
...
closedir($Directorio); //cierra el manejador de directorios
if (count($ArrArchivo)>0)
{
	reset($ArrArchivo); // Fija el puntero interno de una matriz a su primer elemento 
	krsort($ArrArchivo); // Ordena una matriz por clave en orden inverso
...
</table><a href="AnodosTotalesDF.php">Datos Finales - Anodos Totales</a>
</form>
<br>
</body>
</html>
