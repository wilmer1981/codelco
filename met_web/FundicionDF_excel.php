...
<table width="770" height="330" border="0" cellpadding="0" cellspacing="0">
  <tr>
    <td align="center" valign="top"><br>      <table width="324" height="69" border="1" align="center" cellpadding="0" cellspacing="0">
      <tr align="center">
        <td height="14" colspan="5"><strong>Flujos Historicos Fundicion</strong></td>
      </tr>
      <tr>
        <td height="26"><strong>N&uacute;mero de Flujo : </strong></td>
        <td><div align="left"><? echo $txtflujo;?></div></td>
        <td colspan="3" rowspan="2"><div align="left"><? echo $TxtNomProd;?></div></td>
        </tr>
      <tr>
	...
	if ($buscarproOPT=="S")
	{
		$sql = "SELECT NOM_PRODUCTO FROM enabal where N_FLUJO='$txtflujo'"; 
	...
	if ($buscarOPT=="S")
		...
		$sql = "SELECT SUM(P_SECO) as PESOSECO, SUM(F_COBRE) as FINOCOBRE, SUM(F_PLATA) as FINOPLATA, SUM(F_ORO) as FINORO FROM enabal where ((N_FLUJO='$txtflujo') AND (NOM_PRODUCTO='$TxtNomProd') AND (T_MOV='$select2') AND (FECHA BETWEEN '$FechaIni' and '$FechaFin'))"; 
	...
	if ($buscarOPT=="S")
	{
		...
		$sql = "SELECT * FROM enabal where N_FLUJO='$txtflujo' AND  T_MOV='$select2' and NOM_PRODUCTO='$TxtNomProd' and FECHA BETWEEN ' $FechaIni' and '$FechaFin' ORDER BY FECHA"; 	
		$resultados = mysql_query($sql);
